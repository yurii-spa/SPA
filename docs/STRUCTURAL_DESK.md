...
|---|---:|---:|---:|:--:|
| rates_desk | $330,315 | 22.9289 | $64,779 | yes |
| rwa_floor | $592,254,111 | 3.4000 | $0 | yes |
| stable_engines | $130,000 | 4.5000 | $1,430 | no |

- **Naive sum deployable:** $592,714,426
- **Correlation haircut:** −$165,157 (applied: True)
- **Combined deployable AUM:** **$592,549,268**
- **Blended net APY:** **3.4057%/yr**
- **Combined yield ABOVE the floor:** **$33,956/yr** (0.3396% of the $10M/yr target)
- **Gap to $10M/yr:** $9,966,044/yr
- **Binding constraint:** **RWA_YIELD_TOO_LOW**

> **Honest combined fundability verdict.** COMBINED book across 3 sleeve families: rates desk $330,315 @ 22.9289% (above-floor), RWA floor $592,254,111 @ 3.4% (AT floor, deep), stable engines $130,000 @ 4.5%. Naive sum $592,714,426 − correlation haircut $165,157 (shared stablecoin-venue exit liquidity, 50% of the binding overlap) = $592,549,268 combined deployable at a blended 3.41%/yr (RWA floor 3.40%/yr) → $33,956/yr ABOVE the floor. That is only 0.34% of the $10M/yr target — a gap of $9,966,044/yr. Honest verdict: even the COMBINED real-market book is well short of $10M/yr above the floor at today's market depth. Binding constraint: RWA_YIELD_TOO_LOW. The deep family (RWA, $592,254,111 deployable) yields AT the floor → it adds ~$0 above floor despite being by far the largest book. The only above-floor edge is the thin rates desk ($64,779/yr). Depth is not the problem — the deep book's yield is at the floor, and the above-floor edge (rates) is too thin to clear $10M. What would CLOSE the gap: (1) DEEPER PT markets — more per-book carry depth lifts the only meaningful above-floor source; (2) MORE venues/chains — decorrelating the exit rails shrinks the correlation haircut so the deep + carry books become additive; and (3) REAL AUM at the floor — the deep RWA family banks the floor today (~$0 above), so its huge depth only becomes above-floor dollars once real capital is deployed AND a spread over the floor is captured. The honest truth: the combined book is a real, diversified, fail-closed research book, but $10M/yr above floor is a SCALE + DECORRELATION + real-AUM play, not reachable on the current thin above-floor edge.

<!-- END combined portfolio-capacity (portfolio_capacity) -->

## N-book capacity — does stacking MORE gated-carry books scale the above-floor edge?

_Deterministic finer-grained companion to the combined model above (`spa_core/strategy_lab/rates_desk/n_book_capacity.py`; endpoint `/api/rates-desk/n-book-capacity`; re-run `python3 -m spa_core.strategy_lab.rates_desk.n_book_capacity`). It answers the recurring "the edge is a $100k/$250k artifact" objection with a MEASURED curve rather than an assertion._

The single-book ceiling (`capacity.py`) shows one book's above-floor $/yr peaks near ~$5–6k because pouring more AUM into the same thin Pendle/Ethena pools compresses its APY toward the floor. This model asks the other question — **what happens as you add more DISTINCT gated-carry books?** — by composing two things the repo already measures (it invents no new depth): the per-book HONEST deployable depth with the same-venue correlation haircut (`venue_expansion.live_curve`) and the single-pool APY→AUM compression curve (`capacity.py aum_levels`). Books are grouped by rail cluster; a cluster is credited its haircut deployable and reads its spread at the cluster total (same-rail books share depth → they do NOT get additive fresh carry); distinct rails are additive.

**Honest result at today's market depth:** 23 fundable PT books span only **2 distinct rails** (Ethena/USDe + Sky/sUSDS) → above-floor $/yr rises from ~$3.1k (1 book) and **plateaus at ~$5.3k/yr**, with the **marginal value of the 23rd book ≈ $9/yr**. The curve makes the honest point unmissable: **you cannot scale the above-floor edge by stacking books on one rail — the ceiling is set by the number of DISTINCT deep rails.** Closing the gap needs deeper PT markets and more decorrelated venues/chains (which shrink the correlation haircut), not more same-rail books. Advisory — never gates, never moves capital.
