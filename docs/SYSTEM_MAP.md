...
## Мета-стек продуктов владельца (owner, 2026-08-22)

Владелец строит ТРИ продукта, вложенных как матрёшка, и SPA — только внутренний:

| Слой | Продукт | Что строит |
|---|---|---|
| 1 (внутренний) | **SPA** — этот репозиторий | сам DeFi yield-продукт (paper-трек → go-live) |
| 2 | **Студия DeFi OS** | продукт постройки SPA — «продукт постройки нашего продукта» |
| 3 (внешний) | **Бридж** | продукт постройки Студии — «продукт постройки студио дефай» |

Дословно (owner 22.08): «я еще отдельно строю студия дефай ос — это продукт постройки нашего
продукта, и еще есть бридж — это продукт постройки студио дефай».

**Практические следствия для сессий SPA:**
- Слои 2–3 живут ВНЕ этого репозитория. В SPA от них виден только touchpoint —
  агент/бот `studio_bridge` на Маке (шлёт СВОИМ токеном `STUDIO_BRIDGE_TELEGRAM_TOKEN`;
  проверен и оправдан при разборе 409-конфликтов 22.08 — к `com.spa.telegram_bot` не причастен).
- Процесс/сообщение/файл из студии или бриджа — НЕ баг SPA: не «чинить», не убивать,
  не заводить карточку «неизвестный агент». Сомнение → спросить владельца.
- Задачи по слоям 2–3 в бэклог SPA не тянуть, пока владелец явно не принесёт их сюда.
