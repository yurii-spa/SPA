...
**Проблемы (→ карточки):**
- 🟢 **`/admin/*` ЗА Cloudflare Access** (проверено curl → «Sign in · CF Access»; в РЕПО не видно, настроено в CF-дашборде — аудит по коду ошибочно решил «no auth»). Косметика: убрать устаревший коммент «Phase 5 no auth» в admin/index.astro. БЫЛО ошибочно помечено 🔴 БЕЗ аутентификации (по комменту файла «access control = Phase 5, no auth yet»).
