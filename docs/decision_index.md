...
| **CAND-PTSUSDE-001** | Pendle PT-sUSDe fixed carry (11.2%) | **WATCH → CONDITIONAL-ADVANCE** | real **8-12% band** (11.2% fixed); the fixed-to-maturity wrapper **REMOVES the funding-flip tail** that made spot sUSDe a lean-REFUSE → residual = **measurable USDe-solvency-to-maturity** + PT liquidity. Fundable IF that one risk is underwritten + capped. *Structure, not asset, sets the risk* | ~**780 bps** | L2 | `data/strategy_candidates/pt_susde_fixed.candidate.md` |
