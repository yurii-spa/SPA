...
| **CAND-PRIVCRED-001b** | Centrifuge DROP (senior tranche) | **WATCH** | real bound: DROP(senior) absorbs loss LAST, TIN(junior) = first-loss buffer + real RWA cashflows (TVL >$500M). ~8% = low end *because* senior. Gated on per-pool off-chain DD (junior-buffer depth, asset quality, lockup) | ~**460 bps** | L2 | `data/strategy_candidates/private_credit_senior.candidate.md` |
