...
_Обновлено: 2026-07-16 (автономный цикл оркестратора: owner-queue dead-letter hardening, ветка `env-setup-v3`)._
...
- **Hardening (автономный цикл, 2026-07-16, #2):** закрыт **dead-letter баг owner-queue**
  (`spa_core/owner_queue/queue.py`) — `create_card` писал `status:` только `if status`, поэтому карточка
  без явного статуса рождалась БЕЗ поля и была невидима во ВСЕХ фильтрах (вкл. owner-очередь `needs-owner`),
  а `set_status` её не чинил (raise). Теперь: статус обязателен (дефолт по типу трекера — owner-decision→
  needs-owner / inbox→new), `set_status` вставляет отсутствующую строку. Починена реально сломанная карточка
  `owner-decision-20260715-212059-apy` (owner ответил в чате + исполнено pid67426, но карточка была невидима)
  → теперь `needs-owner`, ждёт финальный owner-done владельца. +3 регресс-теста, 18 зелёных (705 в смежном
  наборе). Пуш кодом (правит баг, порогов/решений не меняет). Детали — journal `2026-W29.md`.
