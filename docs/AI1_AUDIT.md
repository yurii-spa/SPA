...
| **3.1** Rebalance Engine | `agent-ai1-31-rebalance-engine-dry-run` | ✅ `done` | план уже существовал (`allocation_rationale`, shadow); замер вскрыл churn 5.3× капитала в неделю |
...
### Закрыто по мандату владельца (2026-08-29, вечер)

| Что | Итог |
|---|---|
| Оба надзорных агента | **подключены** к дневному циклу (шаги 2b/2c, НЕ-гейт, сторож проводки) |
| Тир `morpho_steakhouse` | **исполнено** решение ADR-070 п.6: класс приведён к канону |
| 12 заявок L4+ | **убраны все**; сторож перевёрнут из храповика в полный запрет |
| Колонка «реальный пилот» | **получила путь в код** — `TriggerParams.for_mode()`, по умолчанию paper |
| Числа модели издержек | **названы** — `docs/cost_model_provenance.md` (что известно и что нет) |

Из пяти вопросов задачи 1.0 закрыты **2** (переключатель режима), **3** (below-median
виден в ежедневном аудите), **4** (документ издержек) и **5** (критерии тиров, 2.3).
Открыт **вопрос 1** — деньги в переставшем наблюдаться протоколе; он единственный
двигает капитал и вынесен владельцу вариантами.

### Что дала 2.2 сверх задания (2026-08-29)

Первая задача дня, где переиспользовать было **нечего**: модуля не существовало.
Правило существовало — **ADR-YL-006** (02.07): APY нельзя «stated, displayed, or
**recorded**» без уровня. Замер: четыре живых артефакта записывают APY и **ни один
не несёт уровня**; правило полтора месяца работало без исполнителя.

Первый прогон: **11 из 33 строк ранжирования — L0**, показывать можно 66.67 %.
И это не хвост: **весь топ-5 по доходности — ненаблюдённые литералы**, включая
самое высокое число 14 % (`pendle_yt_susde`, `fallback`). Литералы оптимистичны
по построению и потому всплывают наверх сортировки по доходности.

29 тестов, 5 мутаций (включая «пробит потолок L2 → L3» — 5 покрасневших тестов).
...
Побочно (**исправлено в тот же день**): я записал, что `ADR-YL-009` не существует — неверно, он лежит в `docs/adr/`. Настоящая находка крупнее: ADR-набор в **двух** каталогах — `docs/decisions/` (121) и `docs/adr/` (**56**), реестр `INDEX.md` не ссылается на второй ни одной строкой. Пропущенный ADR называется «Canonical documentation structure (single source per concept)» — ровно про эту болезнь.
