...
**Дата:** 2026-08-29  
**Статус:** 🟢 IN PROGRESS — месяцы 1–2 доставлены, вопросов к владельцу нет  
**Версия:** 0.3
...
| **3.1** Rebalance Engine | `agent-ai1-31-rebalance-engine-dry-run` | ✅ `done` | замер вскрыл churn 5.3× капитала в неделю → **ограничитель доставлен** (ADR-168) |

**Месяц 1 доставлен, оба стандарта месяца 2 закрыты за один день.** Все семь задач
закрыты; открытых вопросов к владельцу по AI1 **не осталось** — три последних
закрыты вечером 29.08 (ADR-168/169/170).
...
## ✅ Три вопроса к владельцу — закрыты (2026-08-29, вечер)

Владелец ответил на все три разом («согласен на всём»). Доставлено `a93b0c47` и `cb8d82ce`.

| Вопрос | Решение | Где записано |
|---|---|---|
| Ограничивать ли частоту перекладок | **Да**, колонка `paper` ADR-060 | ADR-168 |
| Когда массовая слепота = наша поломка | **Ниже половины** перечисленного (плюс прежний абсолютный минимум 3) | ADR-169 |
| Как надзор доходит до владельца | **Три строки в дневном отчёте**, не новый канал | ADR-170 |

### Что это меняет в цифрах

| | было | станет |
|---|---|---|
| ходов в неделю | 22 | **3** (из них 2 — де-риск, он не задерживается никогда) |
| дискреционных перекладок | 22 | **1** |
| односторонний оборот | $566 579 | **$47 895** (8.5 %) |
| издержки/нед | $1 376 | **$116** — в 12 раз меньше |
| из них дискреционных | ~$1 271 | **~$12** |
| издержки против заработка книги ($87/нед) | в 15 раз больше | **в 1.3 раза больше** |
| защита от массовой слепоты включается | при 16 ослепших из 18 | при 10 из 18 |
| надзорных вердиктов доходит до владельца | 0 из 3 | 3 из 3 |

**Числа переиграны через ДОСТАВЛЕННЫЙ код** на той же неделе, а не взяты из
предварительной прикидки: первая редакция этой таблицы обещала 2 хода и $78 и была
неверна (поправка дописана в ADR-168). Главное, что меняет вывод: издержки падают в
12 раз, но **меньше заработка книги не становятся** — $116 против $87. Причём ~$104
из них это два хода де-риска, которые ограничитель не задерживает и не должен.
Дискреционная часть падает с ~$1 271 до ~$12: механизм делает ровно то, ради чего
создан, а остаток говорит уже о масштабе капитала, а не о частоте перекладок.

Прогноз проверяемый: через неделю сверить факт с ожиданием — **~1 дискреционная
перекладка и ~$12** дискреционных издержек. Ноль дискреционных ходов две недели подряд —
сигнал, что затянуто, и повод ослаблять до мягкой колонки (мин. 6 ч, бюджет 100 %).

### Три урока вечера

1. **Замерь, какая ветка сработала, прежде чем называть тесты помехой.** Семь красных
   тестов цикла выглядели как «тесты написаны до ограничителя». Лог сказал другое:
   бюджет недели душил **ввод** капитала в работу ($74 000 из кэша в пустую книгу).
   Дефект был в модуле; ни один тест не тронут.
2. **Тест, пересчитывающий правило сам, проверяет себя, а не код.** Порог покрытия
   пришлось вынести в функцию — иначе мутация настоящей строки осталась бы незамеченной.
3. **Счётчик не является отчётом.** Строка «нарушений 1» выглядела рабочей, но не
   называла, какое правило и где: ключи читались неверные. Из такой строки владелец не
   может начать работу без уточняющего вопроса — а это ровно то, что стандарт отчёта
   запрещает.
...
| 2026-08-29 | Три вопроса владельца закрыты: ADR-168/169/170 доставлены | Claude | ✅ Доставлено |
| [TBD] | Сверка факта с прогнозом через неделю (перекладки/издержки) | Claude | 📋 Плановая |

---

**Статус:** 🟢 **ДОСТАВЛЕНО** — все семь задач закрыты, три решения владельца записаны ADR-168/169/170  
**Следующий шаг:** через неделю сверить факт с прогнозом — ~1 дискреционная перекладка и ~$12 вместо 22 ходов и ~$1 271
