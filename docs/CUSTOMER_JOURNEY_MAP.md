...
| **J2** | **"Checkup" nav item → `/#analyze`** (a homepage anchor), NOT the full Checkup product. Confusing: the flagship free tool is hidden behind a scroll-anchor. | `SiteHeader.astro` groups[checkup] | 'Checkup' nav is now a dropdown: Check my wallet / See a sample analysis / Analyze on this page. | ✅ **LIVE** (19e9fc24) |
...
| **J5** | **No easy "see an example" path** — a first-timer with no wallet can't preview the analysis without knowing the `/sample-report` URL. | home + checkup nav | 'See a sample analysis →' now sits at the hero entry (+ the existing lower-page link). | ✅ **LIVE** (d83b290b) |
