...
| 5 | L4 иммунитет: freshness всех органов + re-проверка fail-closed контрактов ИЗ артефактов (включая refusal-инвариант мозга) + tamper-check последней proof-строки → `swarm_health.json` OK/WARNING | ✅ построен (`swarm/swarm_health.py`, агент `com.spa.swarm_health`) |
| 5b | **Chaos-drill** иммунитета (provably exercised, философия R-sprint): sandbox-копия живых артефактов + 5 инъекций (missing/stale/contract-break/proof-tamper/bad-regime) → каждая ДОЛЖНА быть поймана, control — OK; live не мутируется → `chaos_drill_status.json` | ✅ построен (`swarm/chaos_drill.py`, еженедельно пн в wrapper'е swarm_health; день 1: **6/6 поймано**) |
...
| **S2** | Копить lead-time evidence: сколько раз shadow-сигнал опередил реальные kill/DL-события и на сколько | ✅ харнесс построен (`swarm/leadtime_evidence.py`, hourly в wrapper'е swarm_health): эпизоды выводятся из живой кривой порогами самой лестницы (DL-01 −2%/день, SOFT −5%, HARD −10%), вердикты LED/COINCIDENT/MISSED + false-alarm леджер (цена сигнала учитывается с тем же весом) → `leadtime_evidence.json`. День 1: 52 трек-дня, 0 эпизодов, **1 false alarm (2026-06-10)** — честный старт леджера |
