...
| 6 | Публичная поверхность: страница **`/aggressive-lab` уже live** (per-book paper-доска, island → `/api/aggressive-lab/*`, fail-closed); осталось guardian-обогащение: бейдж ARMED/DERISKED + guarded-vs-raw DD + `/api/swarm/guardian` + ссылка на proof-chain | ◐ наполовину есть |
