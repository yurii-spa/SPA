...
- **P3 (следующее):** enforced gate backtest→paper→live — стратегия входит в paper только при
  `validated` (real-data + package + OOS-hold); подключить `bee/backtest_live_fit.py` (live-vs-
  backtest divergence → авто-демоушн при деградации). Capacity/TVL-глубина + корреляции пакетов.
