...
## Корреляции пакетов ✅ (диверсификация)

`spa_core/backtesting/tier1/correlation.py` — реально ли стратегии в пакете диверсифицируют.
Строит дневной blended-APY каждой стратегии из реальных рядов, берёт **первые разности**
(со-движение изменений ставок, не уровней), считает попарную Pearson-корреляцию. На пакет:
средняя корреляция + **least-correlated ядро** (жадный отбор: corr < 0.7 ко всем выбранным).
Пишет `data/tier1_correlation.json`. В pipeline после gate.

**Инсайт на реальных данных:** conservative — **51 кандидат, средняя корреляция 0.58**, но
**диверсифицированное ядро = всего 4 стратегии** (есть пары с corr=1.0 — полные дубли). Т.е.
большинство «консервативных» стратегий — одна и та же ставка на ставку. Пакет нужно собирать
из ядра, а не из всех 51 (иначе мнимая диверсификация при концентрированном rate-риске).

## Состояние: Tier-1 P0–P3 + корреляции завершены ✅

P0 статистика (DSR/PSR/minTRL) · P1 реальные данные + режимы · P2 13 протоколов + OOS ·
P3 gate + capacity + divergence · корреляции/диверсификация пакетов. 20 тестов.
Опционально дальше: hard-enforcement gate (сейчас advisory), capacity на target-AUM сценарии,
расширение реальных данных на больше протоколов/цепей.
