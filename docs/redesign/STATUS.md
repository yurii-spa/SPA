...
| U3 | One analytics stream (`checkup:*` events + UTM pass-through) | **PARTIAL** (verified 2026-07-12) — UTM pass-through: DONE outbound (the report deep-CTA carries `utm_source=defi-checkup&utm_medium=dashboard` + per-signal `utm_campaign`). GAP: the checkup has NO analytics helper and fires no `checkup:*` events. To unify the stream needs a beacon → either earn-defi's `/api/analytics` (cross-domain → CORS allow for checkup.earn-defi.com on the Mac apiserver) OR the checkup's own Next API route. Moderate cross-domain feature — not owner-gated, but touches the apiserver CORS; sequenced after Phase-0 UI. Incoming-UTM capture (from earn-defi's "Analyze Wallet" link) also not yet wired |
