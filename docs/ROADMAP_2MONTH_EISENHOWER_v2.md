...
| Q1-8 | ✅ SHIPPED 2026-07-11 (verified: `gap_monitor.py:210-231` archives resolved gaps as `resolved`+`resolved_at`, drops the ACTIVE `recovery`, clears stale cycle_gap alerts; live `gap_monitor.json` clean `status:ok`, no scary artifact) — **Self-clearing gap-recovery state** — clear stale `2026-06-30 cycle_ran_but_gap_persists` record; add auto-clear rule so a resolved gap never leaves a scary artifact on the readiness surface | `spa_core/paper_trading/gap_monitor.py`, `data/gap_monitor.json` | S | W2 | No |
| Q1-9 | ✅ SHIPPED 2026-07-12 (new `spa_core/execution/owner_blockers.py` — deterministic per-gate tracker custody/audit/legal/track_days with status open/in_progress/satisfied; code NEVER fabricates progress: audit/legal owner-asserted via `data/owner_blockers_evidence.json`, track_days auto-derived from golive, owner can't sign past the track gate; `data/owner_blockers.json`; served at `/api/readiness` as `owner_blockers`; per-gate status badges on `/readiness`; regenerated in `golive_freshness_cycle` step 3; +9 tests) — **Owner-only blocker status tracker** (custody/audit/legal/HSM-key) — code surfaces which of the 4 true go-live gates are open so owner runs procurement in parallel with remaining track days | `spa_core/execution/readiness_audit.py`, `data/owner_blockers.json`, `landing/src/pages/readiness.astro` | S | W2 | Partial — code tracks; owner must actually procure |
| Q1-10 | ✅ SHIPPED 2026-07-11 (scout-verified: `agent_health_monitor.py:110-131` reads `data/resilience_status.json`, escalates stale >13h OR degraded posture → WARNING; live `agent_health.json`: `resilience_posture=WARNING`, `resilience_age_h=1.57`) — **Escalate stale/failed `resilience_status` into `agent_health` WARNING** — a rotting DR posture currently only writes JSON nobody is paged on | `resilience_status.py`, `agent_health_monitor.py` | S | W2 | No |
| Q1-11 | ✅ SHIPPED 2026-07-11 (scout-verified: `com.spa.golive_freshness.plist` + `agent_golive_freshness.sh` + `golive_freshness_cycle.py`, 6h StartInterval, gate-validated, `launchctl` LOADED exit 0; writes dated `data/golive_freshness.json`) — **Dedicated agent for `golive_checker` + `pre_cutover_gate` freshness** — readiness verdict + money-path proof always fresh & dated, not a daily-cycle side-effect | new `com.spa.golive_freshness` plist + wrapper, `pre_cutover_gate.py` | S | W2 | No |
...
| Q1-13 | **Capital-efficiency guard (owner-flagged 2026-07-12)** — the desk measures RISK world-class but had ZERO check on capital efficiency: the live book sat ~20% idle at 0% (deployable T1 headroom unused) and nothing caught it. **Full spec: `docs/CAPITAL_EFFICIENCY_GUARD.md`.** 3 layers: (B) read-only GUARD flagging silent under-deployment into agent_health/cycle_health [SAFE, ship independently] · (A) allocator re-fill after ALLOC-002 ≤8-collapse [money-path, **owner chose B-timing: apply AFTER go-live**] · (C) test + surface a public "capital efficiency %". Deterministic, fail-CLOSED, RiskPolicy untouched | `spa_core/allocator/allocator.py`, `cycle_runner._compliant_target`, new capital-efficiency monitor, `readiness.astro` | M | **post-go-live for the money-path fix; guard anytime** | Partial — allocator fix owner-timed |
...
| Q2-6 | ✅ SHIPPED 2026-07-11 (scout-verified: `ReportDashboard.tsx:51,135` renders `data_coverage_pct` as a hero "Coverage X% — positions priced" banner; `analyze.ts` coverageGaps + `policyCheck.ts` minCoveragePct=80) — **Coverage/blind-spot meter (checkup)** — prominent "we checked X% of your value, here's what's dark" banner; turns `coverageGaps` honesty invariant into a trust headline + deeper-scan upsell | `packages/riskdesk/src/report/assemble.ts`, `ReportDashboard.tsx` | M | W4–5 | No |
...
| # | Initiative | File / Subsystem | Effort | Weeks | Owner-gated |
|---|---|---|---|---|---|
| Q3-1 | ✅ SHIPPED 2026-07-11 (verified: `com.spa.weekly_backup` in `RETIRED_LABELS` w/ full Q3-1 rationale — redundant with `daily_backup` DB-snapshot + `dr_offsite_copy`; removed from `install_all_agents.sh`; reversible) — **Reconcile `weekly_backup` WARNING** — retire it (redundant with `daily_backup`) or repair plist so `agent_health` reaches clean all-OK; kills alert-fatigue before go-live | `scripts/daily_backup.py`, `com.spa.weekly_backup.plist` | S | W3 | No |
| Q3-2 | ✅ SHIPPED 2026-07-11 (`1f2b3d06`): `fleet_parity_check` (declared vs plist vs retired vs live launchctl) now surfaces DRIFT into `agent_health` WARNING — mirrors the Q1-10 resilience pattern; classes summarized (retired-still-installed=409/flood hazard / orphan-plist / declared-no-plist / declared-not-running); +`generated_at` stamp for staleness; +4 tests. Monitor-only (touches no agent). — **Fleet-parity self-check** — `tier1_governance`/`tier1_digest`/`checkpoint-7day` loaded-not-installed, resilience reverse; deterministic assert declared-fleet == running-fleet (like doc-drift guard) | `install_all_agents.sh`, new `scripts/fleet_parity_check.py` | M | W3 | No |
...
| Q3-4 | ✅ SHIPPED (verified 2026-07-11: `consecutive_ready` surfaced on `readiness.astro` via `readiness.py` + `golive_checker.py`) — **Consecutive-ready-days stability tracker on readiness page** — `consecutive_ready_days=0`; surface rolling ready-streak (runbook wants 7 sustained) → makes remaining waiting a visible de-risking proof | `readiness.astro`, `golive_checker.py` | S | W3 | No |
| Q3-5 | ✅ SHIPPED (verified 2026-07-11: `data/kill_switch_drill_status.json` records latency_ms=9.7 / limit / last_drill_at / verdict; `readiness.py` serves it; `readiness.astro:59` displays "Emergency-stop drill: {verdict} · {latency}ms / {budget}ms · {date}") — **Kill-switch latency + drill-evidence artifact** — record measured latency + last-drill date → emergency-stop auditable | `scripts/kill_switch_drill.py`, `resilience_cycle.py`, `readiness.astro` | S | W3 | No |
| Q3-6 | ✅ SHIPPED (verified 2026-07-12: `resilience_cycle.py` STEPS already include `kill_switch_drill.py` alongside offsite/restore/fleet drills — the money-path brake is folded into the 6h provably-exercised list; sandboxed/de-risk-only). — **Scheduled `kill_switch_drill` in `resilience_cycle`** — folds the money-path brake (sandboxed, de-risk-only) into the provably-exercised 6h list alongside offsite/restore/fleet | `scripts/resilience_cycle.py` | S | W3 | No |
