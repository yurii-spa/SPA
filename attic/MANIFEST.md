...
## Инструмент опровергнутой гипотезы, снят 2026-09-11 (ADR-342)

| файл | что это было | почему снят |
|---|---|---|
| `attic/scripts/analyze_cycle_kickstart_correlation.py` | коррелятор: объясняет ли `launchctl kickstart` гейта деплоя лишние запуски `com.spa.daily_cycle` | замер 11.09: с момента инструментации (03.09) маркер-лог записал ОДИН kickstart, и тот чужой — гейт не объясняет ни одного из 892 запусков за 81 день |
| `attic/scripts/test_cycle_kickstart_correlation.py` | его 9 тестов | переехали за своим предметом |

**Откуда достать:** `git mv attic/scripts/analyze_cycle_kickstart_correlation.py scripts/`
(тест — в `spa_core/tests/`). Маркер-лог гейта (`check_agent_before_deploy.sh` →
`/tmp/spa_deploy_gate_kickstarts.log`) продолжает писаться: он пригодится, если вопрос вернётся.
Сам вопрос «кто же запускает цикл вне расписания» НЕ закрыт — карточка
`inbox-dnevnoi-tsikl-zapuskayut-892-raza-vmesto`.
