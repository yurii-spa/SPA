...
3. **Remains UNKNOWN:** CF auto-build enabled? (CF dashboard) · cabinet live? · dead deploy scripts? · duplicate test trees? — each with a verify command in the audits.
...
12. **Files created (this audit, LOCAL — not yet pushed):** `AUDIT_00–07.md`, `PROJECT_PROBLEM_MAP.md`, `PROJECT_CONTROL/00–15` (16 files), `FILES_PROPOSED_FOR_DELETION.md`, `FILES_PROPOSED_FOR_CONSOLIDATION.md`, this report.
13. **Files updated:** README + CLAUDE.md get a top pointer to `PROJECT_CONTROL/00_START_HERE.md` (PHASE 10).
14. **Safe fixes made:** freshness-check + deploy-sanity commands (`15_CANONICAL_COMMANDS`); rule consolidation into `01_MASTER_RULES`; agent-separation rules; file-ownership map. No product/deploy/risk behavior changed.
