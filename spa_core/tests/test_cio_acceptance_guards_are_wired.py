...
SUBJECTS = {
    ...
    # ADR-235. Добавлен циклом #494 тем же порядком и по той же причине: канал
    # `governance/evidence_staleness.py` был построен решением владельца
    ...
    # Заказ G35 п. 5 (ADR-411). Вписан ОДНОВРЕМЕННО с проводкой, а не следующим
    # циклом: перепись личности списков (ADR-410) сутки лежала на прод-пути без
    # читателя и без производящего вызова — ровно та тишина, которой класс жив.
    # Население этого списка ведут РУКАМИ, и он сам про себя это говорит: он
    # отказывает тогда, когда кто-то забыл, а забыть и есть тот дефект.
    "list_identity_census": "data/list_identity_census.json",
}
