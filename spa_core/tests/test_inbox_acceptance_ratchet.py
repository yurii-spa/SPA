...
#: Статусы приёма: карточка ещё ничья, критерий обязан появиться при взятии в
#: работу. **Имя ВВОЗИТСЯ у исполнителя, а не повторяется здесь** (заказ G42
#: п. 1, ADR-418): отказывает карточке `owner_queue.queue.set_status`, и пока
