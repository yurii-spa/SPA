...
долго не было своего тест-файла — единственный `test_house_view_gap.py` проверял
ТЕНЕВУЮ реализацию `house_view_gap_c125.py` (столкновение двух реализаций Фазы 3,
06.08). Теневая реализация удалена вместе со своим набором (ADR-070 п.5, 18.08);
живой контур покрыт этим файлом и соседними `test_house_view_gap_*.py`.
