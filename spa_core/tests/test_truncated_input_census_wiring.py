"""Проводка переписи обрезанного входа: у числа есть ВЫЗОВ и ЧИТАТЕЛЬ.

Заказ **G49 п. 2** приказа владельца «Portfolio CIO», решение — ADR-427.

## Почему этот файл существует

Его потребовала БАТАРЕЯ МУТАЦИЙ, а не аккуратность. Мутация «снять ступень из
`findings_bridge.main()`, оставив все три объявления на месте» **выжила**: общий
храповик сверяет `CENSUS_PRODUCT` с `CENSUS_STAGE` и манифестом, но о том, зовётся
ли прибор, не спрашивает вовсе. Запись в реестре исполнением не является — у каждой
соседней переписи по этой же причине есть свой файл проводки, а у этой не было.
...
from spa_core.monitoring import truncated_input_census as tic

REPO = Path(__file__).resolve().parents[2]
OFFICE = REPO / "scripts" / "consume_office_reports.py"
BRIDGE = REPO / "spa_core" / "monitoring" / "findings_bridge.py"
ARTIFACT_REL = "data/truncated_input_census.json"
ARTIFACT_NAME = "truncated_input_census.json"
STAGE_KEY = "truncated_input_census"
RUNNER = "com.spa.decision_loop"


def _office():
    spec = importlib.util.spec_from_file_location("_office_tic_under_test", OFFICE)
...
class ProducingCallExistsAndIsACallNotAName(unittest.TestCase):
    ...
    def test_producer_module_exposes_run_with_root(self):
        self.assertIn("root", inspect.signature(tic.run).parameters)
...
class BridgeRegistriesDeclareTheProduct(unittest.TestCase):
    ...
    def test_declared_module_is_the_one_that_writes_the_declared_artifact(self):
        ...
        self.assertEqual(f"data/{tic.ARTIFACT}", ARTIFACT_REL)
        self.assertEqual(tic.PRODUCER, fb.CENSUS_PRODUCT[STAGE_KEY]["module"])


class OfficeReadsTheNumberNotJustTheFile(unittest.TestCase):
    def test_the_producer_is_named(self):
        self.assertEqual(_office()._PRODUCER[ARTIFACT_NAME], tic.PRODUCER)
    ...
    def test_declared_shape_is_the_shape_the_producer_actually_writes(self):
        ...
        doc = tic.run(REPO, write=False)["doc"]
    ...
    def test_the_named_branch_prints_real_numbers_not_an_empty_read(self):
        ...
        self.assertTrue(any("обрезан ли вход" in line for line in lines), lines)
        self.assertTrue(any("[ПРОВОДКА]" in line for line in lines), lines)
