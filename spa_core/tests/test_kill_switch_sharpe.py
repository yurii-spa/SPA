...
class TestEarlyPeriodGrace(unittest.TestCase):
    ...
    def test_early_period_31days_uses_soft_threshold(self) -> None:
        ...
        _make_analytics(sharpe=-1.5, num_days=31, data_dir=self.data_dir)
    ...
    def test_early_period_59days_uses_soft_threshold(self) -> None:
        ...
        _make_analytics(sharpe=-1.9, num_days=59, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered,
    ...
    def test_normal_period_61days_uses_hard_threshold(self) -> None:
        ...
        _make_analytics(sharpe=-1.5, num_days=61, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
    ...
    def test_early_period_still_fires_below_soft_threshold(self) -> None:
        ...
        _make_analytics(sharpe=-3.0, num_days=31, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertTrue(triggered,
    ...
    def test_normal_period_no_fire_above_threshold(self) -> None:
        ...
        _make_analytics(sharpe=0.5, num_days=61, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
    ...
    def test_normal_threshold_exact_value(self) -> None:
        ...
        _make_analytics(sharpe=-1.0, num_days=61, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered,
...
class TestRealDataScenario(unittest.TestCase):
    ...
    def test_real_data_rf0_sharpe_positive(self) -> None:
        ...
        _make_analytics(sharpe=real_sharpe_rf0, num_days=31, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered,
    ...
    def test_real_data_rf5_would_have_triggered(self) -> None:
        ...
        _make_analytics(sharpe=wrong_sharpe_rf5, num_days=31, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertTrue(triggered,
    ...
    def test_combo_fix_resolves_p0_blocker(self) -> None:
        ...
        _make_analytics(sharpe=corrected_sharpe, num_days=31, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered,
    ...
    def test_full_integration_no_kill_switch(self) -> None:
        ...
        _make_analytics(sharpe=1.61, num_days=31, data_dir=self.data_dir)
...
class TestSharpePolicy(unittest.TestCase):
    ...
    def test_checker_uses_custom_policy(self) -> None:
        ...
        _make_analytics(sharpe=-1.5, num_days=70, data_dir=self.data_dir)
...
class TestSharpeEdgeCases(unittest.TestCase):
    ...
    def test_insufficient_data_29days_no_trigger(self) -> None:
        """29 дней < MIN_DAYS_FOR_SHARPE (30) → не triggered вне зависимости от Sharpe."""
        _make_analytics(sharpe=-99.0, num_days=29, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
    ...
    def test_exactly_30days_activates_sharpe_check(self) -> None:
        """30 дней = MIN_DAYS_FOR_SHARPE → Sharpe check включается."""
        _make_analytics(sharpe=-5.0, num_days=30, data_dir=self.data_dir)
        triggered, reason = self.checker.check_sharpe_trigger()
    ...
    # F-3
    def test_no_analytics_file_no_trigger(self) -> None:
        """Нет analytics_summary.json → не triggered."""
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered, f"Нет файла: no trigger, reason: {reason}")

    # F-4
    def test_num_days_from_metrics_fallback(self) -> None:
        """num_days берётся из metrics.num_days если нет в top-level doc."""
        doc = {
            "generated_at": "2026-06-20T00:00:00+00:00",
            "source": "test",
            # Нет top-level num_days!
            "metrics": {"sharpe": 1.5, "num_days": 35},
            "errors": [],
        }
        _write_json(self.data_dir / "analytics_summary.json", doc)
        triggered, reason = self.checker.check_sharpe_trigger()
        self.assertFalse(triggered, f"num_days=35 (из metrics), sharpe=1.5: ok, reason: {reason}")
    ...
    def test_sharpe_positive_always_ok(self) -> None:
        ...
        for sharpe_val in [0.01, 0.5, 1.0, 1.61, 3.0, 10.0]:
            _make_analytics(sharpe=sharpe_val, num_days=90, data_dir=self.data_dir)
            triggered, reason = self.checker.check_sharpe_trigger()
            self.assertFalse(triggered,
