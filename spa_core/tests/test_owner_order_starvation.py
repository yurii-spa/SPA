...
class TestMainExitCode:
    ...
    def test_exit_3_when_starving_card_on_disk(self, guard, tmp_path):
        """Находка — код 3. НАМЕРЕННОЕ изменение теста (инв. #16), обоснование — здесь,
        в ADR-347 и в `docs/journal/2026-W37.md`.

        Раньше здесь стояла единица, и это была не описка, а неразличимость по построению:
        **единицу выдаёт сам CPython** при любом необработанном исключении. 12.09 сторож
        умер на собственном импорте `spa_core`, вышел с кодом 1 — и обёртка, по контракту,
        вклеила его трассировку в промпт цикла словами «ШАГ 0a-ГОЛОД — НАХОДКА, возьми ЭТУ
        карточку первой». Крах сторожа был предъявлен сессии как вердикт сторожа.

        Проверка не ослаблена: она спрашивает ровно то же («голодающая карточка на диске ⇒
        находка»), но на коде, который интерпретатор подделать не может. Единица теперь
        занята ТОЛЬКО исходом «сторож не дошёл до вердикта».
        """
        ...
        assert guard.main(["--tracker-dir", str(tmp_path)]) == 3
