# FROZEN-DATE-OK: injected-clock — часы здесь ВХОД (24 вызова с now=), а отметки выведены
# от того же якоря: пара «last_updated 2026-06-21 против now 2026-08-17» ВОСПРОИЗВОДИТ замеренный
# 17.08 инцидент — литералы 57-суточной давности шли в турнир как живой APY. Обе стороны
# закреплены, поэтому сдвиг календаря на тест не влияет. Сделать отметки относительными нельзя:
# пропадёт сам предмет проверки — КОНКРЕТНЫЙ разрыв, на котором дефект и был пойман.
