...
def _dead_pid() -> int:
    """Свободный номер процесса — **ИЗМЕРЕННЫЙ**, а не литеральный (#453).

    Здесь стоял литерал `999_999`, и он был безопасен ровно по одной причине, которую
    никто не назвал: на macOS `PID_MAX` = 99999, поэтому такого номера не бывает. На Linux
    (а CI — `ubuntu-latest`) `/proc/sys/kernel/pid_max` по умолчанию 4 194 304, и номер
    вполне достижим. То есть проверка держалась на свойстве ХОСТА, которого не проверяла, —
    ровно класс `.claude/rules/deployment.md` «ЛИЧНОСТЬ ПРОЦЕССА в тестах»: литеральный pid
    означает «мёртвый» лишь пока ОС не переиспользует номер (авария 22.08 с `98535`,
    рецидив 28.08 с `999`).

    Инъектировать здесь нечего ПО ЗАМЫСЛУ: этот файл держит РЕШАЮЩИЙ контроль, который
    обязан идти через настоящий `_acquire_cycle_lock` без единого monkeypatch. Поэтому
    второй путь правила — «взять заведомо свободный номер и НАЗВАТЬ, почему он свободен».
    Свободу здесь не предполагают, а спрашивают у той же двери (`ps -p`), которой потом
    ответит проверяемый код.

    Направление отказа: не нашли свободного за весь диапазон ⇒ падаем ГРОМКО. Тест,
    который не смог обеспечить свою предпосылку, обязан сказать это вслух, а не судить
    о живом номере как о мёртвом.
    """
    import subprocess
    ...
    raise AssertionError(
        "не нашёл свободного номера процесса в диапазоне 90000..99000 — предпосылка теста "
        "не обеспечена; судить о мёртвом держателе нечем")
