"""
Tests for MP-1260:
DeFiProtocolVaultPerformanceFeeGrossOfRebalancingTransactionCostAnalyzer
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_rebalancing_transaction_cost_analyzer -v
"""
...
from spa_core.analytics.gross_of.defi_protocol_vault_performance_fee_gross_of_rebalancing_transaction_cost_analyzer import (  # noqa: E501
    GrossOfRebalancingTransactionCostAnalyzer,
    _f,
    _clamp,
    _mean,
    _safe_div,
...
def make_pos(
    ...
    if net_of_rebalancing_yield_pct is not None:
        pos["net_of_rebalancing_yield_pct"] = net_of_rebalancing_yield_pct
    if performance_fee_pct is not None:
        pos["performance_fee_pct"] = performance_fee_pct
    if rebalancing_transaction_cost_rate_pct is not None:
        pos["rebalancing_transaction_cost_rate_pct"] = rebalancing_transaction_cost_rate_pct
    if rebalancing_transaction_cost_gap_pct is not None:
        pos["rebalancing_transaction_cost_gap_pct"] = rebalancing_transaction_cost_gap_pct
...
class TestMainPathClassification(unittest.TestCase):
    def setUp(self):
        self.an = GrossOfRebalancingTransactionCostAnalyzer()
    ...
    def test_clean_tiny_gap(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_rebalancing_yield_pct=9.95,
            performance_fee_pct=10.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_REBALANCING_BASE")
        self.assertLessEqual(
            r["fee_on_rebalancing_fraction"], CLEAN_FRACTION)

    # 16
    def test_mild_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=9.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"],
                         "MILD_FEE_ON_REBALANCING_GAP")
        self.assertGreater(
            r["fee_on_rebalancing_fraction"], CLEAN_FRACTION)
        self.assertLessEqual(
            r["fee_on_rebalancing_fraction"], MILD_FRACTION)
    ...
    def test_moderate_gap(self):
        r = self.an.analyze(make_pos(
            ...
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_REBALANCING_GAP")
        self.assertAlmostEqual(
            r["fee_on_rebalancing_fraction"], 0.5, places=4)

    # 18
    def test_severe_gap_high_fraction(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=1.0,
            performance_fee_pct=20.0))
        self.assertGreater(
            r["fee_on_rebalancing_fraction"], MODERATE_FRACTION)
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_REBALANCING_GAP")
    ...
    def test_zero_net_yield(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_rebalancing_fraction"], 1.0, places=4)

    # 21
    def test_full_fee_on_rebalancing_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_REBALANCING", r["flags"])

    # 22
    def test_high_rebalancing_rate_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_rebalancing_yield_pct=14.5,
            performance_fee_pct=20.0,
            rebalancing_transaction_cost_rate_pct=0.30))
        self.assertIn("HIGH_REBALANCING_COST", r["flags"])

    # 23
    def test_low_rebalancing_rate_no_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_rebalancing_yield_pct=14.5,
            performance_fee_pct=20.0,
            rebalancing_transaction_cost_rate_pct=0.10))
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_basic(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_rebalancing_fraction"], 0.4, places=4)

    # 25
    def test_override_gap_clamped_to_fee(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            rebalancing_transaction_cost_gap_pct=15.0,
            fee_charged_pct=10.0))
        self.assertEqual(r["rebalancing_transaction_cost_gap_pct"], 10.0)
        self.assertEqual(r["fee_on_rebalancing_fraction"], 1.0)
    ...
    def test_override_negative_gap_abs(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            rebalancing_transaction_cost_gap_pct=-3.0,
            fee_charged_pct=10.0))
        self.assertAlmostEqual(
            r["rebalancing_transaction_cost_gap_pct"], 3.0, places=4)

    # 27
    def test_override_zero_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            rebalancing_transaction_cost_gap_pct=0.0,
            fee_charged_pct=10.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_REBALANCING_BASE")
    ...
    def test_override_with_rate(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            rebalancing_transaction_cost_gap_pct=2.0,
            fee_charged_pct=10.0,
            rebalancing_transaction_cost_rate_pct=0.30))
        self.assertIn("HIGH_REBALANCING_COST", r["flags"])
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_no_fee_no_override(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=8.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")
...
class TestScoring(unittest.TestCase):
    ...
    def test_score_moderate(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_rebalancing_yield_pct=10.0,
            performance_fee_pct=20.0))
    ...
    def test_score_severe_negative(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_rebalancing_yield_pct=-3.0,
            performance_fee_pct=50.0))
            ...
            for net in (-5.0, 0.0, gross * 0.5, gross):
                for fee in (0.0, 10.0, 50.0, 100.0):
                    r = self.an.analyze(make_pos(
                        ...
                        net_of_rebalancing_yield_pct=net,
    ...
    def test_score_formula_components(self):
        ...
        frac = r["fee_on_rebalancing_fraction"]
    ...
    def test_grade_B_threshold(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_rebalancing_yield_pct=17.0,
            performance_fee_pct=20.0))
...
class TestRecommendation(unittest.TestCase):
    ...
    def test_recommend_demand(self):
        r = self.an.analyze(make_pos(
            ...
            performance_fee_pct=20.0))
        self.assertEqual(r["recommendation"],
                         "DEMAND_NET_OF_REBALANCING_BASE")
...
class TestFlags(unittest.TestCase):
    ...
    def test_high_rate_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_rebalancing_yield_pct=13.0,
            performance_fee_pct=20.0,
            rebalancing_transaction_cost_rate_pct=HIGH_REBALANCING_COST_PCT))
        self.assertIn("HIGH_REBALANCING_COST", r["flags"])

    # 59
    def test_fee_on_rebalancing_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_rebalancing_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_REBALANCING", r["flags"])
...
class TestPortfolio(unittest.TestCase):
    ...
    def test_portfolio_cleanest_worst(self):
        ...
        self.assertEqual(
            agg["worst_rebalancing_gap_vault"], "Bad")
    ...
    def test_portfolio_all_insufficient(self):
        ...
        self.assertIsNone(agg["cleanest_vault"])
        self.assertIsNone(
            agg["worst_rebalancing_gap_vault"])
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_very_large_yield(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_rebalancing_yield_pct=998.0,
            performance_fee_pct=20.0))
    ...
    def test_very_small_yield(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_rebalancing_yield_pct=0.001,
            performance_fee_pct=20.0))
        self.assertTrue(_all_floats_finite(r))
    ...
    def test_fee_pct_0(self):
        ...
        self.assertAlmostEqual(
            r["rebalancing_transaction_cost_gap_pct"], 0.0, places=4)
    ...
    def test_cli_exit_zero(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m",
             ...
             "rebalancing_transaction_cost_analyzer",
             "--check"],
