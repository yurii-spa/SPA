...
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_early_withdrawal_penalty_base_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_gross_of_early_withdrawal_penalty_base_gap_analyzer import (  # noqa: E501
    DeFiProtocolVaultPerformanceFeeGrossOfEarlyWithdrawalPenaltyBaseGapAnalyzer,
    _f,
    _clamp,
    _mean,
    ...
    MILD_FRACTION,
    MODERATE_FRACTION,
    HIGH_EARLY_WITHDRAWAL_PENALTY_PCT,
    EPS,
    LOG_PATH,
    LOG_CAP,
)
...
def make_pos(
    ...
    if net_of_early_withdrawal_penalty_yield_pct is not None:
        pos["net_of_early_withdrawal_penalty_yield_pct"] = net_of_early_withdrawal_penalty_yield_pct
    ...
    if early_withdrawal_penalty_rate_pct is not None:
        pos["early_withdrawal_penalty_rate_pct"] = early_withdrawal_penalty_rate_pct
    if fee_on_early_withdrawal_penalty_gap_pct is not None:
        pos["fee_on_early_withdrawal_penalty_gap_pct"] = fee_on_early_withdrawal_penalty_gap_pct
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_equal_net_gross(self):
        ...
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
        self.assertIn("CLEAN_NET_BASE", r["flags"])

    def test_clean_net_slightly_below_gross(self):
        # net = 14.8, gross = 15 → consumed = 0.2 → fee_gap = 0.2*0.2 = 0.04
        # fee_charged = 15*0.2 = 3 → fraction = 0.04/3 ≈ 0.013 ≤ 0.05 → CLEAN
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_early_withdrawal_penalty_yield_pct=14.8,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")

    def test_mild_gap(self):
        # gross=20, net=16, fee=20% → charged=4, fair=3.2, gap=0.8
        # fraction = 0.8/4 = 0.2 ≤ 0.20 → MILD
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 0.2, places=4)
        self.assertEqual(r["classification"],
                         "MILD_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")

    def test_moderate_gap(self):
        # gross=20, net=10, fee=20% → charged=4, fair=2, gap=2
        # fraction = 2/4 = 0.5 ≤ 0.50 → MODERATE
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=10.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 0.5, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")

    def test_severe_gap_high_fraction(self):
        # gross=20, net=4, fee=20% → charged=4, fair=0.8, gap=3.2
        # fraction = 3.2/4 = 0.8 > 0.50 → SEVERE
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=4.0,
            performance_fee_pct=20.0))
        self.assertGreater(r["fee_on_early_withdrawal_penalty_fraction"], 0.50)
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")

    def test_severe_gap_net_negative(self):
        # net < 0 → always SEVERE regardless of fraction
        r = self.an.analyze(make_pos(
    ...
    def test_net_zero_yields_full_gap(self):
        # net = 0 → fair_fee = 0 → gap = fee_charged → fraction = 1.0
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 1.0, places=4)
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
    ...
    def test_recommendation_mild(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_early_withdrawal_penalty_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["recommendation"], "MINOR_FEE_ON_EARLY_WITHDRAWAL_PENALTY")
    ...
    def test_recommendation_moderate(self):
        ...
        self.assertEqual(r["recommendation"],
                         "DEMAND_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
    ...
    def test_recommendation_severe(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_early_withdrawal_penalty_yield_pct=-2.0,
            performance_fee_pct=50.0))
        self.assertEqual(r["recommendation"], "AVOID_FEE_ON_EARLY_WITHDRAWAL_PENALTY")
...
class TestMath(unittest.TestCase):
    ...
    def test_fee_charged_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=14.0,
            performance_fee_pct=20.0))
    ...
    def test_gap_formula(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_gap_pct"], expected_gap, places=4)
    ...
    def test_fraction_formula(self):
        ...
        gap = (20.0 - 14.0) * 0.2
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], gap / charged, places=4)

    def test_early_withdrawal_penalty_consumed_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["early_withdrawal_penalty_consumed_yield_pct"], 6.0, places=4)
    ...
    def test_overstatement_equals_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["overstatement_pct"],
                               r["fee_on_early_withdrawal_penalty_gap_pct"], places=4)

    def test_realization_ratio_clean(self):
        # net == gross → fair == charged → realization = 1
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_early_withdrawal_penalty_yield_pct=15.0,
            performance_fee_pct=20.0))
    ...
    def test_realization_ratio_bounds(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=-3.0,
            performance_fee_pct=50.0))
    ...
    def test_performance_fee_pct_clamped_high(self):
        # fee = 200% → clamped to 100% → fee_charged = gross * 1.0
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=8.0,
    ...
    def test_performance_fee_pct_clamped_low(self):
        # fee = -50% → clamped to 0% → fee_charged = 0
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=8.0,
            performance_fee_pct=-50.0))
        ...
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")

    def test_net_above_gross_clamps_consumed_zero(self):
        # net > gross → consumed = 0 → gap = 0 → CLEAN
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["early_withdrawal_penalty_consumed_yield_pct"], 0.0, places=4)
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
    ...
    def test_zero_fee_rate_always_clean(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_gap_pct"], 0.0, places=4)
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_basic(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 0.4, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_early_withdrawal_penalty_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertIsNone(r["net_of_early_withdrawal_penalty_yield_pct"])
        self.assertIsNone(r["early_withdrawal_penalty_consumed_yield_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_early_withdrawal_penalty_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_EARLY_WITHDRAWAL_PENALTY", r["flags"])
        self.assertNotIn("FULL_FEE_ON_EARLY_WITHDRAWAL_PENALTY", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=12.0))
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_gap_pct"], 4.8, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_early_withdrawal_penalty_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["fee_on_early_withdrawal_penalty_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_early_withdrawal_penalty_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_early_withdrawal_penalty_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_early_withdrawal_penalty_fraction"]
    ...
    def test_override_denominator_is_fee_charged(self):
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=12.0))
        self.assertAlmostEqual(
            r["fee_on_early_withdrawal_penalty_fraction"], 3.0 / 12.0, places=4)

    def test_override_high_early_withdrawal_penalty_still_flagged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_early_withdrawal_penalty_gap_pct=3.0,
            fee_charged_pct=12.0,
            early_withdrawal_penalty_rate_pct=0.6))
        self.assertIn("HIGH_EARLY_WITHDRAWAL_PENALTY", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
    ...
    def test_override_requires_positive_fee_charged(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=0.0,
            performance_fee_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=20.0))
        self.assertFalse(r["used_override"])
        self.assertTrue(r["used_main"])
    ...
    def test_override_clean_when_small_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_early_withdrawal_penalty_gap_pct=0.3,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
        self.assertIn("CLEAN_NET_BASE", r["flags"])
    ...
    def test_override_severe_when_large_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_early_withdrawal_penalty_gap_pct=10.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "fee_on_early_withdrawal_penalty_gap_pct", "realization_ratio",
...
class TestFlags(unittest.TestCase):
    ...
    def test_classification_always_in_flags(self):
        for net in (15.0, 12.0, 8.0, 4.0):
            r = self.an.analyze(make_pos(
                gross_yield_pct=15.0,
                net_of_early_withdrawal_penalty_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertIn(r["classification"], r["flags"])

    def test_high_early_withdrawal_penalty_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_early_withdrawal_penalty_yield_pct=13.0,
            performance_fee_pct=20.0,
            early_withdrawal_penalty_rate_pct=HIGH_EARLY_WITHDRAWAL_PENALTY_PCT))
        self.assertIn("HIGH_EARLY_WITHDRAWAL_PENALTY", r["flags"])

    def test_no_high_early_withdrawal_penalty_flag_below_threshold(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_early_withdrawal_penalty_yield_pct=13.0,
            performance_fee_pct=20.0,
            early_withdrawal_penalty_rate_pct=0.2))
        self.assertNotIn("HIGH_EARLY_WITHDRAWAL_PENALTY", r["flags"])

    def test_full_fee_on_early_withdrawal_penalty_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_early_withdrawal_penalty_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_EARLY_WITHDRAWAL_PENALTY", r["flags"])

    def test_fee_on_early_withdrawal_penalty_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_early_withdrawal_penalty_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_EARLY_WITHDRAWAL_PENALTY", r["flags"])
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_early_withdrawal_penalty_gap_vault", agg)
    ...
    def test_aggregate_cleanest_and_worst(self):
        ...
        self.assertEqual(agg["worst_early_withdrawal_penalty_gap_vault"], "Bad")

    def test_aggregate_all_insufficient(self):
        positions = [make_pos(vault="X"), make_pos(vault="Y")]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertIsNone(agg["cleanest_vault"])
        self.assertIsNone(agg["worst_early_withdrawal_penalty_gap_vault"])
    ...
    def test_aggregate_single_position(self):
        ...
        self.assertEqual(agg["worst_early_withdrawal_penalty_gap_vault"], "Solo")
            ...
            for net in (-5.0, 0.0, gross * 0.5, gross, gross + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    ...
                    self.assertGreaterEqual(
                        r["fee_on_early_withdrawal_penalty_fraction"], 0.0)
                    self.assertLessEqual(
                        r["fee_on_early_withdrawal_penalty_fraction"], 1.0)
                    self.assertGreaterEqual(r["realization_ratio"], 0.0)
                    self.assertLessEqual(r["realization_ratio"], 1.0)
                    self.assertTrue(_all_floats_finite(r))

    def test_fraction_monotone_with_early_withdrawal_penalty(self):
        ...
        for net in (20.0, 16.0, 12.0, 8.0, 4.0, 0.0):
            r = self.an.analyze(make_pos(
                ...
                performance_fee_pct=20.0))
            self.assertGreaterEqual(
                r["fee_on_early_withdrawal_penalty_fraction"], prev)
            prev = r["fee_on_early_withdrawal_penalty_fraction"]
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-EWP-Vault-CleanEarlyWithdrawalPenalty"],
            "CLEAN_NET_OF_EARLY_WITHDRAWAL_PENALTY_BASE")
        self.assertEqual(
            classes["CRV-EWP-Vault-ModerateEarlyWithdrawalPenalty"],
            "MODERATE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
        self.assertEqual(
            classes["PENDLE-EWP-Vault-SevereEarlyWithdrawalPenalty"],
            "SEVERE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
        self.assertEqual(
            classes["UNI-EWP-Vault-OverrideEarlyWithdrawalPenaltyGap"],
            "MODERATE_FEE_ON_EARLY_WITHDRAWAL_PENALTY_GAP")
        self.assertEqual(
