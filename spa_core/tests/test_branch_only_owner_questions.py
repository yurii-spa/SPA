...
def test_office_step_reads_an_old_report_as_unmeasured(repo, tmp_path):
    ...
    # ЦИКЛ #528, изменение НАМЕРЕННОЕ и объяснённое (инв. #16). Здесь стояла
    # проза «отчёт старого образца» — ПРИЧИНА, которую никто не измерял: её
    # печатали при ЛЮБОМ отсутствии блока, в том числе когда производитель
    # ключ не пишет вовсе или уже отработал с этим кодом и блока не написал.
    # Утверждение теста («старый отчёт не имеет права выглядеть измеренным»)
    # НЕ ослаблено — строка выше проверяет ровно его. Проверка усилена:
    # причина обязана быть НАЗВАНА и относиться к тому блоку, которого нет.
    assert "branch_queue" in text, text
    assert "производитель" in text, text
