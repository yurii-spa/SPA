...
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_crosschain_message_fee_base_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_gross_of_crosschain_message_fee_base_gap_analyzer import (  # noqa: E501
    DeFiProtocolVaultPerformanceFeeGrossOfCrosschainMessageFeeBaseGapAnalyzer,
    _f,
    _clamp,
    _mean,
...
def make_pos(
    ...
    if net_of_crosschain_message_fee_yield_pct is not None:
        pos["net_of_crosschain_message_fee_yield_pct"] = net_of_crosschain_message_fee_yield_pct
    ...
    if crosschain_message_fee_rate_pct is not None:
        pos["crosschain_message_fee_rate_pct"] = crosschain_message_fee_rate_pct
    if fee_on_crosschain_message_fee_gap_pct is not None:
        pos["fee_on_crosschain_message_fee_gap_pct"] = fee_on_crosschain_message_fee_gap_pct
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_equal_net_gross(self):
        ...
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
    ...
    def test_clean_net_slightly_below_gross(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_crosschain_message_fee_yield_pct=14.8,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
    ...
    def test_mild_gap(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            net_of_crosschain_message_fee_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 0.2, places=4)
        self.assertEqual(r["classification"],
                         "MILD_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
    ...
    def test_moderate_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=10.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 0.5, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
    ...
    def test_severe_gap_high_fraction(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=4.0,
            performance_fee_pct=20.0))
        self.assertGreater(r["fee_on_crosschain_message_fee_fraction"], 0.50)
        self.assertEqual(r["classification"],
    ...
    def test_net_zero_yields_full_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_crosschain_message_fee_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 1.0, places=4)
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
    ...
    def test_recommendation_mild(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_crosschain_message_fee_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["recommendation"], "MINOR_FEE_ON_CROSSCHAIN_MESSAGE_FEE")
    ...
    def test_recommendation_moderate(self):
        ...
        self.assertEqual(r["recommendation"],
                         "DEMAND_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
    ...
    def test_recommendation_severe(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_crosschain_message_fee_yield_pct=-2.0,
            performance_fee_pct=50.0))
        self.assertEqual(r["recommendation"], "AVOID_FEE_ON_CROSSCHAIN_MESSAGE_FEE")
...
class TestMath(unittest.TestCase):
    ...
    def test_fee_charged_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=14.0,
            performance_fee_pct=20.0))
    ...
    def test_gap_formula(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_gap_pct"], expected_gap, places=4)
    ...
    def test_fraction_formula(self):
        ...
        gap = (20.0 - 14.0) * 0.2
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], gap / charged, places=4)

    def test_crosschain_message_fee_consumed_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["crosschain_message_fee_consumed_yield_pct"], 6.0, places=4)
    ...
    def test_overstatement_equals_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["overstatement_pct"],
                               r["fee_on_crosschain_message_fee_gap_pct"], places=4)
    ...
    def test_realization_ratio_bounds(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_crosschain_message_fee_yield_pct=-3.0,
            performance_fee_pct=50.0))
    ...
    def test_performance_fee_pct_clamped_high(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_crosschain_message_fee_yield_pct=8.0,
    ...
    def test_performance_fee_pct_clamped_low(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
    ...
    def test_net_above_gross_clamps_consumed_zero(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            performance_fee_pct=20.0))
        self.assertAlmostEqual(
            r["crosschain_message_fee_consumed_yield_pct"], 0.0, places=4)
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
    ...
    def test_zero_fee_rate_always_clean(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_gap_pct"], 0.0, places=4)
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 0.0, places=4)
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_basic(self):
        ...
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 0.4, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_crosschain_message_fee_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertIsNone(r["net_of_crosschain_message_fee_yield_pct"])
        self.assertIsNone(r["crosschain_message_fee_consumed_yield_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_crosschain_message_fee_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_CROSSCHAIN_MESSAGE_FEE", r["flags"])
        self.assertNotIn("FULL_FEE_ON_CROSSCHAIN_MESSAGE_FEE", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=12.0))
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_gap_pct"], 4.8, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_crosschain_message_fee_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["fee_on_crosschain_message_fee_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_crosschain_message_fee_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_crosschain_message_fee_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_crosschain_message_fee_fraction"]
    ...
    def test_override_denominator_is_fee_charged(self):
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=12.0))
        self.assertAlmostEqual(
            r["fee_on_crosschain_message_fee_fraction"], 3.0 / 12.0, places=4)

    def test_override_high_crosschain_message_fee_still_flagged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_crosschain_message_fee_gap_pct=3.0,
            fee_charged_pct=12.0,
            crosschain_message_fee_rate_pct=0.6))
        self.assertIn("HIGH_CROSSCHAIN_MESSAGE_FEE", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
    ...
    def test_override_requires_positive_fee_charged(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=0.0,
            performance_fee_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=20.0))
        self.assertFalse(r["used_override"])
        self.assertTrue(r["used_main"])
    ...
    def test_override_clean_when_small_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_crosschain_message_fee_gap_pct=0.3,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"],
                         "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
        self.assertIn("CLEAN_NET_BASE", r["flags"])
    ...
    def test_override_severe_when_large_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_crosschain_message_fee_gap_pct=10.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "fee_on_crosschain_message_fee_gap_pct", "realization_ratio",
...
class TestFlags(unittest.TestCase):
    ...
    def test_classification_always_in_flags(self):
        for net in (15.0, 12.0, 8.0, 4.0):
            r = self.an.analyze(make_pos(
                gross_yield_pct=15.0,
                net_of_crosschain_message_fee_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertIn(r["classification"], r["flags"])

    def test_high_crosschain_message_fee_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_crosschain_message_fee_yield_pct=13.0,
            performance_fee_pct=20.0,
            crosschain_message_fee_rate_pct=HIGH_CROSSCHAIN_MESSAGE_FEE_PCT))
        self.assertIn("HIGH_CROSSCHAIN_MESSAGE_FEE", r["flags"])

    def test_no_high_crosschain_message_fee_flag_below_threshold(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_crosschain_message_fee_yield_pct=13.0,
            performance_fee_pct=20.0,
            crosschain_message_fee_rate_pct=0.2))
        self.assertNotIn("HIGH_CROSSCHAIN_MESSAGE_FEE", r["flags"])

    def test_full_fee_on_crosschain_message_fee_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_crosschain_message_fee_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_CROSSCHAIN_MESSAGE_FEE", r["flags"])

    def test_fee_on_crosschain_message_fee_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_crosschain_message_fee_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_CROSSCHAIN_MESSAGE_FEE", r["flags"])
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_crosschain_message_fee_gap_vault", agg)
    ...
    def test_aggregate_cleanest_and_worst(self):
        ...
        self.assertEqual(agg["worst_crosschain_message_fee_gap_vault"], "Bad")

    def test_aggregate_all_insufficient(self):
        positions = [make_pos(vault="X"), make_pos(vault="Y")]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertIsNone(agg["cleanest_vault"])
        self.assertIsNone(agg["worst_crosschain_message_fee_gap_vault"])
    ...
    def test_aggregate_single_position(self):
        ...
        self.assertEqual(agg["worst_crosschain_message_fee_gap_vault"], "Solo")
            ...
            for net in (-5.0, 0.0, gross * 0.5, gross, gross + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    ...
                    self.assertGreaterEqual(
                        r["fee_on_crosschain_message_fee_fraction"], 0.0)
                    self.assertLessEqual(
                        r["fee_on_crosschain_message_fee_fraction"], 1.0)
                    self.assertGreaterEqual(r["realization_ratio"], 0.0)
                    self.assertLessEqual(r["realization_ratio"], 1.0)
                    self.assertTrue(_all_floats_finite(r))

    def test_fraction_monotone_with_crosschain_message_fee(self):
        ...
        for net in (20.0, 16.0, 12.0, 8.0, 4.0, 0.0):
            r = self.an.analyze(make_pos(
                ...
                performance_fee_pct=20.0))
            self.assertGreaterEqual(
                r["fee_on_crosschain_message_fee_fraction"], prev)
            prev = r["fee_on_crosschain_message_fee_fraction"]
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-XC-Vault-CleanCrosschainMessageFee"],
            "CLEAN_NET_OF_CROSSCHAIN_MESSAGE_FEE_BASE")
        self.assertEqual(
            classes["CRV-XC-Vault-ModerateCrosschainMessageFee"],
            "MODERATE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
        self.assertEqual(
            classes["BAL-XC-Vault-SevereCrosschainMessageFee"],
            "SEVERE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
        self.assertEqual(
            classes["UNI-XC-Vault-OverrideCrosschainMessageFeeGap"],
            "MODERATE_FEE_ON_CROSSCHAIN_MESSAGE_FEE_GAP")
        self.assertEqual(
