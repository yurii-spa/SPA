#!/usr/bin/env python3
"""Тесты StusdAdapter — MP-561.

Покрываем:
  TestStusdInit        (12) — константы: protocol, tier=T2, chain_id, vault, risk_score, caps, apy, tvl
  TestStusdAPY         (12) — get_apy, fallback, json override, get_apy_pct, health_check
  TestStusdPeg         (13) — is_peg_healthy: missing→True, 1.0→True, в пределах, граница, депег, нечисловое
  TestStusdEligibility (11) — eligible = peg OK + APY OK; все комбинации
  TestStusdYieldInfo   ( 8) — get_yield_info: apy десятичной дробью, tier, risk_score, exit_latency
  TestStusdVsMorpho    (10) — vs_morpho_gap: положительный/отрицательный/кастомный
  TestStusdAllocate    (10) — нулевой, negative → ValueError, нормальный, структура ответа
  TestStusdWithdraw    (10) — нулевой, negative → ValueError, normal, insufficient
  TestStusdToDict      ( 9) — все ключи: peg_healthy, eligible, protocol, tier=T2
  TestStusdRegistry    ( 5) — импорт, ADAPTER_REGISTRY, __all__

Итого: 100 тестов.

Запуск:
    python3 -m pytest spa_core/tests/test_stusd_adapter.py -q
    python3 -m unittest spa_core.tests.test_stusd_adapter -v
    python3 spa_core/tests/test_stusd_adapter.py
"""
...
from spa_core.adapters.stusd_adapter import StusdAdapter

_VAULT = "0x0022228a2cc5E7eF0274A2C3D1E5e306d3eF0a17"
...
def _make_adapter(
    apy: float | None = 6.0,
    usda_price: float | None = None,
    missing_section: bool = False,
    missing_file: bool = False,
) -> StusdAdapter:
    """Создаёт адаптер с временным data_dir.

    apy=None          → поле stusd.apy отсутствует (тест fallback)
    usda_price=None   → поле usda_price отсутствует (peg считается healthy)
    missing_section   → секция stusd полностью отсутствует
    missing_file      → adapter_status.json не существует
    """
    if missing_file:
        return StusdAdapter(data_dir="/nonexistent_spa_test_stusd_xyz")
    ...
    if missing_section:
        ...
        if usda_price is not None:
            section["usda_price"] = usda_price
        content = {"stusd": section}

    (Path(tmp) / "adapter_status.json").write_text(
        json.dumps(content), encoding="utf-8"
    )
    return StusdAdapter(data_dir=tmp)


def _default() -> StusdAdapter:
    """Адаптер с apy=6.0, без usda_price (peg healthy, eligible)."""
    return _make_adapter(apy=6.0)


def _no_file() -> StusdAdapter:
    """Адаптер с несуществующим data_dir → fallback, peg healthy."""
    return _make_adapter(missing_file=True)


# ════════════════════════════════════════════════════════════════════════════
# 1. TestStusdInit — константы и идентичность
# ════════════════════════════════════════════════════════════════════════════

class TestStusdInit(unittest.TestCase):
    """12 тестов: все ключевые константы класса."""

    def test_protocol_key(self):
        self.assertEqual(StusdAdapter.PROTOCOL, "stusd")

    def test_protocol_name(self):
        self.assertEqual(
            StusdAdapter.PROTOCOL_NAME,
            "Angle Staked USDA (stUSD)",
        )

    def test_tier(self):
        self.assertEqual(StusdAdapter.TIER, "T2")

    def test_chain_id(self):
        self.assertEqual(StusdAdapter.CHAIN_ID, 1)

    def test_chain(self):
        self.assertEqual(StusdAdapter.CHAIN, "ethereum")

    def test_vault_address(self):
        self.assertEqual(StusdAdapter.VAULT_ADDRESS, _VAULT)

    def test_risk_score(self):
        self.assertAlmostEqual(StusdAdapter.RISK_SCORE, 0.43)

    def test_t2_cap(self):
        self.assertAlmostEqual(StusdAdapter.T2_CAP, 0.20)

    def test_exit_latency_instant(self):
        self.assertEqual(StusdAdapter.EXIT_LATENCY_HOURS, 0.0)

    def test_apy_bounds(self):
        self.assertAlmostEqual(StusdAdapter.MIN_APY_PCT, 3.0)
        self.assertAlmostEqual(StusdAdapter.MAX_APY_PCT, 12.0)
        self.assertAlmostEqual(StusdAdapter.DEFAULT_APY_PCT, 6.0)

    def test_tvl(self):
        self.assertAlmostEqual(StusdAdapter.TVL_USD, 50_000_000)
...
# ════════════════════════════════════════════════════════════════════════════
# 2. TestStusdAPY — чтение APY и health_check
# ════════════════════════════════════════════════════════════════════════════

class TestStusdAPY(unittest.TestCase):
    ...
    def test_apy_override_higher(self):
        a = _make_adapter(apy=9.5)
        self.assertAlmostEqual(a.get_apy(), 9.5)
    ...
    def test_apy_fallback_missing_field(self):
        ...
        self.assertAlmostEqual(a.get_apy(), StusdAdapter.DEFAULT_APY_PCT)
    ...
    def test_get_apy_pct_fallback(self):
        a = _no_file()
        self.assertAlmostEqual(a.get_apy_pct(), StusdAdapter.DEFAULT_APY_PCT)
...
# ════════════════════════════════════════════════════════════════════════════
# 3. TestStusdPeg — peg compliance gate (PEG_TOLERANCE = 0.5%)
# ════════════════════════════════════════════════════════════════════════════

class TestStusdPeg(unittest.TestCase):
    """13 тестов: is_peg_healthy при различных значениях usda_price."""

    def test_peg_missing_field_healthy(self):
        """Поле usda_price отсутствует → healthy (нет данных != депег)."""
        a = _make_adapter(usda_price=None)
...
# ════════════════════════════════════════════════════════════════════════════
# 4. TestStusdEligibility — is_eligible (peg OK + APY OK)
# ════════════════════════════════════════════════════════════════════════════

class TestStusdEligibility(unittest.TestCase):
    ...
    def test_not_eligible_depeg(self):
        ...
        a = _make_adapter(apy=6.0, usda_price=0.95)
    ...
    def test_not_eligible_both_fail(self):
        ...
        a = _make_adapter(apy=1.0, usda_price=0.90)
        self.assertFalse(a.is_eligible())
    ...
    def test_not_eligible_depeg_just_over(self):
        """usda_price=1.0051 (чуть за порогом), APY OK → not eligible."""
        a = _make_adapter(apy=6.0, usda_price=1.0051)
        self.assertFalse(a.is_eligible())

    def test_eligible_peg_at_boundary(self):
        """usda_price=0.995 (граница peg), APY OK → eligible."""
        a = _make_adapter(apy=6.0, usda_price=0.995)
        self.assertTrue(a.is_eligible())


# ════════════════════════════════════════════════════════════════════════════
# 5. TestStusdYieldInfo — get_yield_info
# ════════════════════════════════════════════════════════════════════════════

class TestStusdYieldInfo(unittest.TestCase):
    ...
    def test_yield_info_apy_decimal_override(self):
        ...
        self.assertAlmostEqual(a.get_yield_info().apy, 0.10)

    def test_yield_info_protocol(self):
        a = _default()
        self.assertEqual(a.get_yield_info().protocol, "stusd")
    ...
    def test_yield_info_risk_score(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().risk_score, 0.43)
    ...
    def test_yield_info_tvl(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().tvl_usd, 50_000_000)


# ════════════════════════════════════════════════════════════════════════════
# 6. TestStusdVsMorpho — vs_morpho_gap
# ════════════════════════════════════════════════════════════════════════════

class TestStusdVsMorpho(unittest.TestCase):
    """10 тестов: vs_morpho_gap (положительный/отрицательный/кастомный)."""

    def test_default_gap_positive_morpho_better(self):
        """Morpho 6.5% vs stUSD 6.0% → gap = +0.5 (Morpho лучше)."""
        a = _make_adapter(apy=6.0)
        self.assertAlmostEqual(a.vs_morpho_gap(), 0.5)

    def test_gap_negative_stusd_better(self):
        """Morpho 6.5% vs stUSD 8.0% → gap = -1.5 (stUSD лучше)."""
        a = _make_adapter(apy=8.0)
        self.assertAlmostEqual(a.vs_morpho_gap(), -1.5)

    def test_gap_zero_equal(self):
        """Morpho 6.5% vs stUSD 6.5% → gap = 0.0."""
        a = _make_adapter(apy=6.5)
        self.assertAlmostEqual(a.vs_morpho_gap(), 0.0)

    def test_custom_morpho_apy_higher(self):
        """Кастомный morpho_apy=10.0 vs stUSD 6.0% → gap = 4.0."""
        a = _make_adapter(apy=6.0)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=10.0), 4.0)

    def test_custom_morpho_apy_lower(self):
        """Кастомный morpho_apy=4.0 vs stUSD 6.0% → gap = -2.0."""
        a = _make_adapter(apy=6.0)
    ...
    def test_gap_large_positive(self):
        """Morpho 20% vs stUSD 6.0% → gap = 14.0."""
        a = _make_adapter(apy=6.0)
...
# ════════════════════════════════════════════════════════════════════════════
# 7. TestStusdAllocate — paper trading аллокация
# ════════════════════════════════════════════════════════════════════════════

class TestStusdAllocate(unittest.TestCase):
    ...
    def test_allocate_result_has_protocol(self):
        ...
        self.assertEqual(result["protocol"], "stusd")
...
# ════════════════════════════════════════════════════════════════════════════
# 8. TestStusdWithdraw — paper trading вывод
# ════════════════════════════════════════════════════════════════════════════

class TestStusdWithdraw(unittest.TestCase):
...
class TestStusdToDict(unittest.TestCase):
    ...
    def test_to_dict_has_eligible_false(self):
        ...
        a = _make_adapter(apy=6.0, usda_price=0.90)
        d = a.to_dict()
...
# ════════════════════════════════════════════════════════════════════════════
# 10. TestStusdRegistry — регистрация в пакете adapters
# ════════════════════════════════════════════════════════════════════════════

class TestStusdRegistry(unittest.TestCase):
    """5 тестов: импорт, ADAPTER_REGISTRY, __all__."""

    def test_import_from_package(self):
        from spa_core.adapters import StusdAdapter as Imported
        self.assertIs(Imported, StusdAdapter)

    def test_in_adapter_registry(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        keys = [r[0] for r in ADAPTER_REGISTRY]
        self.assertIn("stusd", keys)

    def test_registry_tier_t2(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "stusd"][0]
        self.assertEqual(entry[1], "T2")

    def test_registry_class(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "stusd"][0]
        self.assertIs(entry[2], StusdAdapter)

    def test_in_all(self):
        import spa_core.adapters as pkg
        self.assertIn("StusdAdapter", pkg.__all__)
