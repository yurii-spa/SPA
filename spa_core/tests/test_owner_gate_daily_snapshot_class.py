# FROZEN-DATE-OK: даты здесь — СОДЕРЖИМОЕ golden-снимка трека, а не понятие свежести.
# `evidenced_anchor: 2026-06-22`, `target_date: 2026-07-21` и ряд баров 2026-06-22.. —
# это вход генератора `build_snapshot`, который тесты заставляют пересчитать поле и
# сойтись байт-в-байт. Ни один ассерт в файле не судит о возрасте: гейт спрашивает
# «воспроизводится ли число из канона», и ответ не зависит от того, какой сегодня день.
# Сдвинуть их на относительные отметки нельзя — сместится сам пересчитываемый ряд, и
# тест перестанет быть воспроизведением аварии цикла #221.
