#!/usr/bin/env python3
"""Тесты ScrvusdAdapter — MP-560.

Покрываем:
  TestScrvusdInit        (12) — константы: protocol, tier=T2, chain_id, vault, risk_score, caps, apy, tvl
  TestScrvusdAPY         (12) — get_apy, fallback, json override, get_apy_pct, health_check
  TestScrvusdPeg         (13) — is_peg_healthy: missing→True, 1.0→True, в пределах, граница, депег, нечисловое
  TestScrvusdEligibility (11) — eligible = peg OK + APY OK; все комбинации
  TestScrvusdYieldInfo   ( 8) — get_yield_info: apy десятичной дробью, tier, risk_score, exit_latency
  TestScrvusdVsMorpho    (10) — vs_morpho_gap: положительный/отрицательный/кастомный
  TestScrvusdAllocate    (10) — нулевой, negative → ValueError, нормальный, структура ответа
  TestScrvusdWithdraw    (10) — нулевой, negative → ValueError, normal, insufficient
  TestScrvusdToDict      ( 9) — все ключи: peg_healthy, eligible, protocol, tier=T2
  TestScrvusdRegistry    ( 5) — импорт, ADAPTER_REGISTRY, __all__

Итого: 100 тестов.

Запуск:
    python3 -m pytest spa_core/tests/test_scrvusd_adapter.py -q
    python3 -m unittest spa_core.tests.test_scrvusd_adapter -v
    python3 spa_core/tests/test_scrvusd_adapter.py
"""
...
_VAULT = "0x0655977FEb2f289A4aB78af67BAB0d17aAb84367"
...
def _make_adapter(
    apy: float | None = 7.0,
    crvusd_price: float | None = None,
    missing_section: bool = False,
    missing_file: bool = False,
) -> ScrvusdAdapter:
    """Создаёт адаптер с временным data_dir.

    apy=None           → поле scrvusd.apy отсутствует (тест fallback)
    crvusd_price=None  → поле crvusd_price отсутствует (peg считается healthy)
    missing_section    → секция scrvusd полностью отсутствует
    missing_file       → adapter_status.json не существует
    """
    if missing_file:
        return ScrvusdAdapter(data_dir="/nonexistent_spa_test_scrvusd_xyz")
    ...
    if missing_section:
        ...
        if apy is not None:
            section["apy"] = apy
        if crvusd_price is not None:
            section["crvusd_price"] = crvusd_price
        content = {"scrvusd": section}

    (Path(tmp) / "adapter_status.json").write_text(
        json.dumps(content), encoding="utf-8"
    )
    return ScrvusdAdapter(data_dir=tmp)


def _default() -> ScrvusdAdapter:
    """Адаптер с apy=7.0, без crvusd_price (peg healthy, eligible)."""
    return _make_adapter(apy=7.0)


def _no_file() -> ScrvusdAdapter:
    """Адаптер с несуществующим data_dir → fallback, peg healthy."""
    return _make_adapter(missing_file=True)


# ════════════════════════════════════════════════════════════════════════════
# 1. TestScrvusdInit — константы и идентичность
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdInit(unittest.TestCase):
    """12 тестов: все ключевые константы класса."""

    def test_protocol_key(self):
        self.assertEqual(ScrvusdAdapter.PROTOCOL, "scrvusd")

    def test_protocol_name(self):
        self.assertEqual(
            ScrvusdAdapter.PROTOCOL_NAME,
            "Curve Savings crvUSD (scrvUSD)",
        )

    def test_tier(self):
        self.assertEqual(ScrvusdAdapter.TIER, "T2")

    def test_chain_id(self):
        self.assertEqual(ScrvusdAdapter.CHAIN_ID, 1)

    def test_chain(self):
        self.assertEqual(ScrvusdAdapter.CHAIN, "ethereum")

    def test_vault_address(self):
        self.assertEqual(ScrvusdAdapter.VAULT_ADDRESS, _VAULT)

    def test_risk_score(self):
        self.assertAlmostEqual(ScrvusdAdapter.RISK_SCORE, 0.42)

    def test_t2_cap(self):
        self.assertAlmostEqual(ScrvusdAdapter.T2_CAP, 0.20)

    def test_exit_latency_instant(self):
        self.assertEqual(ScrvusdAdapter.EXIT_LATENCY_HOURS, 0.0)

    def test_apy_bounds(self):
        self.assertAlmostEqual(ScrvusdAdapter.MIN_APY_PCT, 3.0)
        self.assertAlmostEqual(ScrvusdAdapter.MAX_APY_PCT, 15.0)
        self.assertAlmostEqual(ScrvusdAdapter.DEFAULT_APY_PCT, 7.0)

    def test_tvl(self):
        self.assertAlmostEqual(ScrvusdAdapter.TVL_USD, 100_000_000)
...
# ════════════════════════════════════════════════════════════════════════════
# 2. TestScrvusdAPY — чтение APY и health_check
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdAPY(unittest.TestCase):
    ...
    def test_apy_override_higher(self):
        a = _make_adapter(apy=11.5)
        self.assertAlmostEqual(a.get_apy(), 11.5)
    ...
    def test_apy_fallback_missing_field(self):
        # apy=None → поле отсутствует → fallback 7.0
        a = _make_adapter(apy=None)
        self.assertAlmostEqual(a.get_apy(), ScrvusdAdapter.DEFAULT_APY_PCT)
    ...
    def test_get_apy_pct_equals_get_apy(self):
        a = _make_adapter(apy=8.1)
        self.assertAlmostEqual(a.get_apy_pct(), a.get_apy())

    def test_get_apy_pct_fallback(self):
        a = _no_file()
        self.assertAlmostEqual(a.get_apy_pct(), ScrvusdAdapter.DEFAULT_APY_PCT)
...
# ════════════════════════════════════════════════════════════════════════════
# 3. TestScrvusdPeg — peg compliance gate (PEG_TOLERANCE = 1.0%)
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdPeg(unittest.TestCase):
    """13 тестов: is_peg_healthy при различных значениях crvusd_price."""

    def test_peg_missing_field_healthy(self):
        """Поле crvusd_price отсутствует → healthy (нет данных != депег)."""
        a = _make_adapter(crvusd_price=None)
...
# ════════════════════════════════════════════════════════════════════════════
# 4. TestScrvusdEligibility — is_eligible (peg OK + APY OK)
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdEligibility(unittest.TestCase):
    """11 тестов: is_eligible — все комбинации peg и APY."""

    def test_eligible_peg_ok_apy_ok(self):
        """peg healthy (no field), APY=7.0 → eligible."""
        a = _make_adapter(apy=7.0)
        self.assertTrue(a.is_eligible())

    def test_not_eligible_depeg(self):
        """Депег, APY OK → not eligible."""
        a = _make_adapter(apy=7.0, crvusd_price=0.95)
    ...
    def test_not_eligible_peg_ok_apy_too_high(self):
        """peg OK, APY=25.0 (above MAX) → not eligible."""
        a = _make_adapter(apy=25.0)
        self.assertFalse(a.is_eligible())
    ...
    def test_eligible_at_max_apy(self):
        """peg OK, APY=15.0 (MAX boundary) → eligible."""
        a = _make_adapter(apy=15.0)
        self.assertTrue(a.is_eligible())

    def test_not_eligible_both_fail(self):
        """Депег + APY=1.0 → not eligible."""
        a = _make_adapter(apy=1.0, crvusd_price=0.90)
        self.assertFalse(a.is_eligible())

    def test_eligible_no_file(self):
        """Нет файла → peg healthy (default), APY=fallback 7.0 → eligible."""
        a = _no_file()
        self.assertTrue(a.is_eligible())
    ...
    def test_not_eligible_depeg_just_over(self):
        """crvusd_price=1.0101 (чуть за порогом), APY OK → not eligible."""
        a = _make_adapter(apy=7.0, crvusd_price=1.0101)
        self.assertFalse(a.is_eligible())

    def test_eligible_peg_at_boundary(self):
        """crvusd_price=0.99 (граница peg), APY OK → eligible."""
        a = _make_adapter(apy=7.0, crvusd_price=0.99)
        self.assertTrue(a.is_eligible())


# ════════════════════════════════════════════════════════════════════════════
# 5. TestScrvusdYieldInfo — get_yield_info
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdYieldInfo(unittest.TestCase):
    ...
    def test_yield_info_apy_decimal(self):
        """APY=7.0% → 0.07 в YieldInfo (десятичная дробь)."""
        a = _make_adapter(apy=7.0)
        self.assertAlmostEqual(a.get_yield_info().apy, 0.07)
    ...
    def test_yield_info_protocol(self):
        a = _default()
        self.assertEqual(a.get_yield_info().protocol, "scrvusd")
    ...
    def test_yield_info_risk_score(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().risk_score, 0.42)
    ...
    def test_yield_info_tvl(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().tvl_usd, 100_000_000)


# ════════════════════════════════════════════════════════════════════════════
# 6. TestScrvusdVsMorpho — vs_morpho_gap
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdVsMorpho(unittest.TestCase):
    """10 тестов: vs_morpho_gap (положительный/отрицательный/кастомный)."""

    def test_default_gap_negative_scrvusd_better(self):
        """Morpho 6.5% vs scrvUSD 7.0% → gap = -0.5 (scrvUSD лучше)."""
        a = _make_adapter(apy=7.0)
        self.assertAlmostEqual(a.vs_morpho_gap(), -0.5)

    def test_gap_positive_morpho_better(self):
        """Morpho 6.5% vs scrvUSD 5.0% → gap = +1.5 (Morpho лучше)."""
        a = _make_adapter(apy=5.0)
        self.assertAlmostEqual(a.vs_morpho_gap(), 1.5)

    def test_gap_zero_equal(self):
        """Morpho 6.5% vs scrvUSD 6.5% → gap = 0.0."""
        a = _make_adapter(apy=6.5)
        self.assertAlmostEqual(a.vs_morpho_gap(), 0.0)

    def test_custom_morpho_apy_higher(self):
        """Кастомный morpho_apy=10.0 vs scrvUSD 7.0% → gap = 3.0."""
        a = _make_adapter(apy=7.0)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=10.0), 3.0)

    def test_custom_morpho_apy_lower(self):
        """Кастомный morpho_apy=4.0 vs scrvUSD 7.0% → gap = -3.0."""
        a = _make_adapter(apy=7.0)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=4.0), -3.0)

    def test_gap_with_fallback_apy(self):
        """Fallback APY=7.0 → gap = 6.5 - 7.0 = -0.5."""
        a = _no_file()
        self.assertAlmostEqual(a.vs_morpho_gap(), -0.5)
    ...
    def test_gap_symmetry(self):
        ...
        a1 = _make_adapter(apy=8.5)
        a2 = _make_adapter(apy=4.5)
        # gap1 = 6.5 - 8.5 = -2.0; gap2 = 6.5 - 4.5 = +2.0
        self.assertAlmostEqual(a1.vs_morpho_gap() + a2.vs_morpho_gap(), 0.0)

    def test_gap_large_positive(self):
        """Morpho 20% vs scrvUSD 7.0% → gap = 13.0."""
        a = _make_adapter(apy=7.0)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=20.0), 13.0)
...
# ════════════════════════════════════════════════════════════════════════════
# 7. TestScrvusdAllocate — paper trading аллокация
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdAllocate(unittest.TestCase):
    ...
    def test_allocate_result_has_protocol(self):
        ...
        self.assertEqual(result["protocol"], "scrvusd")

    def test_allocate_result_has_vault(self):
        a = _default()
        result = a.allocate(1000.0)
        self.assertEqual(result["vault"], _VAULT)
...
# ════════════════════════════════════════════════════════════════════════════
# 8. TestScrvusdWithdraw — paper trading вывод
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdWithdraw(unittest.TestCase):
...
# ════════════════════════════════════════════════════════════════════════════
# 9. TestScrvusdToDict — сериализация
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdToDict(unittest.TestCase):
    """9 тестов: все ключевые поля to_dict(), peg_healthy, eligible, protocol, tier=T2."""

    def test_to_dict_has_protocol(self):
        a = _default()
        d = a.to_dict()
        self.assertEqual(d["protocol"], "scrvusd")
    ...
    def test_to_dict_has_eligible_false(self):
        ...
        a = _make_adapter(apy=7.0, crvusd_price=0.90)
        d = a.to_dict()
    ...
    def test_to_dict_vault_address_correct(self):
        a = _default()
        d = a.to_dict()
        self.assertEqual(d["vault_address"], _VAULT)


# ════════════════════════════════════════════════════════════════════════════
# 10. TestScrvusdRegistry — регистрация в пакете adapters
# ════════════════════════════════════════════════════════════════════════════

class TestScrvusdRegistry(unittest.TestCase):
    """5 тестов: импорт, ADAPTER_REGISTRY, __all__."""

    def test_import_from_package(self):
        from spa_core.adapters import ScrvusdAdapter as Imported
        self.assertIs(Imported, ScrvusdAdapter)

    def test_in_adapter_registry(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        keys = [r[0] for r in ADAPTER_REGISTRY]
        self.assertIn("scrvusd", keys)

    def test_registry_tier_t2(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "scrvusd"][0]
        self.assertEqual(entry[1], "T2")

    def test_registry_class(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "scrvusd"][0]
        self.assertIs(entry[2], ScrvusdAdapter)

    def test_in_all(self):
        import spa_core.adapters as pkg
        self.assertIn("ScrvusdAdapter", pkg.__all__)
