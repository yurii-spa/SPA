...
class HandlerInterlock(unittest.TestCase):
    ...
    def test_stale_beacon_is_refused(self):
        ...
        ИЗМЕНЕНО НАМЕРЕННО 2026-09-16 (инв. #16, ADR-400): «давно» здесь было «час», а
    ...
    def test_a_restart_window_does_not_strip_the_buttons(self):
        """Положительный контроль ADR-400: маячку час (бот перезапускался) — кнопки есть.
    ...
    def test_the_button_threshold_is_not_the_liveness_threshold(self):
        """Два вопроса — два порога (ADR-400). Один порог на оба и был дефектом.
