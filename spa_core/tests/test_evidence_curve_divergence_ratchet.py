...
**Механизм — `own-32`, замерен прогоном 2026-08-17.** Версия «кривую пишут ДВА пути, и
в день остановки они берут «вчера» из разных источников» ОПРОВЕРГНУТА: halt-путь
`equity._write_equity` зовёт тот же `_upsert_equity_point`, «вчера» у него одно.
Настоящая причина — РАЗНАЯ семантика повторной записи за один день: кривая
(`equity.py:240`) сбрасывает сегодняшний бар и пересчитывает его ⇒ выигрывает
ПОСЛЕДНИЙ прогон дня; доказательная база (`paper_evidence_tracker.py:119`) на дубликат
даты возвращает уже записанную строку ⇒ выигрывает ПЕРВЫЙ. Один прогон в сутки ⇒
числа совпадают, два и больше ⇒ расходятся всегда.

Июньские даты в базе ниже — ОТДЕЛЬНЫЙ дефект того же файла: в кривой один разрыв
цепочки, 2026-06-10 `open_equity=100000.00` против вчерашнего закрытия 100216.00.
Отсюда максимум расхождения $215.99.
