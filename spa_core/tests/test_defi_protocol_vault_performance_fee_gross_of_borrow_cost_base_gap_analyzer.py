"""
Tests for MP-1216: DeFiProtocolVaultPerformanceFeeGrossOfBorrowCostBaseGapAnalyzer
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_borrow_cost_base_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_gross_of_borrow_cost_base_gap_analyzer import (  # noqa: E501
...
def make_pos(
    ...
    if net_of_borrow_cost_yield_pct is not None:
        pos["net_of_borrow_cost_yield_pct"] = net_of_borrow_cost_yield_pct
    if performance_fee_pct is not None:
        pos["performance_fee_pct"] = performance_fee_pct
    if borrow_cost_rate_pct is not None:
        pos["borrow_cost_rate_pct"] = borrow_cost_rate_pct
    if fee_on_borrow_cost_gap_pct is not None:
        pos["fee_on_borrow_cost_gap_pct"] = fee_on_borrow_cost_gap_pct
...
class TestHelpers(unittest.TestCase):
    ...
    def test_thresholds_ordered(self):
        ...
        self.assertGreater(HIGH_BORROW_COST_PCT, 0.0)

    def test_high_borrow_cost_value(self):
        self.assertEqual(HIGH_BORROW_COST_PCT, 10.0)

    def test_log_path_filename(self):
        self.assertTrue(LOG_PATH.endswith(
            "vault_performance_fee_gross_of_borrow_cost_base_gap_log.json"))
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_net_of_borrow_base(self):
        # net_of_borrow == gross → borrow cost consumed nothing → CLEAN.
        r = self.an.analyze(make_pos(
            gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BORROW_BASE")
        self.assertIn("CLEAN_NET_BASE", r["flags"])
        self.assertLessEqual(r["fee_on_borrow_cost_fraction"], CLEAN_FRACTION)
    ...
    def test_clean_default_net_zero_is_full(self):
        ...
        self.assertEqual(r["net_of_borrow_cost_yield_pct"], 0.0)
        self.assertIn("FULL_FEE_ON_BORROW", r["flags"])

    def test_mild_fee_on_borrow_gap(self):
        # gross 20, net 18 → fraction = 0.1 → MILD.
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0, net_of_borrow_cost_yield_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MILD_FEE_ON_BORROW_GAP")
        self.assertEqual(r["recommendation"], "MINOR_FEE_ON_BORROW")
        self.assertGreater(r["fee_on_borrow_cost_fraction"], CLEAN_FRACTION)
        self.assertLessEqual(r["fee_on_borrow_cost_fraction"], MILD_FRACTION)

    def test_moderate_fee_on_borrow_gap(self):
        # gross 16, net 8 → fraction = 0.5 → MODERATE (at boundary).
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MODERATE_FEE_ON_BORROW_GAP")
        self.assertAlmostEqual(r["fee_on_borrow_cost_fraction"], 0.5, places=4)
        self.assertIn("FEE_ON_BORROW_COST", r["flags"])
        self.assertEqual(r["recommendation"], "DEMAND_NET_OF_BORROW_BASE")

    def test_severe_fee_on_borrow_gap(self):
        # gross 20, net 4 → fraction = 0.8 → SEVERE.
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0, net_of_borrow_cost_yield_pct=4.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "SEVERE_FEE_ON_BORROW_GAP")
        self.assertGreater(r["fee_on_borrow_cost_fraction"], MODERATE_FRACTION)
        self.assertEqual(r["recommendation"], "AVOID_FEE_ON_BORROW")

    def test_severe_net_negative(self):
        # borrow cost drove net negative, fee on gross → net_fair negative.
        r = self.an.analyze(make_pos(
            gross_yield_pct=12.0, net_of_borrow_cost_yield_pct=-3.0,
    ...
    def test_fee_on_borrow_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_BORROW_COST", r["flags"])

    def test_full_fee_on_borrow_flag(self):
        # net <= 0 with positive gross → FULL_FEE_ON_BORROW.
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_BORROW", r["flags"])

    def test_high_borrow_cost_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0, borrow_cost_rate_pct=12.0))
        self.assertIn("HIGH_BORROW_COST", r["flags"])

    def test_low_borrow_cost_no_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=18.0,
            performance_fee_pct=20.0, borrow_cost_rate_pct=5.0))
        self.assertNotIn("HIGH_BORROW_COST", r["flags"])

    def test_high_borrow_cost_at_threshold(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0, borrow_cost_rate_pct=10.0))
        self.assertIn("HIGH_BORROW_COST", r["flags"])

    def test_borrow_cost_rate_echoed(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0, borrow_cost_rate_pct=7.5))
        self.assertEqual(r["borrow_cost_rate_pct"], 7.5)
...
class TestMath(unittest.TestCase):
    ...
    def test_moderate_geometry(self):
        ...
        self.assertAlmostEqual(r["fee_on_borrow_cost_gap_pct"], 1.6)
        self.assertAlmostEqual(r["borrow_consumed_yield_pct"], 8.0)
        self.assertAlmostEqual(r["fee_on_borrow_cost_fraction"], 0.5, places=4)
    ...
    def test_clean_geometry_zero_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=18.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_on_borrow_cost_gap_pct"], 0.0)
        self.assertAlmostEqual(r["borrow_consumed_yield_pct"], 0.0)
        self.assertEqual(r["fee_on_borrow_cost_fraction"], 0.0)
    ...
    def test_score_formula(self):
        ...
        frac = r["fee_on_borrow_cost_fraction"]
    ...
    def test_fee_rate_negative_clamped(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=10.0,
            performance_fee_pct=-20.0))
        self.assertEqual(r["performance_fee_pct"], 0.0)
        self.assertEqual(r["fee_charged_pct"], 0.0)
        self.assertEqual(r["fee_on_borrow_cost_gap_pct"], 0.0)

    def test_fee_rate_zero(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0, net_of_borrow_cost_yield_pct=10.0,
    ...
    def test_net_above_gross_no_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0, net_of_borrow_cost_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["fee_on_borrow_cost_gap_pct"], 0.0)
        self.assertEqual(r["borrow_consumed_yield_pct"], 0.0)
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BORROW_BASE")
    ...
    def test_gap_bounded_by_fee_charged(self):
        for net in (0.0, 2.0, 5.0, 9.9):
            r = self.an.analyze(make_pos(
                gross_yield_pct=10.0, net_of_borrow_cost_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertLessEqual(r["fee_on_borrow_cost_gap_pct"],
                                 r["fee_charged_pct"] + 1e-9)
            self.assertLessEqual(r["fee_on_borrow_cost_fraction"], 1.0)
            self.assertGreaterEqual(r["fee_on_borrow_cost_fraction"], 0.0)

    def test_string_numeric_inputs(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct="16", net_of_borrow_cost_yield_pct="8",
    ...
    def test_fraction_scale_free(self):
        ...
        r2 = self.an.analyze(make_pos(
            gross_yield_pct=32.0, net_of_borrow_cost_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r1["fee_on_borrow_cost_fraction"],
                               r2["fee_on_borrow_cost_fraction"], places=6)
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertIsNone(r["net_of_borrow_cost_yield_pct"])
        self.assertIsNone(r["borrow_consumed_yield_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_BORROW_COST", r["flags"])
        self.assertNotIn("FULL_FEE_ON_BORROW", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=-4.8,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_borrow_cost_gap_pct"], 4.8, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["fee_on_borrow_cost_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_borrow_cost_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_borrow_cost_fraction"]
    ...
    def test_override_denominator_is_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=3.0,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_borrow_cost_fraction"], 3.0 / 12.0,
                               places=4)

    def test_override_high_borrow_cost_still_flagged(self):
        # HIGH_BORROW_COST is not geometry-only → still raised on override.
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=3.0,
            fee_charged_pct=12.0, borrow_cost_rate_pct=30.0))
        self.assertIn("HIGH_BORROW_COST", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
    ...
    def test_override_severe_when_large_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=24.0, fee_on_borrow_cost_gap_pct=10.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"], "SEVERE_FEE_ON_BORROW_GAP")
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "fee_on_borrow_cost_gap_pct", "realization_ratio",
                  "fee_on_borrow_cost_fraction",
...
class TestFlags(unittest.TestCase):
    ...
    def test_classification_always_in_flags(self):
        for net in (18.0, 16.0, 8.0, 4.0):
            r = self.an.analyze(make_pos(
                gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=net,
                performance_fee_pct=20.0))
    ...
    def test_no_full_fee_on_borrow_when_positive_net(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=16.0, net_of_borrow_cost_yield_pct=8.0,
            performance_fee_pct=20.0))
        self.assertNotIn("FULL_FEE_ON_BORROW", r["flags"])

    def test_no_fee_on_borrow_when_clean(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=18.0, net_of_borrow_cost_yield_pct=18.0,
            performance_fee_pct=20.0))
        self.assertNotIn("FEE_ON_BORROW_COST", r["flags"])
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_borrow_gap_vault", agg)
...
class TestInvariants(unittest.TestCase):
            ...
            for net in (-5.0, 0.0, gross * 0.5, gross, gross + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    r = self.an.analyze(make_pos(
                        ...
                        net_of_borrow_cost_yield_pct=net,
                        performance_fee_pct=fee))
                    self.assertGreaterEqual(r["score"], 0.0)
                    self.assertLessEqual(r["score"], 100.0)
                    self.assertGreaterEqual(
                        r["fee_on_borrow_cost_fraction"], 0.0)
                    self.assertLessEqual(
                        r["fee_on_borrow_cost_fraction"], 1.0)
    ...
    def test_fraction_monotone_with_borrow(self):
        # holding gross/fee fixed, lower net (more borrow cost) → larger
        # (or equal) fraction (monotone non-decreasing).
        prev = -1.0
        for net in (20.0, 16.0, 12.0, 8.0, 4.0, 0.0):
            r = self.an.analyze(make_pos(
                gross_yield_pct=20.0, net_of_borrow_cost_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertGreaterEqual(r["fee_on_borrow_cost_fraction"], prev)
            prev = r["fee_on_borrow_cost_fraction"]
...
class TestGradesAndScore(unittest.TestCase):
    ...
    def test_grade_matches_score(self):
        for gross, net, fee in (
            ...
            r = self.an.analyze(make_pos(
                gross_yield_pct=gross, net_of_borrow_cost_yield_pct=net,
                performance_fee_pct=fee))
    ...
    def test_grade_boundary_b_c(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0, net_of_borrow_cost_yield_pct=12.0,
            performance_fee_pct=20.0))
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-Vault-CleanNetBase"], "CLEAN_NET_OF_BORROW_BASE")
        self.assertEqual(
            classes["stETH-Vault-ModerateBorrow"],
            "MODERATE_FEE_ON_BORROW_GAP")
        self.assertEqual(
            classes["GOV-Vault-SevereBorrow"],
