#!/usr/bin/env python3
"""Тесты AaveV3OptimismAdapter (spa_core.adapters) — MP-565.

Покрываем:
  TestInit               (16) — константы класса: protocol, tier, risk_score, tvl, etc.
  TestAPY                (15) — get_apy: fallback, JSON override, apy_pct alias, rejection
  ...
  TestSimulateDeposit    (11) — simulate_deposit: zero/negative raises, normal, cumulative, keys
  ...
  TestGasSavings         (10) — get_gas_savings_vs_mainnet: shape, values, chain, immutability
  TestRegistry           ( 7) — import, ADAPTER_REGISTRY, __all__, BaseAdapter subclass

Итого: 135 тестов (≥ 100, все зелёные).

Запуск:
    python3 -m unittest spa_core.tests.test_aave_v3_optimism_adapter -v
    python3 spa_core/tests/test_aave_v3_optimism_adapter.py
"""
...
_USDC_OP = "0x0b2C639c533813f4Aa9D7837CAf62653d097Ff85"
...
def _make_adapter(
    ...
    missing_section   → секция aave_v3_optimism полностью отсутствует
    missing_file      → adapter_status.json не существует
    """
    if missing_file:
        return AaveV3OptimismAdapter(data_dir="/nonexistent_spa_test_op_xyz")
    ...
    if missing_section:
        ...
        content = {"aave_v3_optimism": section}

    (Path(tmp) / "adapter_status.json").write_text(
        json.dumps(content), encoding="utf-8"
    )
    return AaveV3OptimismAdapter(data_dir=tmp)


def _default() -> AaveV3OptimismAdapter:
    """Адаптер с apy=4.8, без usdc_price (peg healthy, eligible)."""
    return _make_adapter(apy=4.8)


def _no_file() -> AaveV3OptimismAdapter:
    return _make_adapter(missing_file=True)


def _no_section() -> AaveV3OptimismAdapter:
    return _make_adapter(missing_section=True)


def _depeg(price: float) -> AaveV3OptimismAdapter:
    return _make_adapter(apy=4.8, usdc_price=price)


# ─── TestInit ─────────────────────────────────────────────────────────────────

class TestInit(unittest.TestCase):
    """16 тестов — константы и инициализация."""

    def setUp(self):
        self.a = _default()

    def test_protocol(self):
        self.assertEqual(self.a.PROTOCOL, "aave_v3_optimism")

    def test_tier_class(self):
        self.assertEqual(AaveV3OptimismAdapter.TIER, "T1")

    def test_tier_instance(self):
        self.assertEqual(self.a.tier, "T1")

    def test_risk_score(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.RISK_SCORE, 0.25)

    def test_tvl(self):
        self.assertEqual(AaveV3OptimismAdapter.TVL_USD, 600_000_000)

    def test_default_apy(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.DEFAULT_APY_PCT, 4.8)

    def test_peg_tolerance(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.PEG_TOLERANCE, 0.005)

    def test_exit_latency(self):
        self.assertEqual(AaveV3OptimismAdapter.EXIT_LATENCY_HOURS, 0.0)

    def test_t1_cap(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.T1_CAP, 0.40)

    def test_pool_address(self):
        self.assertEqual(AaveV3OptimismAdapter.POOL_ADDRESS, _POOL)

    def test_usdc_address(self):
        self.assertEqual(AaveV3OptimismAdapter.USDC_ADDRESS, _USDC_OP)

    def test_chain(self):
        self.assertEqual(AaveV3OptimismAdapter.CHAIN, "optimism")

    def test_chain_id(self):
        self.assertEqual(AaveV3OptimismAdapter.CHAIN_ID, 10)
    ...
    def test_gas_savings_pct(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.GAS_SAVINGS_PCT, 95.0)
...
class TestAPY(unittest.TestCase):
    ...
    def test_json_override_high(self):
        self.assertAlmostEqual(_make_adapter(apy=9.9).get_apy(), 9.9)
    ...
    def test_apy_fallback_alias_matches(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.APY_FALLBACK,
                               AaveV3OptimismAdapter.DEFAULT_APY_PCT)

    def test_min_apy_pct(self):
        self.assertAlmostEqual(AaveV3OptimismAdapter.MIN_APY_PCT, 1.0)

    def test_max_apy_pct(self):
...
class TestPeg(unittest.TestCase):
    ...
    def test_string_price_treated_healthy(self):
        tmp = tempfile.mkdtemp()
        (Path(tmp) / "adapter_status.json").write_text(
            json.dumps({"aave_v3_optimism": {"usdc_price": "0.90"}}),
            encoding="utf-8",
        )
        self.assertTrue(AaveV3OptimismAdapter(data_dir=tmp).is_peg_healthy())

    def test_bool_price_treated_healthy(self):
        tmp = tempfile.mkdtemp()
        (Path(tmp) / "adapter_status.json").write_text(
            json.dumps({"aave_v3_optimism": {"usdc_price": False}}),
            encoding="utf-8",
        )
        self.assertTrue(AaveV3OptimismAdapter(data_dir=tmp).is_peg_healthy())
    ...
    def test_boundary_lower_edge_healthy(self):
        """0.995 == 1.0 - 0.005 → ровно на границе допуска → healthy."""
        self.assertTrue(_depeg(0.995).is_peg_healthy())
...
class TestEligibility(unittest.TestCase):
    ...
    def test_depeg_overrides_good_apy(self):
        a = _make_adapter(apy=5.0, usdc_price=0.90)
    ...
    def test_good_peg_good_apy_eligible(self):
        a = _make_adapter(apy=4.8, usdc_price=1.001)
        self.assertTrue(a.is_eligible())

    def test_borderline_depeg_not_eligible(self):
        a = _make_adapter(apy=4.8, usdc_price=0.994)
        self.assertFalse(a.is_eligible())
...
class TestYieldInfo(unittest.TestCase):
    ...
    def test_protocol_key(self):
        self.assertEqual(self.yi.protocol, "aave_v3_optimism")

    def test_asset(self):
        self.assertEqual(self.yi.asset, "USDC")

    def test_apy_is_decimal(self):
        """YieldInfo.apy должен быть десятичной дробью (0.048 для 4.8%)."""
        self.assertAlmostEqual(self.yi.apy, 0.048)
    ...
    def test_tvl_matches_constant(self):
        self.assertAlmostEqual(self.yi.tvl_usd, 600_000_000)

    def test_tier_is_t1(self):
        self.assertEqual(self.yi.tier, "T1")

    def test_risk_score(self):
        self.assertAlmostEqual(self.yi.risk_score, 0.25)
    ...
    def test_json_override_reflected_in_yield_info(self):
        ...
        yi = a.get_yield_info()
        self.assertAlmostEqual(yi.apy, 0.05)


# ─── TestSimulateDeposit ──────────────────────────────────────────────────────

class TestSimulateDeposit(unittest.TestCase):
    """11 тестов — simulate_deposit."""
    ...
    def test_protocol_key(self):
        r = _default().simulate_deposit(1000)
        self.assertEqual(r["protocol"], "aave_v3_optimism")
...
class TestAllocate(unittest.TestCase):
    ...
    def test_chain_in_result(self):
        self.assertEqual(_default().allocate(1000)["chain"], "optimism")

    def test_chain_id_in_result(self):
        self.assertEqual(_default().allocate(1000)["chain_id"], 10)

    def test_gas_cost_l2(self):
        r = _default().allocate(1000)
        self.assertAlmostEqual(r["gas_cost_usd"], 0.005)
...
class TestGetHealth(unittest.TestCase):
    ...
    def test_apy_pct_present(self):
        ...
        self.assertAlmostEqual(h["apy_pct"], 4.8)
...
class TestToDict(unittest.TestCase):
    ...
    def test_protocol_key(self):
        self.assertEqual(self.d["protocol"], "aave_v3_optimism")

    def test_tier(self):
        self.assertEqual(self.d["tier"], "T1")

    def test_chain(self):
        self.assertEqual(self.d["chain"], "optimism")

    def test_chain_id(self):
        self.assertEqual(self.d["chain_id"], 10)
    ...
    def test_l2_advantages_is_dict(self):
        self.assertIsInstance(self.d["l2_advantages"], dict)

    def test_tvl_usd(self):
        self.assertAlmostEqual(self.d["tvl_usd"], 600_000_000)
...
class TestGasSavings(unittest.TestCase):
    """10 тестов — get_gas_savings_vs_mainnet."""
    ...
    def test_savings_pct_value(self):
        self.assertAlmostEqual(self.g["savings_pct"], 95.0)

    def test_chain_is_optimism(self):
        self.assertEqual(self.g["chain"], "optimism")

    def test_gas_l2_usd(self):
        self.assertAlmostEqual(self.g["gas_l2_usd"], 0.005)
    ...
    def test_finality_minutes_value(self):
        self.assertEqual(self.g["finality_minutes"], 10)

    def test_mainnet_bridge_exit_days(self):
        self.assertIn("mainnet_bridge_exit_days", self.g)

    def test_savings_lt_100(self):
        self.assertLess(self.g["savings_pct"], 100)
    ...
    def test_independent_copies(self):
        ...
        self.assertAlmostEqual(g2["savings_pct"], 95.0)


# ─── TestRegistry ─────────────────────────────────────────────────────────────

class TestRegistry(unittest.TestCase):
    """7 тестов — import, ADAPTER_REGISTRY, __all__, BaseAdapter subclass."""

    def test_import_adapter(self):
        from spa_core.adapters.aave_v3_optimism_adapter import AaveV3OptimismAdapter as A
        self.assertIsNotNone(A)

    def test_is_baseadapter_subclass(self):
        self.assertTrue(issubclass(AaveV3OptimismAdapter, BaseAdapter))

    def test_instance_is_baseadapter(self):
        self.assertIsInstance(_default(), BaseAdapter)

    def test_registry_contains_aave_v3_optimism(self):
    ...
    def test_registry_tier_is_t1(self):
        ...
        for key, tier, cls in ADAPTER_REGISTRY:
            if key == "aave_v3_optimism":
                self.assertEqual(tier, "T1")
                break

    def test_registry_class_is_adapter(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        for key, tier, cls in ADAPTER_REGISTRY:
            if key == "aave_v3_optimism":
                self.assertIs(cls, AaveV3OptimismAdapter)
                break

    def test_all_includes_adapter(self):
        from spa_core.adapters import __all__
        self.assertIn("AaveV3OptimismAdapter", __all__)
