...
GOLDEN_ROUTES = {
    ...
    # Endpoints added this cycle (were never captured in the golden because CI never
    # reached the unit step all session): privacy analytics (own counter), the honest
    # go-live-readiness surface, and the N-book capacity proof surface.
    ("/api/analytics/event", ("POST",)),
    ("/api/analytics/summary", ("GET",)),
    ("/api/interest", ("POST",)),
    ("/api/interest/summary", ("GET",)),
    ("/api/pilot/summary", ("GET",)),
    ("/api/readiness", ("GET",)),
    ...
    ("/api/tier1/proof", ("GET",)),
    # Added since last golden refresh (2026-07-15): CMO editorial layer draft-approval surface
    # (flow B — draft → owner approve/reject → publish; deterministic honesty-gate; LLM only in the
    # marketing-rewrite step, never in risk/exec/monitoring).
    ("/api/cmo/drafts", ("GET",)),
...
def test_route_count_stable():
    ...
    129 HTTP handlers + 1 websocket (/ws/agents) = 130 entries in GOLDEN_ROUTES. This
