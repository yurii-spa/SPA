...
def test_route_count_stable():
    ...
    112 HTTP handlers + 1 websocket (/ws/agents) = 113 entries in GOLDEN_ROUTES. This
    ...
    """
    assert len(_app_route_table()) == 113
...
def test_openapi_path_count_stable():
    ...
    assert len(paths) == 112  # HTTP handlers; /ws/agents is a websocket (not an OpenAPI path)
