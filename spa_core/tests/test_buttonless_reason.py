...
#: Выбор НАПИСАН, а разобрать его нельзя: карточка задаёт ДВА решения, и метки
#: повторяются («а»/«б» в каждом). Разбор отказывает по правилу «дубль ⇒ два вопроса,
#: смешивать нельзя» (fail-CLOSED) — и это ВЕРНО. Неверно было называть такой отказ
#: «вариантов в карточке не нашёл, перепиши вопрос»: перечень уже по §2.4, а лечится
#: состояние кодом или делением карточки, не переписыванием.
_UNREADABLE_OPTIONS = (
    "## Что случилось и почему это важно\n\nВоронка ведёт в никуда.\n\n"
    "## Что от тебя нужно\n\n"
