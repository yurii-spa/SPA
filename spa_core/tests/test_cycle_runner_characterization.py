...
_GOLDEN = {
    ...
    "result": {
        # ADR-301: у результата теперь ДВА поля — заработанное и ожидаемое. В этой сцене
        # всё наблюдаемо, поэтому они совпадают; расходятся они ровно тогда, когда в книге
        # есть ненаблюдаемый пул, и тем самым поле перестаёт быть немым.
        "apy_expected_pct": 2.57,
        "apy_today_pct": 2.57,
    ...
    "trade": {
        "capital": 100000.0,
        # ИЗМЕНЕНО 2026-08-30 32500.0 -> 65000.0 (инв. 16; журнал W35).
        # `from_allocation` в этом эталоне ПУСТ — книга разворачивается из
        # кэша, у хода одна нога. Прежние 32 500 = 65 000/2 занижали оборот
        # вдвое: L1 идёт по ключам протоколов, у кэша ключа нет. Число
        # приведено к тому же определению, которым демпфер судит ход
        # (churn_damper.one_sided_turnover, ADR-168). Поведение цикла,
        # кроме этой записи, не менялось — прочие поля эталона совпали.
        "delta_abs": 65000.0,
        "diff_usd": 65000.0,
        "from_allocation": {},
        "is_demo": False,
