...
лежат в дереве до сих пор: `ADR-073` разошёлся честно, `ADR-067` — нет (два разных решения,
оба `Accepted`).
...
def test_baseline_entries_carry_a_traceable_reason():
    ...
    baseline = json.loads(BASELINE.read_text(encoding="utf-8"))
    assert baseline["known_duplicates"], "пустая база — сначала удали храповик, а не запись"
    for key, why in baseline["known_duplicates"].items():
