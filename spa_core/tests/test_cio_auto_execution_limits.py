...
class TestTheTrapThatMadeThisModuleNecessary(unittest.TestCase):
    ...
    def test_t3_total_cap_passes_the_gate_untouched(self):
        """Настоящая ошибка замера 07.09, воспроизведённая дословно.

        Двадцать пять процентов раскладываются на ДВА пула T3 (15 % + 10 %):
        каждый ниже потолка НА ПРОТОКОЛ (20 %), сумма выше потолка НА ТИР (15 %).
        Иначе спрашивалось бы совсем другое ограничение — на этом первая
        редакция пробы и ошиблась.

        Если проверка покраснеет «сама» — то есть гейт начнёт применять
        суммарный потолок T3, — вывод модуля про `allowed tiers` изменится, и
        узнать об этом надо здесь, а не из отчёта.
        """
        ...
        self.assertTrue(
            ...
            "гейт обязан пропустить T3 СУММАРНО 25 % при каждом пуле ниже "
            "потолка на протокол — на этом факте держится весь смысл "
            f"двухповерхностного замера; получено: {verdict}")
