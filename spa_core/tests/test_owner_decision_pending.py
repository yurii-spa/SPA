...
def test_an_old_report_without_the_block_is_not_read_as_a_full_queue():
    ...
    assert "полнота очереди НЕ ИЗМЕРЕНА" in text
    # ЦИКЛ #528, изменение НАМЕРЕННОЕ и объяснённое (инв. #16): см. тот же
    # разбор в `test_branch_only_owner_questions`. Проза «отчёт старого
    # образца» была причиной БЕЗ замера; утверждение «отчёт, записанный ДО
    # #270, не выглядит измеренным» проверяет строка выше и оно не тронуто.
    # У ЭТОЙ сцены отчёт вообще нечем датировать (нет `generated_at`), и
    # честный ответ — «сказать НЕЧЕМ», а не «старый образец».
    assert "origin_queue" in text, text
    assert "нечем датировать" in text, text
