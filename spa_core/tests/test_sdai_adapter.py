#!/usr/bin/env python3
"""Тесты SdaiAdapter — MP-562.

Покрываем:
  TestSdaiInit        (12) — константы: protocol, tier=T2, chain_id, vault, risk_score, caps, apy, tvl
  TestSdaiAPY         (12) — get_apy, fallback, json override, get_apy_pct, health_check
  TestSdaiPeg         (13) — is_peg_healthy: missing→True, 1.0→True, в пределах, граница, депег, нечисловое
  TestSdaiEligibility (11) — eligible = peg OK + APY OK; все комбинации
  TestSdaiYieldInfo   ( 8) — get_yield_info: apy десятичной дробью, tier, risk_score, exit_latency
  TestSdaiVsMorpho    (10) — vs_morpho_gap: положительный/отрицательный/кастомный
  TestSdaiAllocate    (10) — нулевой, negative → ValueError, нормальный, структура ответа
  TestSdaiWithdraw    (10) — нулевой, negative → ValueError, normal, insufficient
  TestSdaiToDict      ( 9) — все ключи: peg_healthy, eligible, protocol, tier=T2
  TestSdaiRegistry    ( 5) — импорт, ADAPTER_REGISTRY, __all__

Итого: 100 тестов.

Запуск:
    python3 -m pytest spa_core/tests/test_sdai_adapter.py -q
    python3 -m unittest spa_core.tests.test_sdai_adapter -v
    python3 spa_core/tests/test_sdai_adapter.py
"""
...
_VAULT = "0x83F20F44975D03b1b09e64809B757c47f942BEeA"
...
def _make_adapter(
    apy: float | None = 5.5,
    dai_price: float | None = None,
    missing_section: bool = False,
    missing_file: bool = False,
) -> SdaiAdapter:
    """Создаёт адаптер с временным data_dir.

    apy=None          → поле sdai.apy отсутствует (тест fallback)
    dai_price=None    → поле dai_price отсутствует (peg считается healthy)
    missing_section   → секция sdai полностью отсутствует
    missing_file      → adapter_status.json не существует
    """
    if missing_file:
        return SdaiAdapter(data_dir="/nonexistent_spa_test_sdai_xyz")
    ...
    if missing_section:
        ...
        if dai_price is not None:
            section["dai_price"] = dai_price
        content = {"sdai": section}

    (Path(tmp) / "adapter_status.json").write_text(
        json.dumps(content), encoding="utf-8"
    )
    return SdaiAdapter(data_dir=tmp)


def _default() -> SdaiAdapter:
    """Адаптер с apy=5.5, без dai_price (peg healthy, eligible)."""
    return _make_adapter(apy=5.5)


def _no_file() -> SdaiAdapter:
    """Адаптер с несуществующим data_dir → fallback, peg healthy."""
    return _make_adapter(missing_file=True)


# ════════════════════════════════════════════════════════════════════════════
# 1. TestSdaiInit — константы и идентичность
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiInit(unittest.TestCase):
    """12 тестов: все ключевые константы класса."""

    def test_protocol_key(self):
        self.assertEqual(SdaiAdapter.PROTOCOL, "sdai")

    def test_protocol_name(self):
        self.assertEqual(
            SdaiAdapter.PROTOCOL_NAME,
            "MakerDAO Savings DAI (sDAI)",
        )

    def test_tier(self):
        self.assertEqual(SdaiAdapter.TIER, "T2")

    def test_chain_id(self):
        self.assertEqual(SdaiAdapter.CHAIN_ID, 1)

    def test_chain(self):
        self.assertEqual(SdaiAdapter.CHAIN, "ethereum")

    def test_vault_address(self):
        self.assertEqual(SdaiAdapter.VAULT_ADDRESS, _VAULT)

    def test_risk_score(self):
        self.assertAlmostEqual(SdaiAdapter.RISK_SCORE, 0.38)

    def test_t2_cap(self):
        self.assertAlmostEqual(SdaiAdapter.T2_CAP, 0.20)

    def test_exit_latency_instant(self):
        self.assertEqual(SdaiAdapter.EXIT_LATENCY_HOURS, 0.0)

    def test_apy_bounds(self):
        self.assertAlmostEqual(SdaiAdapter.MIN_APY_PCT, 2.0)
        self.assertAlmostEqual(SdaiAdapter.MAX_APY_PCT, 12.0)
        self.assertAlmostEqual(SdaiAdapter.DEFAULT_APY_PCT, 5.5)

    def test_tvl(self):
        self.assertAlmostEqual(SdaiAdapter.TVL_USD, 1_300_000_000)
...
# ════════════════════════════════════════════════════════════════════════════
# 2. TestSdaiAPY — чтение APY и health_check
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiAPY(unittest.TestCase):
    ...
    def test_apy_override_lower(self):
        a = _make_adapter(apy=3.2)
        self.assertAlmostEqual(a.get_apy(), 3.2)

    def test_apy_fallback_missing_field(self):
        # apy=None → поле отсутствует → fallback 5.5
        a = _make_adapter(apy=None)
        self.assertAlmostEqual(a.get_apy(), SdaiAdapter.DEFAULT_APY_PCT)
    ...
    def test_get_apy_pct_fallback(self):
        a = _no_file()
        self.assertAlmostEqual(a.get_apy_pct(), SdaiAdapter.DEFAULT_APY_PCT)
...
# ════════════════════════════════════════════════════════════════════════════
# 3. TestSdaiPeg — peg compliance gate (PEG_TOLERANCE = 0.5%)
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiPeg(unittest.TestCase):
    """13 тестов: is_peg_healthy при различных значениях dai_price."""

    def test_peg_missing_field_healthy(self):
        """Поле dai_price отсутствует → healthy (нет данных != депег)."""
        a = _make_adapter(dai_price=None)
...
class TestSdaiEligibility(unittest.TestCase):
    ...
    def test_eligible_at_min_apy(self):
        """peg OK, APY=2.0 (MIN boundary) → eligible."""
        a = _make_adapter(apy=2.0)
        self.assertTrue(a.is_eligible())
    ...
    def test_not_eligible_both_fail(self):
        """Депег + APY=0.5 → not eligible."""
        a = _make_adapter(apy=0.5, dai_price=0.97)
        self.assertFalse(a.is_eligible())

    def test_eligible_no_file(self):
        """Нет файла → peg healthy (default), APY=fallback 5.5 → eligible."""
        a = _no_file()
        self.assertTrue(a.is_eligible())
    ...
    def test_not_eligible_depeg_just_over(self):
        """dai_price=1.0051 (чуть за порогом), APY OK → not eligible."""
        a = _make_adapter(apy=5.5, dai_price=1.0051)
        self.assertFalse(a.is_eligible())

    def test_eligible_peg_at_boundary(self):
        """dai_price=0.995 (граница peg), APY OK → eligible."""
        a = _make_adapter(apy=5.5, dai_price=0.995)
        self.assertTrue(a.is_eligible())


# ════════════════════════════════════════════════════════════════════════════
# 5. TestSdaiYieldInfo — get_yield_info
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiYieldInfo(unittest.TestCase):
    ...
    def test_yield_info_apy_decimal(self):
        """APY=5.5% → 0.055 в YieldInfo (десятичная дробь)."""
        a = _make_adapter(apy=5.5)
        self.assertAlmostEqual(a.get_yield_info().apy, 0.055)

    def test_yield_info_apy_decimal_override(self):
        a = _make_adapter(apy=8.0)
        self.assertAlmostEqual(a.get_yield_info().apy, 0.08)

    def test_yield_info_protocol(self):
        a = _default()
        self.assertEqual(a.get_yield_info().protocol, "sdai")
    ...
    def test_yield_info_risk_score(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().risk_score, 0.38)
    ...
    def test_yield_info_tvl(self):
        a = _default()
        self.assertAlmostEqual(a.get_yield_info().tvl_usd, 1_300_000_000)


# ════════════════════════════════════════════════════════════════════════════
# 6. TestSdaiVsMorpho — vs_morpho_gap
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiVsMorpho(unittest.TestCase):
    """10 тестов: vs_morpho_gap (положительный/отрицательный/кастомный)."""

    def test_default_gap_positive_morpho_better(self):
        """Morpho 6.5% vs sDAI 5.5% → gap = +1.0 (Morpho лучше)."""
        a = _make_adapter(apy=5.5)
        self.assertAlmostEqual(a.vs_morpho_gap(), 1.0)

    def test_gap_negative_sdai_better(self):
        """Morpho 6.5% vs sDAI 8.0% → gap = -1.5 (sDAI лучше)."""
        a = _make_adapter(apy=8.0)
        self.assertAlmostEqual(a.vs_morpho_gap(), -1.5)

    def test_gap_zero_equal(self):
        """Morpho 6.5% vs sDAI 6.5% → gap = 0.0."""
        a = _make_adapter(apy=6.5)
        self.assertAlmostEqual(a.vs_morpho_gap(), 0.0)

    def test_custom_morpho_apy_higher(self):
        """Кастомный morpho_apy=10.0 vs sDAI 5.5% → gap = 4.5."""
        a = _make_adapter(apy=5.5)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=10.0), 4.5)

    def test_custom_morpho_apy_lower(self):
        """Кастомный morpho_apy=3.0 vs sDAI 5.5% → gap = -2.5."""
        a = _make_adapter(apy=5.5)
        self.assertAlmostEqual(a.vs_morpho_gap(morpho_apy=3.0), -2.5)
    ...
    def test_gap_symmetry(self):
        ...
        a1 = _make_adapter(apy=8.5)  # gap = 6.5 - 8.5 = -2.0
        a2 = _make_adapter(apy=4.5)  # gap = 6.5 - 4.5 = +2.0
        self.assertAlmostEqual(a1.vs_morpho_gap() + a2.vs_morpho_gap(), 0.0)

    def test_gap_large_positive(self):
        """Morpho 20% vs sDAI 5.5% → gap = 14.5."""
        a = _make_adapter(apy=5.5)
...
# ════════════════════════════════════════════════════════════════════════════
# 7. TestSdaiAllocate — paper trading аллокация
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiAllocate(unittest.TestCase):
    ...
    def test_allocate_result_has_protocol(self):
        ...
        self.assertEqual(result["protocol"], "sdai")
...
# ════════════════════════════════════════════════════════════════════════════
# 8. TestSdaiWithdraw — paper trading вывод
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiWithdraw(unittest.TestCase):
...
# ════════════════════════════════════════════════════════════════════════════
# 10. TestSdaiRegistry — регистрация в пакете adapters
# ════════════════════════════════════════════════════════════════════════════

class TestSdaiRegistry(unittest.TestCase):
    """5 тестов: импорт, ADAPTER_REGISTRY, __all__."""

    def test_import_from_package(self):
        from spa_core.adapters import SdaiAdapter as Imported
        self.assertIs(Imported, SdaiAdapter)

    def test_in_adapter_registry(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        keys = [r[0] for r in ADAPTER_REGISTRY]
        self.assertIn("sdai", keys)

    def test_registry_tier_t2(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "sdai"][0]
        self.assertEqual(entry[1], "T2")

    def test_registry_class(self):
        from spa_core.adapters import ADAPTER_REGISTRY
        entry = [r for r in ADAPTER_REGISTRY if r[0] == "sdai"][0]
        self.assertIs(entry[2], SdaiAdapter)

    def test_in_all(self):
        import spa_core.adapters as pkg
        self.assertIn("SdaiAdapter", pkg.__all__)
