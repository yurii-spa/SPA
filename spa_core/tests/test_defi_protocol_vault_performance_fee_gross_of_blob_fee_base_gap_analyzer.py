"""
Tests for MP-1235: DeFiProtocolVaultPerformanceFeeGrossOfBlobFeeBaseGapAnalyzer
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_blob_fee_base_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_gross_of_blob_fee_base_gap_analyzer import (  # noqa: E501
    DeFiProtocolVaultPerformanceFeeGrossOfBlobFeeBaseGapAnalyzer,
    _f,
    _clamp,
    _mean,
    _safe_div,
...
def make_pos(
    ...
    if net_of_blob_fee_yield_pct is not None:
        pos["net_of_blob_fee_yield_pct"] = net_of_blob_fee_yield_pct
    if performance_fee_pct is not None:
        pos["performance_fee_pct"] = performance_fee_pct
    if blob_fee_rate_pct is not None:
        pos["blob_fee_rate_pct"] = blob_fee_rate_pct
    if fee_on_blob_fee_gap_pct is not None:
        pos["fee_on_blob_fee_gap_pct"] = fee_on_blob_fee_gap_pct
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_equal_net_gross(self):
        ...
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BLOB_FEE_BASE")
    ...
    def test_clean_net_slightly_below_gross(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_blob_fee_yield_pct=14.8,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BLOB_FEE_BASE")
    ...
    def test_mild_gap(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            net_of_blob_fee_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 0.2, places=4)
        self.assertEqual(r["classification"], "MILD_FEE_ON_BLOB_FEE_GAP")
    ...
    def test_moderate_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_blob_fee_yield_pct=10.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 0.5, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_BLOB_FEE_GAP")
    ...
    def test_severe_gap_high_fraction(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_blob_fee_yield_pct=4.0,
            performance_fee_pct=20.0))
        self.assertGreater(r["fee_on_blob_fee_fraction"], 0.50)
        self.assertEqual(r["classification"],
    ...
    def test_net_zero_yields_full_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_blob_fee_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 1.0, places=4)
        self.assertEqual(r["classification"],
                         "SEVERE_FEE_ON_BLOB_FEE_GAP")
    ...
    def test_recommendation_mild(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_blob_fee_yield_pct=16.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["recommendation"], "MINOR_FEE_ON_BLOB_FEE")
    ...
    def test_recommendation_severe(self):
        r = self.an.analyze(make_pos(
            ...
            net_of_blob_fee_yield_pct=-2.0,
            performance_fee_pct=50.0))
        self.assertEqual(r["recommendation"], "AVOID_FEE_ON_BLOB_FEE")
...
class TestMath(unittest.TestCase):
    ...
    def test_fee_charged_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_blob_fee_yield_pct=14.0,
            performance_fee_pct=20.0))
    ...
    def test_gap_formula(self):
        ...
        self.assertAlmostEqual(r["fee_on_blob_fee_gap_pct"], expected_gap, places=4)
    ...
    def test_fraction_formula(self):
        ...
        charged = 20.0 * 0.2
        gap = (20.0 - 14.0) * 0.2
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], gap / charged,
                               places=4)

    def test_blob_fee_consumed_formula(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_blob_fee_yield_pct=14.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["blob_fee_consumed_yield_pct"], 6.0,
                               places=4)
    ...
    def test_overstatement_equals_gap(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            net_of_blob_fee_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["overstatement_pct"],
                               r["fee_on_blob_fee_gap_pct"], places=4)
    ...
    def test_performance_fee_pct_clamped_low(self):
        ...
        self.assertAlmostEqual(r["fee_on_blob_fee_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BLOB_FEE_BASE")
    ...
    def test_net_above_gross_clamps_consumed_zero(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            net_of_blob_fee_yield_pct=12.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["blob_fee_consumed_yield_pct"], 0.0,
                               places=4)
        self.assertAlmostEqual(r["fee_on_blob_fee_gap_pct"], 0.0, places=4)
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BLOB_FEE_BASE")
    ...
    def test_zero_fee_rate_always_clean(self):
        ...
        self.assertAlmostEqual(r["fee_on_blob_fee_gap_pct"], 0.0, places=4)
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 0.0, places=4)
        self.assertEqual(r["classification"], "CLEAN_NET_OF_BLOB_FEE_BASE")
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_basic(self):
        ...
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 0.4, places=4)
        self.assertEqual(r["classification"],
                         "MODERATE_FEE_ON_BLOB_FEE_GAP")
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_blob_fee_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertIsNone(r["net_of_blob_fee_yield_pct"])
        self.assertIsNone(r["blob_fee_consumed_yield_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_blob_fee_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_BLOB_FEE", r["flags"])
        self.assertNotIn("FULL_FEE_ON_BLOB_FEE", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_blob_fee_gap_pct=-4.8,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_blob_fee_gap_pct"], 4.8, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_blob_fee_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["fee_on_blob_fee_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_blob_fee_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_blob_fee_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_blob_fee_fraction"]
    ...
    def test_override_denominator_is_fee_charged(self):
        r = self.an.analyze(make_pos(
            ...
            fee_on_blob_fee_gap_pct=3.0,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_blob_fee_fraction"], 3.0 / 12.0,
                               places=4)

    def test_override_high_blob_fee_still_flagged(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=20.0,
            fee_on_blob_fee_gap_pct=3.0,
            fee_charged_pct=12.0,
            blob_fee_rate_pct=0.6))
        self.assertIn("HIGH_BLOB_FEE", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
    ...
    def test_override_requires_positive_fee_charged(self):
        ...
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=0.0,
            performance_fee_pct=20.0,
            net_of_blob_fee_yield_pct=20.0))
        self.assertFalse(r["used_override"])
        self.assertTrue(r["used_main"])
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "fee_on_blob_fee_gap_pct", "realization_ratio",
...
class TestFlags(unittest.TestCase):
    ...
    def test_classification_always_in_flags(self):
        for net in (15.0, 12.0, 8.0, 4.0):
            r = self.an.analyze(make_pos(
                gross_yield_pct=15.0,
                net_of_blob_fee_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertIn(r["classification"], r["flags"])

    def test_high_blob_fee_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_blob_fee_yield_pct=13.0,
            performance_fee_pct=20.0,
            blob_fee_rate_pct=HIGH_BLOB_FEE_PCT))
        self.assertIn("HIGH_BLOB_FEE", r["flags"])

    def test_no_high_blob_fee_flag_below_threshold(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=15.0,
            net_of_blob_fee_yield_pct=13.0,
            performance_fee_pct=20.0,
            blob_fee_rate_pct=0.2))
        self.assertNotIn("HIGH_BLOB_FEE", r["flags"])

    def test_full_fee_on_blob_fee_flag(self):
        r = self.an.analyze(make_pos(
            gross_yield_pct=10.0,
            net_of_blob_fee_yield_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_BLOB_FEE", r["flags"])
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_blob_fee_gap_vault", agg)
    ...
    def test_aggregate_single_position(self):
        ...
        self.assertEqual(agg["worst_blob_fee_gap_vault"], "Solo")
        ...
        for gross in (1.0, 5.0, 12.0, 30.0, 100.0):
            for net in (-5.0, 0.0, gross * 0.5, gross, gross + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    ...
                    self.assertGreaterEqual(r["fee_on_blob_fee_fraction"], 0.0)
                    self.assertLessEqual(r["fee_on_blob_fee_fraction"], 1.0)
                    self.assertGreaterEqual(r["realization_ratio"], 0.0)
                    self.assertLessEqual(r["realization_ratio"], 1.0)
                    self.assertTrue(_all_floats_finite(r))

    def test_fraction_monotone_with_blob_fee(self):
        ...
        for net in (20.0, 16.0, 12.0, 8.0, 4.0, 0.0):
            r = self.an.analyze(make_pos(
                ...
                net_of_blob_fee_yield_pct=net,
                performance_fee_pct=20.0))
            self.assertGreaterEqual(r["fee_on_blob_fee_fraction"], prev)
            prev = r["fee_on_blob_fee_fraction"]
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-Base-Vault-CleanBlobFee"],
            "CLEAN_NET_OF_BLOB_FEE_BASE")
        self.assertEqual(
            classes["CRV-Optimism-Vault-ModerateBlobFee"],
            "MODERATE_FEE_ON_BLOB_FEE_GAP")
        self.assertEqual(
            classes["BAL-Arbitrum-Vault-SevereBlobFeeSpike"],
            "SEVERE_FEE_ON_BLOB_FEE_GAP")
        self.assertEqual(
            classes["UNI-Base-Vault-OverrideBlobFeeGap"],
            "MODERATE_FEE_ON_BLOB_FEE_GAP")
        self.assertEqual(
