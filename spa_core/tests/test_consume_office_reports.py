...
def test_declared_schema_matches_the_live_producer(name: str) -> None:
    ...
    if live is None:
        # ОБЪЯВЛЕНО ОСОЗНАННЫМ, цикл #642 (заказ G47 п. 1 — «починить ИЛИ
        # объявить осознанным»). Зонд `absent_path_probe` назвал эту строку
        # слепой: унеси `data/` — тест остаётся зелёным. Чинить её пропуском
        # НЕЛЬЗЯ, и это не осторожность, а замер соседа: ровно `pytest.skip`
        # здесь стоял до цикла #248, и на CI тест из-за него не выполнялся
        # ВООБЩЕ (см. докстроку выше) — сильная первая половина, сверка схемы
        # с ИСХОДНИКОМ производителя, пропадала вместе со слабой второй.
        # Слепа тут не проверка, а её ДОКАЗАТЕЛЬНАЯ добавка: живой артефакт
        # есть доказательство, когда он есть, и молчание, когда его нет.
        # Вернуть скип значило бы обменять сторожа на вежливость.
        return
    try:
