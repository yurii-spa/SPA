...
# ИЗМЕНЕНО НАМЕРЕННО (инв. #16) — проверка УСИЛЕНА, ни одно утверждение не тронуто.
# Якорь `NOW` вычислялся ОДИН раз на импорте модуля, то есть на СБОРЕ набора, а субъект
# (`_duplicate_recently`) спрашивает настоящие часы в момент выполнения. В одиночном
# прогоне между ними доли секунды, в полном — полтора часа: запись «120 секунд назад»
# оказывается СТАРШЕ получасового окна дедупа, и тест краснеет от длительности прогона,
# а не от поведения кода. Это тот же класс, что «фиксированная дата — бомба»
# (`.claude/rules/deployment.md`), только фитиль другой: не календарь, а длительность
# набора. Храповик замороженных дат его не видит — литеральной даты здесь нет вовсе.
# Якорь связывается ПЕРЕД КАЖДЫМ тестом; одним мгновением на тест, а не на обращение,
# потому что текст дедупа несёт отметку в себе и два обращения дали бы РАЗНЫЙ текст.
@pytest.fixture(autouse=True)
def _anchor_now_at_assertion_time():
    global NOW
    NOW = now_utc()
