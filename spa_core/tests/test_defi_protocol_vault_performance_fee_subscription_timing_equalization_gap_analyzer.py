"""
Tests for MP-1210: DeFiProtocolVaultPerformanceFeeSubscriptionTimingEqualizationGapAnalyzer
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_subscription_timing_equalization_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_subscription_timing_equalization_gap_analyzer import (  # noqa: E501
...
def make_pos(
    ...
    pos = {"vault": vault}
    if full_period_gain_pct is not None:
        pos["full_period_gain_pct"] = full_period_gain_pct
    if post_entry_gain_pct is not None:
        pos["post_entry_gain_pct"] = post_entry_gain_pct
    if performance_fee_pct is not None:
        pos["performance_fee_pct"] = performance_fee_pct
    if entry_fraction_of_period is not None:
        pos["entry_fraction_of_period"] = entry_fraction_of_period
    if equalization_gap_pct is not None:
        pos["equalization_gap_pct"] = equalization_gap_pct
...
class TestHelpers(unittest.TestCase):
    ...
    def test_thresholds_ordered(self):
        ...
        self.assertGreater(LATE_SUBSCRIPTION_FRACTION, 0.0)
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_fully_equalized(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=18.0, post_entry_gain_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "CLEAN_FULLY_EQUALIZED")
        self.assertIn("CLEAN_FULL_EQUALIZATION", r["flags"])
        self.assertEqual(r["equalization_gap_pct"], 0.0)
        self.assertEqual(r["fee_on_pre_entry_fraction"], 0.0)
        self.assertGreaterEqual(r["score"], 85.0)
        self.assertEqual(r["grade"], "A")
        self.assertEqual(r["recommendation"], "TRUST_FEE_STRUCTURE")

    def test_mild_equalization_gap(self):
        # full 20, post 18 → pre_entry 2, fee_charged=4, fair_fee=3.6,
        # gap=0.4, fraction=0.1 → MILD.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=20.0, post_entry_gain_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MILD_EQUALIZATION_GAP")
        self.assertAlmostEqual(r["fee_on_pre_entry_fraction"], 0.1, places=4)
        self.assertEqual(r["recommendation"], "MINOR_EQUALIZATION_GAP")

    def test_moderate_equalization_gap(self):
        # full 16, post 8 → pre_entry 8, fee_charged=3.2, fair_fee=1.6,
        # gap=1.6, fraction=0.5 → MODERATE.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MODERATE_EQUALIZATION_GAP")
        self.assertAlmostEqual(r["fee_on_pre_entry_fraction"], 0.5, places=4)
        self.assertIn("FEE_ON_PRE_ENTRY_GAINS", r["flags"])
        self.assertEqual(r["recommendation"], "DEMAND_EQUALIZATION_ACCOUNTING")

    def test_severe_equalization_gap_high_fraction(self):
        # full 20, post 2 → pre_entry 18, fraction = 18/20 = 0.9 → SEVERE.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=20.0, post_entry_gain_pct=2.0,
            performance_fee_pct=10.0))
        self.assertEqual(r["classification"], "SEVERE_EQUALIZATION_GAP")
        self.assertGreater(r["fee_on_pre_entry_fraction"], MODERATE_FRACTION)
        self.assertEqual(r["recommendation"], "AVOID_NO_EQUALIZATION")

    def test_severe_net_negative(self):
        # subscriber entered above the high → post-entry negative → fair net
        # return is negative → SEVERE.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=12.0, post_entry_gain_pct=-3.0,
            performance_fee_pct=50.0))
        self.assertEqual(r["classification"], "SEVERE_EQUALIZATION_GAP")
        self.assertTrue(r["net_is_negative"])
        self.assertIn("NET_NEGATIVE_AFTER_FEE", r["flags"])

    def test_full_pre_entry_flag(self):
        # post 0 with positive full → entered at the peak, full pre-entry.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=10.0, post_entry_gain_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_PRE_ENTRY", r["flags"])

    def test_post_default_when_missing(self):
        # no post_entry_gain_pct supplied → treated as 0.0 (entered at peak).
        r = self.an.analyze(make_pos(
            full_period_gain_pct=10.0, performance_fee_pct=20.0))
        self.assertEqual(r["post_entry_gain_pct"], 0.0)
        self.assertIn("FULL_PRE_ENTRY", r["flags"])

    def test_post_negative_input(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=10.0, post_entry_gain_pct=-5.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["post_entry_gain_pct"], -5.0)
        self.assertTrue(r["net_is_negative"])
        self.assertIn("FULL_PRE_ENTRY", r["flags"])
...
class TestMath(unittest.TestCase):
    ...
    def test_metric_geometry(self):
        r = self.an.analyze(make_pos(
            ...
            performance_fee_pct=20.0))
        # fee_frac = 0.2
        self.assertEqual(r["pre_entry_gain_pct"], 8.0)     # 16 - 8
        self.assertAlmostEqual(r["fee_charged_pct"], 3.2)  # 0.2*16
        self.assertAlmostEqual(r["fair_fee_pct"], 1.6)     # 0.2*8
        self.assertAlmostEqual(r["equalization_gap_pct"], 1.6)  # 3.2-1.6
    ...
    def test_fee_rate_zero(self):
        ...
        r = self.an.analyze(make_pos(
            full_period_gain_pct=20.0, post_entry_gain_pct=5.0,
    ...
    def test_post_exceeds_full_pre_entry_clamped_zero(self):
        # post > full → pre_entry clamps to 0, no gap, clean.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=10.0, post_entry_gain_pct=14.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["pre_entry_gain_pct"], 0.0)
        self.assertEqual(r["equalization_gap_pct"], 0.0)
        self.assertEqual(r["classification"], "CLEAN_FULLY_EQUALIZED")
        self.assertNotIn("FEE_ON_PRE_ENTRY_GAINS", r["flags"])
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=24.0, equalization_gap_pct=5.0,
            fee_charged_pct=12.0))
        self.assertIsNone(r["post_entry_gain_pct"])
        self.assertIsNone(r["pre_entry_gain_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            ...
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_PRE_ENTRY_GAINS", r["flags"])
        self.assertNotIn("FULL_PRE_ENTRY", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=24.0, equalization_gap_pct=-5.0,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["equalization_gap_pct"], 5.0, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        # gap supplied larger than fee_charged → capped → fraction = 1.0.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=24.0, equalization_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["equalization_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_pre_entry_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=24.0, equalization_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_pre_entry_fraction"]
        self.assertAlmostEqual(r["realization_ratio"], 1.0 - frac, places=4)

    def test_override_with_late_subscription_flag(self):
        # override path still honours the informational entry_fraction flag.
        r = self.an.analyze(make_pos(
            full_period_gain_pct=24.0, equalization_gap_pct=3.0,
            fee_charged_pct=12.0, entry_fraction_of_period=0.8))
        self.assertIn("LATE_SUBSCRIPTION", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_zero_full(self):
        r = self.an.analyze(make_pos(full_period_gain_pct=0.0,
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_negative_full(self):
        r = self.an.analyze(make_pos(full_period_gain_pct=-5.0,
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_nan_full(self):
        r = self.an.analyze(make_pos(full_period_gain_pct=float("nan"),
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_inf_full(self):
        r = self.an.analyze(make_pos(full_period_gain_pct=float("inf"),
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_missing_fee_main_path(self):
        # full present, no fee, no override → INSUFFICIENT_DATA.
        r = self.an.analyze(make_pos(full_period_gain_pct=10.0,
                                     post_entry_gain_pct=5.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_invalid_fee_main_path(self):
        # full present, non-finite fee, no override → INSUFFICIENT_DATA.
        r = self.an.analyze(make_pos(full_period_gain_pct=10.0,
                                     post_entry_gain_pct=5.0,
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "equalization_gap_pct", "realization_ratio"):
...
class TestFlags(unittest.TestCase):
    ...
    def test_late_subscription_flag(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0, entry_fraction_of_period=0.5))
        self.assertIn("LATE_SUBSCRIPTION", r["flags"])

    def test_late_subscription_flag_high(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0, entry_fraction_of_period=0.9))
        self.assertIn("LATE_SUBSCRIPTION", r["flags"])

    def test_early_subscription_no_flag(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0, entry_fraction_of_period=0.2))
        self.assertNotIn("LATE_SUBSCRIPTION", r["flags"])

    def test_no_entry_fraction_no_flag(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0))
        self.assertNotIn("LATE_SUBSCRIPTION", r["flags"])
        self.assertIsNone(r["entry_fraction_of_period"])

    def test_clean_full_equalization_flag(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=10.0, post_entry_gain_pct=10.0,
            performance_fee_pct=20.0))
        self.assertIn("CLEAN_FULL_EQUALIZATION", r["flags"])

    def test_fee_on_pre_entry_flag(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct=16.0, post_entry_gain_pct=8.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_PRE_ENTRY_GAINS", r["flags"])
    ...
    def test_classification_always_in_flags(self):
        for post in (18.0, 8.0, 1.0, -5.0):
            r = self.an.analyze(make_pos(
                full_period_gain_pct=18.0, post_entry_gain_pct=post,
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_equalization_vault", agg)

    def test_aggregate_cleanest_and_worst(self):
        positions = [
            make_pos(vault="Clean", full_period_gain_pct=10.0,
                     post_entry_gain_pct=10.0, performance_fee_pct=20.0),
            make_pos(vault="Bad", full_period_gain_pct=12.0,
                     post_entry_gain_pct=-3.0, performance_fee_pct=50.0),
        ]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertEqual(agg["cleanest_vault"], "Clean")
        self.assertEqual(agg["worst_equalization_vault"], "Bad")

    def test_aggregate_all_insufficient(self):
        positions = [make_pos(vault="X"), make_pos(vault="Y")]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertIsNone(agg["cleanest_vault"])
        self.assertIsNone(agg["worst_equalization_vault"])
        self.assertEqual(agg["avg_score"], 0.0)
        self.assertEqual(agg["position_count"], 2)

    def test_aggregate_net_negative_count(self):
        positions = [
            make_pos(vault="A", full_period_gain_pct=12.0,
                     post_entry_gain_pct=-3.0, performance_fee_pct=50.0),
            make_pos(vault="B", full_period_gain_pct=10.0,
                     post_entry_gain_pct=10.0, performance_fee_pct=20.0),
        ]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
...
class TestRingBufferLog(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for _ in range(5):
                self.an.analyze_portfolio(
                    [make_pos(full_period_gain_pct=10.0,
    ...
    def test_no_tmp_left(self):
        with tempfile.TemporaryDirectory() as d:
            log_path = os.path.join(d, "log.json")
            cfg = {"log_path": log_path, "log_cap": 3}
            self.an.analyze(make_pos(full_period_gain_pct=10.0,
                                     post_entry_gain_pct=8.0,
                                     performance_fee_pct=20.0),
                            cfg=cfg, write_log=True)
...
class TestInvariants(unittest.TestCase):
    ...
    def test_score_bounds_random_grid(self):
        for full in (1.0, 5.0, 12.0, 30.0, 100.0):
            for post in (-10.0, 0.0, 3.0, full, full + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    r = self.an.analyze(make_pos(
                        full_period_gain_pct=full, post_entry_gain_pct=post,
                        performance_fee_pct=fee))
                    self.assertGreaterEqual(r["score"], 0.0)
                    self.assertLessEqual(r["score"], 100.0)
                    self.assertGreaterEqual(r["fee_on_pre_entry_fraction"], 0.0)
                    self.assertLessEqual(r["fee_on_pre_entry_fraction"], 1.0)
    ...
    def test_fraction_monotone_with_pre_entry(self):
        # holding full & fee fixed, lower post → larger pre_entry → larger
        # fee_on_pre_entry_fraction (monotone non-decreasing).
        prev = -1.0
        for post in (18.0, 12.0, 6.0, 0.0, -6.0):
            r = self.an.analyze(make_pos(
                full_period_gain_pct=18.0, post_entry_gain_pct=post,
                performance_fee_pct=20.0))
            self.assertGreaterEqual(r["fee_on_pre_entry_fraction"], prev)
            prev = r["fee_on_pre_entry_fraction"]

    def test_token_fallback(self):
        r = self.an.analyze({"token": "TKN", "full_period_gain_pct": 10.0,
                             "post_entry_gain_pct": 10.0,
                             "performance_fee_pct": 20.0})
        self.assertEqual(r["token"], "TKN")

    def test_unknown_token(self):
        r = self.an.analyze({"full_period_gain_pct": 10.0,
                             "post_entry_gain_pct": 10.0,
                             "performance_fee_pct": 20.0})
    ...
    def test_string_numeric_inputs(self):
        r = self.an.analyze(make_pos(
            full_period_gain_pct="16", post_entry_gain_pct="8",
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-Vault-CleanEqualized"], "CLEAN_FULLY_EQUALIZED")
        self.assertEqual(
            classes["stETH-Vault-ModerateGap"], "MODERATE_EQUALIZATION_GAP")
        self.assertEqual(
            classes["GOV-Vault-SevereGap"], "SEVERE_EQUALIZATION_GAP")
        self.assertEqual(
            classes["LST-Vault-OverrideGap"], "MODERATE_EQUALIZATION_GAP")
        self.assertEqual(
