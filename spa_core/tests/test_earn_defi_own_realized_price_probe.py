...
Дат-литералов нет намеренно: все отметки считаются от одних часов, которые тест сам же
и передаёт пробе (`now=`), поэтому календарь вердикт не двигает.
"""
...
def _build(tmp_path, *, own=True, own_days=DAYS, skew=None, copy=False, drop_own=(),
           ...
           with_signal=True, with_ref=True, with_tables=True):
    ...
    if with_tables:
        ...
        rows, own_series = [], {}
        for i in range(DAYS):
            d = _day(DAYS - i)  # самый свежий день — «вчера»
        ...
        if with_signal:
            sd = max(own_series) if own_series else _day(1)
...
def test_registered_and_reachable_through_the_registry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("EARN_DEFI_ROOT", _build(tmp_path))
