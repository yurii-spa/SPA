"""
Tests for MP-1213: DeFiProtocolVaultPerformanceFeeGrossOfCostBaseGapAnalyzer
Run: python3 -m unittest spa_core.tests.test_defi_protocol_vault_performance_fee_gross_of_cost_base_gap_analyzer -v
"""
...
from spa_core.analytics.defi_protocol_vault_performance_fee_gross_of_cost_base_gap_analyzer import (  # noqa: E501
...
def make_pos(
    ...
    if gross_profit_pct is not None:
        pos["gross_profit_pct"] = gross_profit_pct
    if net_of_cost_profit_pct is not None:
        pos["net_of_cost_profit_pct"] = net_of_cost_profit_pct
    if performance_fee_pct is not None:
        pos["performance_fee_pct"] = performance_fee_pct
    if harvest_cost_pct is not None:
        pos["harvest_cost_pct"] = harvest_cost_pct
    if fee_on_cost_gap_pct is not None:
        pos["fee_on_cost_gap_pct"] = fee_on_cost_gap_pct
...
class TestHelpers(unittest.TestCase):
    ...
    def test_thresholds_ordered(self):
        ...
        self.assertGreater(HIGH_HARVEST_COST_PCT, 0.0)

    def test_high_harvest_cost_value(self):
        self.assertEqual(HIGH_HARVEST_COST_PCT, 5.0)

    def test_log_path_filename(self):
        self.assertTrue(LOG_PATH.endswith(
            "vault_performance_fee_gross_of_cost_base_gap_log.json"))
...
class TestMainPathClassification(unittest.TestCase):
    ...
    def test_clean_net_of_cost_base(self):
        # net_of_cost == gross → harvest cost consumed nothing → CLEAN.
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, net_of_cost_profit_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "CLEAN_NET_OF_COST_BASE")
        self.assertIn("CLEAN_NET_BASE", r["flags"])
        self.assertLessEqual(r["fee_on_cost_fraction"], CLEAN_FRACTION)
    ...
    def test_clean_default_net_zero_is_full(self):
        # no net_of_cost supplied → default 0 → whole profit consumed → FULL.
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, performance_fee_pct=20.0))
        self.assertEqual(r["net_of_cost_profit_pct"], 0.0)
        self.assertIn("FULL_FEE_ON_COST", r["flags"])

    def test_mild_fee_on_cost_gap(self):
        # gross 20, net 18 → fee_on_cost = 0.1 → MILD.
        r = self.an.analyze(make_pos(
            gross_profit_pct=20.0, net_of_cost_profit_pct=18.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MILD_FEE_ON_COST_GAP")
        self.assertEqual(r["recommendation"], "MINOR_FEE_ON_COST")
        self.assertGreater(r["fee_on_cost_fraction"], CLEAN_FRACTION)
        self.assertLessEqual(r["fee_on_cost_fraction"], MILD_FRACTION)

    def test_moderate_fee_on_cost_gap(self):
        # gross 16, net 8 → fee_on_cost = 0.5 → MODERATE (at boundary).
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "MODERATE_FEE_ON_COST_GAP")
        self.assertAlmostEqual(r["fee_on_cost_fraction"], 0.5, places=4)
        self.assertIn("FEE_ON_HARVEST_COST", r["flags"])
        self.assertEqual(r["recommendation"], "DEMAND_NET_OF_COST_BASE")

    def test_severe_fee_on_cost_gap(self):
        # gross 20, net 4 → fee_on_cost = 0.8 → SEVERE.
        r = self.an.analyze(make_pos(
            gross_profit_pct=20.0, net_of_cost_profit_pct=4.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "SEVERE_FEE_ON_COST_GAP")
        self.assertGreater(r["fee_on_cost_fraction"], MODERATE_FRACTION)
        self.assertEqual(r["recommendation"], "AVOID_FEE_ON_COST")

    def test_severe_net_negative(self):
        # harvest cost drove net negative, fee on gross → net_fair negative.
        r = self.an.analyze(make_pos(
            gross_profit_pct=12.0, net_of_cost_profit_pct=-3.0,
    ...
    def test_fee_on_harvest_cost_flag(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0))
        self.assertIn("FEE_ON_HARVEST_COST", r["flags"])

    def test_full_fee_on_cost_flag(self):
        # net_of_cost <= 0 with positive gross → FULL_FEE_ON_COST.
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=0.0,
            performance_fee_pct=20.0))
        self.assertIn("FULL_FEE_ON_COST", r["flags"])

    def test_high_harvest_cost_flag(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0, harvest_cost_pct=6.0))
        self.assertIn("HIGH_HARVEST_COST", r["flags"])

    def test_low_harvest_cost_no_flag(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, net_of_cost_profit_pct=18.0,
            performance_fee_pct=20.0, harvest_cost_pct=1.0))
        self.assertNotIn("HIGH_HARVEST_COST", r["flags"])

    def test_high_harvest_cost_at_threshold(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0, harvest_cost_pct=5.0))
        self.assertIn("HIGH_HARVEST_COST", r["flags"])

    def test_harvest_cost_echoed(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0, harvest_cost_pct=3.5))
        self.assertEqual(r["harvest_cost_pct"], 3.5)
...
class TestMath(unittest.TestCase):
    ...
    def test_moderate_geometry(self):
        ...
        # fee_charged=0.2*16=3.2; fair=0.2*8=1.6; gap=1.6; cost_consumed=8.
        # fee_on_cost=1.6/3.2=0.5.
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_charged_pct"], 3.2)
        self.assertAlmostEqual(r["fair_fee_pct"], 1.6)
        self.assertAlmostEqual(r["fee_on_cost_gap_pct"], 1.6)
        self.assertAlmostEqual(r["cost_consumed_profit_pct"], 8.0)
        self.assertAlmostEqual(r["fee_on_cost_fraction"], 0.5, places=4)
    ...
    def test_clean_geometry_zero_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, net_of_cost_profit_pct=18.0,
            performance_fee_pct=20.0))
        self.assertAlmostEqual(r["fee_on_cost_gap_pct"], 0.0)
        self.assertAlmostEqual(r["cost_consumed_profit_pct"], 0.0)
        self.assertEqual(r["fee_on_cost_fraction"], 0.0)
    ...
    def test_fee_rate_negative_clamped(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, net_of_cost_profit_pct=10.0,
            performance_fee_pct=-20.0))
        self.assertEqual(r["performance_fee_pct"], 0.0)
        self.assertEqual(r["fee_charged_pct"], 0.0)
        self.assertEqual(r["fee_on_cost_gap_pct"], 0.0)

    def test_fee_rate_zero(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=20.0, net_of_cost_profit_pct=10.0,
    ...
    def test_net_above_gross_no_gap(self):
        ...
        r = self.an.analyze(make_pos(
            gross_profit_pct=10.0, net_of_cost_profit_pct=12.0,
            performance_fee_pct=20.0))
        self.assertEqual(r["fee_on_cost_gap_pct"], 0.0)
        self.assertEqual(r["cost_consumed_profit_pct"], 0.0)
        self.assertEqual(r["classification"], "CLEAN_NET_OF_COST_BASE")
    ...
    def test_gap_bounded_by_fee_charged(self):
        for net in (0.0, 2.0, 5.0, 9.9):
            r = self.an.analyze(make_pos(
                gross_profit_pct=10.0, net_of_cost_profit_pct=net,
                performance_fee_pct=20.0))
            self.assertLessEqual(r["fee_on_cost_gap_pct"],
                                 r["fee_charged_pct"] + 1e-9)
            self.assertLessEqual(r["fee_on_cost_fraction"], 1.0)
            self.assertGreaterEqual(r["fee_on_cost_fraction"], 0.0)

    def test_string_numeric_inputs(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct="16", net_of_cost_profit_pct="8",
...
class TestOverridePath(unittest.TestCase):
    ...
    def test_override_geometry_none(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertIsNone(r["net_of_cost_profit_pct"])
        self.assertIsNone(r["cost_consumed_profit_pct"])
    ...
    def test_override_geometry_flags_suppressed(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=4.8,
            fee_charged_pct=12.0))
        self.assertNotIn("FEE_ON_HARVEST_COST", r["flags"])
        self.assertNotIn("FULL_FEE_ON_COST", r["flags"])
    ...
    def test_override_negative_gap_magnitude(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=-4.8,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_cost_gap_pct"], 4.8, places=4)

    def test_override_gap_capped_at_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=99.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["fee_on_cost_gap_pct"], 12.0)
        self.assertEqual(r["fee_on_cost_fraction"], 1.0)
        self.assertEqual(r["fair_fee_pct"], 0.0)

    def test_override_realization_anchor(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=3.0,
            fee_charged_pct=12.0))
        frac = r["fee_on_cost_fraction"]
    ...
    def test_override_denominator_is_fee_charged(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=3.0,
            fee_charged_pct=12.0))
        self.assertAlmostEqual(r["fee_on_cost_fraction"], 3.0 / 12.0,
                               places=4)

    def test_override_high_harvest_cost_still_flagged(self):
        # HIGH_HARVEST_COST is not geometry-only → still raised on override.
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=3.0,
            fee_charged_pct=12.0, harvest_cost_pct=8.0))
        self.assertIn("HIGH_HARVEST_COST", r["flags"])
        self.assertIn("GAP_FROM_OVERRIDE", r["flags"])
    ...
    def test_override_severe_when_large_gap(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=24.0, fee_on_cost_gap_pct=10.0,
            fee_charged_pct=12.0))
        self.assertEqual(r["classification"], "SEVERE_FEE_ON_COST_GAP")
...
class TestInsufficientData(unittest.TestCase):
    ...
    def test_zero_gross(self):
        r = self.an.analyze(make_pos(gross_profit_pct=0.0,
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_negative_gross(self):
        r = self.an.analyze(make_pos(gross_profit_pct=-5.0,
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_nan_gross(self):
        r = self.an.analyze(make_pos(gross_profit_pct=float("nan"),
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_inf_gross(self):
        r = self.an.analyze(make_pos(gross_profit_pct=float("inf"),
                                     performance_fee_pct=20.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")

    def test_missing_fee_main_path(self):
        r = self.an.analyze(make_pos(gross_profit_pct=10.0,
                                     net_of_cost_profit_pct=5.0))
        self.assertEqual(r["classification"], "INSUFFICIENT_DATA")
    ...
    def test_insufficient_all_none(self):
        ...
        for k in ("net_return_after_fee_pct", "fee_charged_pct",
                  "fee_on_cost_gap_pct", "realization_ratio",
                  "fee_on_cost_fraction", "cost_consumed_profit_pct"):
...
class TestFlags(unittest.TestCase):
    ...
    def test_classification_always_in_flags(self):
        for net in (18.0, 16.0, 8.0, 4.0):
            r = self.an.analyze(make_pos(
                gross_profit_pct=18.0, net_of_cost_profit_pct=net,
                performance_fee_pct=20.0))
    ...
    def test_no_full_fee_on_cost_when_positive_net(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
            performance_fee_pct=20.0))
        self.assertNotIn("FULL_FEE_ON_COST", r["flags"])

    def test_no_fee_on_harvest_cost_when_clean(self):
        r = self.an.analyze(make_pos(
            gross_profit_pct=18.0, net_of_cost_profit_pct=18.0,
            performance_fee_pct=20.0))
        self.assertNotIn("FEE_ON_HARVEST_COST", r["flags"])
...
class TestAggregate(unittest.TestCase):
    ...
    def test_portfolio_structure(self):
        ...
        self.assertIn("worst_fee_on_cost_vault", agg)

    def test_aggregate_cleanest_and_worst(self):
        positions = [
            make_pos(vault="Clean", gross_profit_pct=18.0,
                     net_of_cost_profit_pct=18.0, performance_fee_pct=20.0),
            make_pos(vault="Bad", gross_profit_pct=12.0,
                     net_of_cost_profit_pct=-3.0, performance_fee_pct=50.0),
        ]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertEqual(agg["cleanest_vault"], "Clean")
        self.assertEqual(agg["worst_fee_on_cost_vault"], "Bad")

    def test_aggregate_all_insufficient(self):
        positions = [make_pos(vault="X"), make_pos(vault="Y")]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
        self.assertIsNone(agg["cleanest_vault"])
        self.assertIsNone(agg["worst_fee_on_cost_vault"])
        self.assertEqual(agg["avg_score"], 0.0)
        self.assertEqual(agg["position_count"], 2)

    def test_aggregate_net_negative_count(self):
        positions = [
            make_pos(vault="A", gross_profit_pct=18.0,
                     net_of_cost_profit_pct=18.0, performance_fee_pct=20.0),
            make_pos(vault="B", gross_profit_pct=12.0,
                     net_of_cost_profit_pct=-3.0, performance_fee_pct=50.0),
        ]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
    ...
    def test_aggregate_avg_score(self):
        positions = [
            make_pos(vault="A", gross_profit_pct=18.0,
                     net_of_cost_profit_pct=18.0, performance_fee_pct=20.0),
            make_pos(vault="B", gross_profit_pct=18.0,
                     net_of_cost_profit_pct=18.0, performance_fee_pct=20.0),
        ]
        agg = self.an.analyze_portfolio(positions)["aggregate"]
...
class TestRingBufferLog(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for _ in range(5):
                self.an.analyze_portfolio(
                    [make_pos(gross_profit_pct=16.0,
    ...
    def test_log_corrupt_recovers(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            cfg = {"log_path": log_path, "log_cap": 3}
            self.an.analyze_portfolio(
                [make_pos(gross_profit_pct=16.0, net_of_cost_profit_pct=8.0,
                          performance_fee_pct=20.0)],
                cfg=cfg, write_log=True)
            with open(log_path) as fh:
                data = json.load(fh)
    ...
    def test_no_tmp_left(self):
        with tempfile.TemporaryDirectory() as d:
            log_path = os.path.join(d, "log.json")
            cfg = {"log_path": log_path, "log_cap": 3}
            self.an.analyze(make_pos(gross_profit_pct=16.0,
                                     net_of_cost_profit_pct=8.0,
                                     performance_fee_pct=20.0),
                            cfg=cfg, write_log=True)
...
class TestInvariants(unittest.TestCase):
        ...
        for gross in (1.0, 5.0, 12.0, 30.0, 100.0):
            for net in (-5.0, 0.0, gross * 0.5, gross, gross + 5.0):
                for fee in (0.0, 10.0, 20.0, 50.0, 100.0):
                    r = self.an.analyze(make_pos(
                        gross_profit_pct=gross, net_of_cost_profit_pct=net,
    ...
    def test_fraction_monotone_with_cost(self):
        # holding gross/fee fixed, lower net (more cost) → larger (or equal)
        # fee_on_cost fraction (monotone non-decreasing).
        prev = -1.0
        for net in (20.0, 16.0, 12.0, 8.0, 4.0, 0.0):
            r = self.an.analyze(make_pos(
                gross_profit_pct=20.0, net_of_cost_profit_pct=net,
                performance_fee_pct=20.0))
            self.assertGreaterEqual(r["fee_on_cost_fraction"], prev)
            prev = r["fee_on_cost_fraction"]

    def test_token_fallback(self):
        r = self.an.analyze({"token": "TKN", "gross_profit_pct": 18.0,
                             "net_of_cost_profit_pct": 18.0,
                             "performance_fee_pct": 20.0})
        self.assertEqual(r["token"], "TKN")

    def test_unknown_token(self):
        r = self.an.analyze({"gross_profit_pct": 18.0,
                             "net_of_cost_profit_pct": 18.0,
                             "performance_fee_pct": 20.0})
...
class TestDemo(unittest.TestCase):
    ...
    def test_demo_classifications(self):
        ...
        self.assertEqual(
            classes["USDC-Vault-CleanNetBase"], "CLEAN_NET_OF_COST_BASE")
        self.assertEqual(
            classes["stETH-Vault-ModerateFeeOnCost"],
            "MODERATE_FEE_ON_COST_GAP")
        self.assertEqual(
            classes["GOV-Vault-SevereFeeOnCost"], "SEVERE_FEE_ON_COST_GAP")
        self.assertEqual(
            classes["LST-Vault-OverrideGap"], "MODERATE_FEE_ON_COST_GAP")
        self.assertEqual(
