...
#: Тиры, которые система ЗНАЕТ. Значение вне набора — ОТКАЗ, а не потолок по
#: умолчанию (решение владельца 12.09, ADR-357 п. 3, исполнено ADR-359). Набор
#: ОБЪЯВЛЕН, а не выведен из веток: ветка `else` по построению не может сказать,
#: какие тиры существуют, — она говорит только «не T1».
KNOWN_TIERS = frozenset({"T1", "T2", "T3"})


def tier_from_registry(raw: object) -> "str | None":
    """Тир из записи реестра в канонической форме, либо ``None`` — НЕ ЗНАЕМ.

    **Зачем одна функция.** Одно и то же поле разбирали ТРИ места и два из них
    спорили (замер 12.09):

    * ``adapters/apy_aggregator`` — верно: ``f"T{n}"`` для числа, строка как есть;
    * ``allocator`` и ``strategy_lab/realized_ab`` — ``"T1" if tier == 1 else "T2"``,
      то есть ВСЁ, кроме единицы, объявлялось вторым тиром.

    Цена расхождения измерена на живом реестре: 4 записи с тиром 3 уходили в гейт
    как ``T2`` (суммарный потолок T3 = 15 %, ADR-020/337, не связывал по построению),
    а ``sky_susds`` хранит тир СТРОКОЙ ``"T1"`` — сравнение ``"T1" == 1`` ложно, и
    протокол, поднятый владельцем в первый тир (ADR-065, инвариант #10), приходил
    на гейт вторым. Ошибка в обе стороны сразу: одна строже правды, другая мягче.

    ``None`` возвращается для всего, чего мы не знаем, — и это ОТКАЗ, а не повод
    подставить второй тир (решение владельца 12.09, ADR-357 п. 3). Ноль и
    отрицательные тоже неизвестны: ``tier=0`` в реестре означает «не размещаем»,
    и молча превращать его во второй тир значило бы финансировать запрещённое.
    """
    if isinstance(raw, bool):          # bool — подкласс int, и тиром не бывает
        return None
    if isinstance(raw, int):
        ...
        return None
    return cand if cand in KNOWN_TIERS else None
...
class RiskPolicy:
    ...
    def check_new_position(
        ...
        #
        # СРАВНЕНИЕ В ЦЕНТАХ, А НЕ В ДОЛЯХ (замер 29.08). Порог НЕ изменён — изменена
        # только точность сравнения, и вот почему это оказалось нужно.
        #
        # Трое суток подряд перераздача ADR-072 отклонялась с «cash buffer 5.0% <
        # minimum 5.0%», и 33.7 % капитала стояли без работы (дневная доходность
        # 4.21 % → 3.17 %). Утверждение ложно как арифметика: 5.0 не меньше 5.0.
        # Воспроизвести отказ удалось только повторив СПОСОБ СЧЁТА гейта. `cash_usd`
        # это `капитал − sum(позиции)`, то есть на каждой ноге остаток получается как
        # `капитал − sum(предыдущие) − сумма`, а не последовательным вычитанием. На
        # реальных ногах (доли вида 22105.260000000006) это дало
        #
        #     остаток 4999.999999999996  →  доля 0.04999999999999996  <  0.05
        #
        # то есть план, ложащийся РОВНО на обязательные 5 %, отвергался из-за ошибки
        # в последних битах — ценой трети капитала каждый цикл. Последовательное
        # вычитание тех же чисел даёт ровно 5000.0.
        #
        # Деньги измеряются в центах, и порог — тоже. Полцента ниже буфера это НЕ
        # нарушение буфера. Настоящее нарушение (на цент и больше) по-прежнему
        # отвергается: правило не ослаблено, оно перестало срабатывать на шуме.
        ...
        # 5b. НЕЗНАКОМЫЙ ТИР — ОТКАЗ, а не потолок T2 (решение владельца 12.09,
        #     ADR-357 п. 3; исполнено ADR-359).
        #
        #     Ниже потолок выбирается веткой `T1 if tier == "T1" else T2`, то есть
        #     ЛЮБОЕ иное значение — `T9`, пустая строка, опечатка регистра — молча
        #     получало лимит T2 и финансировалось. Замер ADR-250: тир `T9` на той же
        #     книге давал ровно тот же вердикт, что `T1`. Это было единственное место
        #     в системе, где сомнение трактовалось В ПОЛЬЗУ сделки; инвариант #2
        #     («сомневаешься — откажи») здесь не действовал.
        #
        #     Набор ОБЪЯВЛЕН, а не выведен из веток: ветка `else` по построению не
        #     может сказать, какие тиры существуют, — она может сказать только «не T1».
        #     Порогов не тронуто ни одного, версия RiskPolicy остаётся v1.0
        #     (инвариант #1): добавлен ОТКАЗ, а не изменено правило допуска.
        if tier not in KNOWN_TIERS:
            violations.append(
                f"Unknown tier {tier!r}: not one of {sorted(KNOWN_TIERS)} — "
                f"не знаем, не финансируем (ADR-357)"
            )
