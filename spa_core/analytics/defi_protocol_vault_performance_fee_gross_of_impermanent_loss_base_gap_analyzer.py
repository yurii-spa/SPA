...
An AMM / LP vault earns a GROSS yield (trading fees + reward emissions), but the
LP position simultaneously accrues IMPERMANENT (divergence) LOSS versus simply
HODLing the underlying tokens — a loss the depositor bears but does NOT see as
spendable yield this period. The depositor's economically realized yield is the
yield NET OF the impermanent loss (gross_yield − impermanent_loss). The vault
charges its PERFORMANCE fee on the GROSS yield (BEFORE netting the impermanent
loss), not on the NET-OF-IL yield the depositor economically realized. So the
depositor pays a performance fee on the very slice of yield that impermanent
loss already erased — a "fee-on-IL" / fee-base inflation. The fee is levied on
the gross yield; the FAIR fee would be levied only on the net-of-IL yield:

    fee_frac                          = clamp(performance_fee_pct / 100, 0, 1)
    il_consumed_yield_pct             = max(0, gross_yield - net_of_il_yield)
    fee_charged_pct                   = fee_frac * max(0, gross_yield)
    fair_fee_pct                      = fee_frac * max(0, net_of_il_yield)
    fee_on_il_gap_pct                 = max(0, fee_charged - fair_fee)
                                      (= performance fee charged on the
                                       impermanent-loss slice of the yield,
    ...
    overstatement_pct                 = fee_on_il_gap_pct
    fee_on_il_fraction                = clamp(gap / fee_charged, 0, 1)
...
pre-IL yield while the depositor only ever realized the net-of-IL slice — so a
chunk of the performance fee landed on the impermanent-loss slice the position
already gave back. The scale-free fee_on_il_fraction is the share of the charged
performance fee that landed on the impermanent-loss slice; it is the basis of
the classification. When the impermanent loss consumed nothing (net_of_il ≈
gross) there was no IL slice and the performance fee was fair (HIGHER score).
When the impermanent loss consumed most of the yield (net_of_il ≈ 0 or the net
return goes negative after the fee), the performance fee was charged almost
entirely on the IL slice (LOWER score).

HIGHER score = the performance fee was charged on the net-of-IL base
(gross ≈ net_of_il), the fee was effectively fair, nothing to fix.
LOWER score = a large share of the performance fee landed on the
impermanent-loss slice, or the net return goes negative after the fee.

Override path (when fee_on_il_gap_pct is supplied directly, finite, AND a valid
POSITIVE gross_yield_pct and POSITIVE fee_charged_pct are present): take the gap
verbatim (negative → magnitude) and skip the net-of-IL / impermanent-loss
geometry — fee_on_il_fraction and the metrics are computed the same way:

    fee_on_il_fraction = clamp(gap / fee_charged_pct, 0, 1)

(On the override path the net-of-IL / IL-slice / fair geometry is not known →
those fields are reported as None, and the geometry-only flags
FEE_ON_IMPERMANENT_LOSS / FULL_FEE_ON_IL / NET_NEGATIVE_AFTER_FEE are NOT
raised; realization_ratio is anchored to (1 - fee_on_il_fraction).)

Distinct from (this is the GROSS-OF-IMPERMANENT-LOSS performance-fee BASE — the
fee being charged on the gross yield before the LP's impermanent loss is netted
out, not the IL itself nor an external/internal fee layer):
  * the other gross_of_* perf-fee modules
    ...
    slippage, an internal reserve buffer, an external protocol's revenue-share
    cut, the vault's own management-fee layer). NONE of those layers is
    impermanent loss; the IL slice is the divergence loss the LP position bears
    versus HODL, a distinct axis.
  * the impermanent_loss_* modules (impermanent_loss_calculator,
    defi_impermanent_loss_breakeven_analyzer,
    defi_impermanent_loss_hedging_analyzer,
    defi_amm_impermanent_loss_simulator, etc.) — those MEASURE / FORECAST / HEDGE
    the impermanent loss ITSELF (its size, breakeven, hedge, simulated path).
    HERE the axis is the performance-fee BASE inflation from charging the fee on
    the gross (pre-IL) yield, NOT the magnitude or path of the IL.
  * net_of_loss_yield_realization — that nets a general realized-loss stream
    ...
    SINGLE period: the static gap between the gross base and the net-of-IL base.
  * defi_protocol_vault_performance_fee_high_water_mark_analyzer — that prices
    ...
    the static gap between the GROSS base and the NET-OF-IL base for a SINGLE fee
    period.
  * defi_protocol_vault_performance_fee_volatility_tax_analyzer — that prices the
    ...
    it is the static gap between the gross yield and the yield net of impermanent
    loss.
  * defi_protocol_performance_fee_crystallization_frequency_analyzer — that
    ...
    ACROSS (gross yield vs the net-of-IL slice), regardless of frequency.
  * defi_protocol_vault_performance_fee_hurdle_rate_gap_analyzer — that prices
    ...
    ALPHA. HERE it is the fee charged on the impermanent-loss slice of the yield,
  ...
  * defi_protocol_vault_performance_fee_catch_up_clause_gap_analyzer — that
    ...
    static gross-of-impermanent-loss base, independent of any hurdle/catch-up.

The novel axis here: the performance-fee BASE being GROSS-OF-IMPERMANENT-LOSS
rather than NET-OF-IL — a fee-on-IL / fee-base inflation in which the performance
fee is charged on the slice of yield the LP position's impermanent loss erased.
...
# Classification thresholds on the scale-free fee_on_il_fraction in
# [0, 1] (= fee_on_il_gap_pct / fee_charged_pct).
CLEAN_FRACTION = 0.05        # at/below → cleanly on the net-of-IL base
MILD_FRACTION = 0.20         # at/below → mild fee-on-IL gap
...
def _coerce_signed(val) -> Optional[float]:
    ...
    net-of-IL-yield field, which may legitimately be negative.
    """
...
class DeFiProtocolVaultPerformanceFeeGrossOfImpermanentLossBaseGapAnalyzer:
    """
    ...
    on the IMPERMANENT-LOSS slice of the yield (a fee-on-IL / fee-base inflation).
    ...
    charged only on the net-of-IL yield. When the net-of-IL yield equals (or
    exceeds) the gross yield the impermanent loss consumed nothing and the
    performance fee was charged on the right base (CLEAN_NET_OF_IL_BASE). When the
    impermanent loss consumed a large share of the yield, a large share of the
    performance fee was charged on the IL slice (MODERATE / SEVERE fee-on-IL gap),
    and if the fee exceeds the net-of-IL yield the net return goes negative.

    HIGHER score = the performance fee was charged on the net-of-IL base
    (gross ≈ net_of_il), the fee was effectively fair, nothing to fix.
    LOWER score = a large share of the performance fee landed on the
    impermanent-loss slice the depositor never realized, or the net return goes
    ...
    Per-position input dict fields:
        ...
        net_of_il_yield_pct            : float — the yield NET OF the impermanent
        ...
        impermanent_loss_pct           : float — OPTIONAL informational impermanent
                                         (divergence) loss rate %;
                                         ≥ HIGH_IMPERMANENT_LOSS_PCT raises
                                         HIGH_IMPERMANENT_LOSS.
        fee_on_il_gap_pct              : float — OPTIONAL direct override of the
                                         fee-on-IL gap (the performance fee charged
                                         on the impermanent-loss slice). When
                                         supplied (finite; negative → magnitude)
                                         AND a valid POSITIVE gross_yield_pct and
                                         POSITIVE fee_charged_pct are present, take
                                         this gap directly and skip the net-of-IL /
        ...
        fee_charged_pct                : float — OPTIONAL, only used on the override
                                         ...
                                         fee_on_il_fraction (finite > 0 required to
    ...
    def _analyze_one(self, p: dict) -> dict:
        ...
        il_rate = _coerce_num(p.get("impermanent_loss_pct"))

        # Override path: a direct fee-on-IL gap + a positive fee_charged.
        gap_o = _coerce_num(p.get("fee_on_il_gap_pct"))
        ...
        return self._analyze_main(token, p, gross_gain, fee_pct, il_rate)
    ...
    def _analyze_main(
        ...
        # net-of-IL yield may legitimately be negative (the impermanent loss
        ...
        fee_on_il_gap_pct = max(0.0, fee_charged_pct - fair_fee_pct)
        ...
        return self._finish(
            ...
            fee_on_il_gap_pct=fee_on_il_gap_pct,
            impermanent_loss_pct=il_rate,
            used_override=False,
            used_main=True,
        )
    ...
    def _analyze_override(
        ...
        # net-of-IL / IL-slice / fair geometry is unknown on the override path →
        # report None; net return can not be derived without net_of_il_yield, so
        # net-negative / full-fee-on-IL flags / ratio fall back to the gap share.
        return self._finish(
            token=token,
            gross_yield_pct=gross_gain,
            fee_frac=None,
    ...
    def _finish(
        ...
        fair_fee_pct: float,
        fee_on_il_gap_pct: float,
        impermanent_loss_pct: Optional[float],
        used_override: bool,
        used_main: bool,
    ) -> dict:
        # overstatement = the performance fee charged on the IL slice
        # (kept for family consistency with the headline-honesty family).
        overstatement_pct = fee_on_il_gap_pct

        # Net return: only computable when net-of-IL geometry is known.
        if net_of_il_yield_pct is not None:
            ...
            # Override path: net-of-IL geometry unknown. Treat realisation via
            # the fee-on-IL share as the proxy below; flag as not known.
            net_return_after_fee_pct = None
            net_return_fair_pct = None
            net_is_negative = False
            realization_ratio = None

        # Scale-free fee-on-IL fraction — the share of the charged performance
        # fee that landed on the impermanent-loss slice.
        if fee_charged_pct > EPS:
            fee_on_il_fraction = _clamp(
                fee_on_il_gap_pct / fee_charged_pct, 0.0, 1.0)
        else:
            fee_on_il_fraction = 0.0

        # On the override path, with no net-of-IL geometry, anchor the
        # realisation on (1 - fee_on_il_fraction): the share of the fee that fell
        # on the net-of-IL yield is the share the depositor "paid fairly".
        ...
        return {
            ...
            "fee_on_il_fraction": round(
                fee_on_il_fraction, 4),
            "net_is_negative": net_is_negative,
            "impermanent_loss_pct": (
                round(impermanent_loss_pct, 4)
                if impermanent_loss_pct is not None else None),
            "sample_count": 0,
    ...
    def _score(
        ...
        """
        0–100, HIGHER = the performance fee was charged on the net-of-IL yield
        the depositor actually realized: the depositor keeps the yield that
        survived the impermanent loss. Two components:
          * realisation = clamp(realization_ratio, 0, 1) — the fraction of the
            fair net return that survives the gross-based fee,
          * fee-base penalty = clamp(1 − fee_on_il_fraction, 0, 1) — penalises a
            large share of the fee being charged on the IL slice.
    ...
    def _classify(
        ...
        if net_is_negative:
            # The fee has eaten the whole net-of-IL yield (or more).
    ...
    def _flags(
        ...
        if (impermanent_loss_pct is not None
                and impermanent_loss_pct >= HIGH_IMPERMANENT_LOSS_PCT):
            flags.append("HIGH_IMPERMANENT_LOSS")
        ...
        if used_override:
            ...
            if (il_consumed_yield_pct is not None
                ...
                flags.append("FEE_ON_IMPERMANENT_LOSS")
            if (net_of_il_yield_pct is not None
    ...
    def _aggregate(self, results: List[dict]) -> dict:
        ...
        # Higher score = charged on the net-of-IL base / fee fair →
