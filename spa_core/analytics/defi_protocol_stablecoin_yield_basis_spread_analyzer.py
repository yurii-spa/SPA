...
def analyze(
    ...
    *,
    ...
    if _pf.is_protocol_context(token):
        ...
        _legacy = {
            "name": _p["name"],
            "headline_apy_pct": _p["apy_pct"],
            "risk_free_rate_pct": 4.0,
            "protocol_risk_proxy_pct": _p["basis_spread_pp"],
            # структурно: depeg-вероятность из basis-спреда, доли неликвидных
            # активов и утилизации — различает протоколы детерминированно
            "depeg_probability": (
                _p["basis_spread_pp"] + _p["illiquid_asset_pct"] / 10.0
                + _p["utilization_rate_pct"] / 100.0) / 100.0,
            "depeg_severity_pct": 10.0 + _p["illiquid_asset_pct"] / 2.0,
            "holding_days": _p["holding_days"],
            "data_quality": "ok",
        }
