...
_CTX_DOMAIN_KEYS = ("headline_apr_pct", "window_aprs", "vault", "token")


def _context_branch(analyzer, position):
    """(handled, result). Реальный ряд APY: headline = последняя точка,
    window_aprs = средние по фактическим трейлинговым окнам 7/14/30 дней
    (только окна, полностью покрытые рядом; <14 точек → None: движку нужно
    ≥2 окна). Движок «выше = заголовок согласуется с длинной базой» →
    risk = 100 - score."""
    ...
    series = _apy.get_series(position["protocol"], min_days=14,
    ...
    for w in (7, 14, 30):
        if len(values) >= w:
            tail = values[-w:]
            window_aprs[w] = sum(tail) / len(tail)
    if len(window_aprs) < 2:
        return True, None
