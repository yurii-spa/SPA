...
_CTX_DOMAIN_KEYS = ("position_usd", "gross_apr_pct", "entry_gas_usd",
                    "exit_gas_usd", "holding_period_days",
                    "opportunity_cost_apr_pct", "expected_extra_tx_count",
                    "gas_per_extra_tx_usd", "token")


def _context_branch(analyzer, position):
    """(handled, result). Газ = структурная стоимость транзакции чейна,
    позиция = $25k, gross APR — реальная точка ряда (fallback: профиль),
    hurdle — дефолт движка. Движок «выше = профитабельнее» → risk = 100-score."""
    ...
    res = analyzer._analyze_one({
        ...
        "gross_apr_pct": apr,
        "entry_gas_usd": float(prof["gas_cost_usd"]),
        "exit_gas_usd": float(prof["gas_cost_usd"]),
        "holding_period_days": float(prof["holding_days"]),
    })
    return True, engine_risk(res, prof["name"], higher_is_better=True,
                             score_key="capital_efficiency_score")
