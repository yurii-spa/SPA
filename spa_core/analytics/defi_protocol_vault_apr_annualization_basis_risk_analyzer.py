...
_CTX_DOMAIN_KEYS = ("headline_apr_pct", "measurement_window_days",
...
def _context_branch(analyzer, position):
    ...
    окно измерения = фактическое число дневных точек ряда, база = 365,
    волатильность периода = pstdev ряда. <3 точек → None.
    Движок «выше = надёжнее база» → risk = 100 - score."""
    ...
    res = analyzer._analyze_one({
        ...
        "measurement_window_days": float(st["n"]),
        "expected_basis_days": 365.0,
        "period_volatility_pct": st["std"],
    })
