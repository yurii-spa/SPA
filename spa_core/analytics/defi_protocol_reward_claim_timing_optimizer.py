...
def analyze(
    ...
    *,
    ...
    if _pf.is_protocol_context(token):
        ...
        if _p is None:
            return None
        _daily = _p["capital_usd"] * (_p["apy_pct"] / 100.0) / 365.0
        _legacy = {
            "name": _p["name"],
            "accrued_reward_usd": _daily * _p["holding_days"],
            "daily_accrual_usd": _daily,
            "claim_gas_cost_usd": _p["gas_cost_usd"],
            "reinvestment_apr_pct": _p["apy_pct"],
            "days_since_last_claim": _p["holding_days"],
            "data_quality": "ok",
        }
