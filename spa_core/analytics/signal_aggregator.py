...
class SignalAggregator:
    ...
    def run_tier_b(self, protocols: List[str], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        подделываем. 35 модулей из ``UNSOURCED_MODULES`` различают протоколы,
        ...
        composite как измерение. Набор 20 → 35 (цикл #138): производящий
        разметку инструмент звал ТОЛЬКО движки, объявившие вход списком, и
        `dict`-образные (186 из 479 Tier-B) в разметку не попадали вовсе —
        все 15 добавленных именно такие. Обходятся так же, как слепые: НЕ исполняются,
