...
def save_backtest_results(
    ...
    # Атомарная запись через канонический atomic_save (tmp в той же директории
    # + os.replace; создаёт parent dirs). WS4: заменил raw temp-file idiom на
    # canonical helper (см. CLAUDE.md правило #4 / atomic-migration guard).
    atomic_save(serialized, path)
