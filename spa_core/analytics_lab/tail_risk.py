...
CLI (offline, exit 0 always, no tracebacks; junk args → clear ERROR stderr)::
    ...
    python3 -m spa_core.analytics_lab.tail_risk --check   # compute+print, no write (default)
    python3 -m spa_core.analytics_lab.tail_risk --run     # + atomic write
    python3 -m spa_core.analytics_lab.tail_risk --run --data-dir <dir>
...
log = logging.getLogger("spa.analytics_lab.tail_risk")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python3 -m spa_core.analytics_lab.tail_risk",
        description=(
