...
def record_owner_answer(
    ...
    *,
    ...
    ``answer_line`` — чем записать ответ в теле карточки, когда он НЕ является номером
    варианта. Нужен ровно одному случаю: карточка-поручение выбора не предлагает, и ответ
    владельца — подтверждение («Принято», ``owner_decisions.ACK_CHOICE``, #274). Без него
    в карточке появлялось бы «**Вариант ack** — …», то есть выбор, которого автор карточки
    не писал. По умолчанию формулировка прежняя, ни один существующий вызов не меняется.
    ...
    line = answer_line or f"**Вариант {choice_num}** — {choice_label}"
    body_addition = (
        f"\n\n---\n\n{ANSWER_HEADING}\n\n"
        f"{line}\n\n"
