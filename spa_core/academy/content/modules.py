...
def _svg_supply(lang: str) -> str:
    """Inline SVG: supply USDC into Aave pool → receive aUSDC receipt that accrues interest."""
    if lang == "en":
        t = ("You supply", "USDC", "Aave pool", "you receive", "aUSDC", "balance grows with interest")
    else:
        t = ("Вы вносите", "USDC", "Пул Aave", "вы получаете", "aUSDC", "баланс растёт с процентами")
    return ('<figure class="diagram"><svg viewBox="0 0 600 170" role="img" '
            'style="width:100%;max-width:560px;height:auto;font-family:system-ui,sans-serif">'
            '<rect x="20" y="40" width="150" height="46" rx="10" fill="rgba(20,184,166,.08)" stroke="#14b8a6" stroke-width="1.5"/>'
            f'<text x="95" y="60" text-anchor="middle" fill="#9ca3af" font-size="11">{t[0]}</text>'
            f'<text x="95" y="78" text-anchor="middle" fill="#14b8a6" font-size="14" font-weight="700">💵 {t[1]}</text>'
            '<line x1="170" y1="63" x2="228" y2="63" stroke="#9ca3af" stroke-width="1.4" marker-end="url(#su)"/>'
            '<defs><marker id="su" markerWidth="7" markerHeight="7" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#9ca3af"/></marker></defs>'
            '<rect x="230" y="40" width="140" height="46" rx="10" fill="rgba(139,92,246,.08)" stroke="#8b5cf6" stroke-width="1.5"/>'
            f'<text x="300" y="68" text-anchor="middle" fill="#8b5cf6" font-size="14" font-weight="700">🏦 {t[2]}</text>'
            '<line x1="370" y1="63" x2="428" y2="63" stroke="#9ca3af" stroke-width="1.4" marker-end="url(#su)"/>'
            '<rect x="430" y="40" width="150" height="46" rx="10" fill="rgba(20,184,166,.10)" stroke="#14b8a6" stroke-width="1.5"/>'
            f'<text x="505" y="60" text-anchor="middle" fill="#9ca3af" font-size="11">{t[3]}</text>'
            f'<text x="505" y="78" text-anchor="middle" fill="#14b8a6" font-size="14" font-weight="700">🧾 {t[4]}</text>'
            f'<text x="300" y="120" text-anchor="middle" fill="#14b8a6" font-size="12">📈 {t[5]}</text>'
            '</svg></figure>')


def _svg_util(lang: str) -> str:
    """Inline SVG: pool utilization bar — supplied vs borrowed; high utilization can block instant withdraw."""
    if lang == "en":
        t = ("Pool: supplied vs borrowed", "borrowed (earning you APY)", "free (withdrawable now)",
             "near 100% borrowed → withdraw may wait")
    else:
        t = ("Пул: внесено vs занято", "занято (даёт вам APY)", "свободно (можно вывести сейчас)",
             "занято ~100% → вывод может подождать")
    return ('<figure class="diagram"><svg viewBox="0 0 600 150" role="img" '
            'style="width:100%;max-width:560px;height:auto;font-family:system-ui,sans-serif">'
            f'<text x="300" y="26" text-anchor="middle" fill="#e5e7eb" font-size="13" font-weight="600">{t[0]}</text>'
            '<rect x="40" y="44" width="520" height="34" rx="6" fill="rgba(20,184,166,.10)" stroke="#14b8a6" stroke-width="1.2"/>'
            '<rect x="40" y="44" width="390" height="34" rx="6" fill="rgba(245,158,11,.30)" stroke="#f59e0b" stroke-width="1.2"/>'
            f'<text x="235" y="66" text-anchor="middle" fill="#fbbf24" font-size="12" font-weight="600">{t[1]}</text>'
            f'<text x="495" y="66" text-anchor="middle" fill="#14b8a6" font-size="11">{t[2]}</text>'
            f'<text x="300" y="108" text-anchor="middle" fill="#9ca3af" font-size="12">⚠ {t[3]}</text>'
            '</svg></figure>')


_SVG_SUPPLY_RU, _SVG_SUPPLY_EN = _svg_supply("ru"), _svg_supply("en")
...
MODULES: Dict[int, dict] = {
    ...
    5: {
        ...
        "wallet_limit_usd": 150,
        "theory_html_ru": (
            "<p>Операция <strong>Supply</strong> вносит ваши токены в пул ликвидности Aave, откуда их "
            "занимают другие пользователи под процент. Взамен вы получаете <strong>aToken</strong> "
            "(например, aUSDC) — квитанцию депозита, баланс которой сам растёт с процентами прямо в кошельке.</p>"
            + _SVG_SUPPLY_RU +
            "<h4>Как это устроено</h4>"
            "<p>Откуда берётся доход? Заёмщики платят процент за взятые из пула средства; этот процент "
            "распределяется вкладчикам. <strong>APY</strong> — годовая доходность со сложным процентом; она "
            "<em>плавает</em> вместе с <em>utilization</em> (какая доля пула занята): чем выше спрос на "
            "займы, тем выше ставка вкладчику.</p>"
            "<h4>Разбор на примере</h4>"
            "<p>Вы вносите 100 USDC → получаете ~100 aUSDC → через месяц при APY ~4% баланс aUSDC станет "
            "~100.33. Никаких действий не нужно: проценты «капают» в сам aToken. Supply-only (без займа под "
            "залог) — самый простой и безопасный режим: вам нечего ликвидировать.</p>"
            "<h4>Что может пойти не так</h4>"
            "<ul>"
            "<li><strong>Фейковый контракт.</strong> Мошеннический «Aave» уведёт депозит. Сверяйте "
            "официальный адрес контракта перед подписью.</li>"
            "<li><strong>Depeg / oracle-риск.</strong> Если стейбл в пуле теряет привязку или оракул даёт "
            "неверную цену — возможны потери. Держитесь ликвидных, проверенных активов.</li>"
            "<li><strong>Плавающий APY.</strong> Ставка не гарантирована — она меняется каждый блок. "
            "Вчерашние 8% сегодня могут быть 3%.</li>"
            "</ul>"
            "<p class=\"glossary\"><strong>Словарь:</strong> <em>supply</em> — внести в пул; <em>aToken</em> "
            "— растущая квитанция депозита; <em>APY</em> — годовая доходность (сложный процент); "
            "<em>utilization</em> — доля занятого пула; <em>oracle</em> — источник цены для протокола.</p>"
        ),
        "theory_html_en": (
            "<p>The <strong>Supply</strong> action puts your tokens into an Aave liquidity pool, where other "
            "users borrow them for interest. In return you receive an <strong>aToken</strong> (e.g. aUSDC) — "
            "a deposit receipt whose balance grows with interest right in your wallet.</p>"
            + _SVG_SUPPLY_EN +
            "<h4>How it works</h4>"
            "<p>Where does the yield come from? Borrowers pay interest on funds taken from the pool; that "
            "interest is distributed to suppliers. <strong>APY</strong> is the annual yield with compounding; "
            "it <em>floats</em> with <em>utilization</em> (how much of the pool is borrowed): the higher the "
            "borrowing demand, the higher the supplier rate.</p>"
            "<h4>Worked example</h4>"
            "<p>You supply 100 USDC → receive ~100 aUSDC → after a month at ~4% APY the aUSDC balance is "
            "~100.33. No action needed: interest accrues into the aToken itself. Supply-only (no borrowing "
            "against collateral) is the simplest and safest mode: there is nothing to liquidate.</p>"
            "<h4>What can go wrong</h4>"
            "<ul>"
            "<li><strong>Fake contract.</strong> A scam \"Aave\" will steal the deposit. Verify the official "
            "contract address before signing.</li>"
            "<li><strong>Depeg / oracle risk.</strong> If a pool stablecoin loses its peg or an oracle "
            "misprices it, losses are possible. Stick to liquid, proven assets.</li>"
            "<li><strong>Floating APY.</strong> The rate is not guaranteed — it changes every block. "
            "Yesterday's 8% can be 3% today.</li>"
            "</ul>"
            "<p class=\"glossary\"><strong>Glossary:</strong> <em>supply</em> — put into the pool; "
            "<em>aToken</em> — a growing deposit receipt; <em>APY</em> — annual yield (compounded); "
            "<em>utilization</em> — the borrowed share of a pool; <em>oracle</em> — a protocol's price "
            "source.</p>"
        ),
        "practice_html_ru": (
            "<p><strong>Задание:</strong> внесите малую сумму стейбла (в пределах лимита) в Aave на Base и "
            "убедитесь, что на кошельке появился aToken. Сверьте адрес официального контракта Aave перед "
            "подписью.</p>"
            "<p><strong>Шаги:</strong> (1) проверьте официальный адрес пула Aave; (2) approve нужную сумму; "
            "(3) сделайте supply малой суммы USDC; (4) убедитесь, что в кошельке появился aUSDC.</p>"
            + _SAFETY_NOTE
        ),
        "practice_html_en": (
            "<p><strong>Task:</strong> supply a small stablecoin amount (within the limit) to Aave on Base "
            "and confirm the aToken appears in your wallet. Verify the official Aave contract address before "
            "signing.</p>"
            "<p><strong>Steps:</strong> (1) check the official Aave pool address; (2) approve the needed "
            "amount; (3) supply a small USDC amount; (4) confirm aUSDC now shows in your wallet.</p>"
            + _SAFETY_NOTE_EN
        ),
        "spa_connection_html_ru": (
            "<p><strong>Что бы здесь сделал SPA:</strong> Aave V3 — T1-протокол в whitelist SPA; допуск "
            "проходит детерминированную RiskPolicy (TVL-floor ≥ $5M, APY-границы, per-protocol cap'ы). Ваш "
            "аналог: депонируйте только в проверенные протоколы с реальной ликвидностью, а не в то, что "
            "обещает больше всех. Высокий APY — часто плата за риск, а не подарок.</p>"
        ),
        "spa_connection_html_en": (
            "<p><strong>What SPA would do here:</strong> Aave V3 is a T1 protocol in SPA's whitelist; "
            "admission passes the deterministic RiskPolicy (TVL floor ≥ $5M, APY bounds, per-protocol caps). "
            "Your version: deposit only into proven protocols with real liquidity, not whatever promises the "
            "most. A high APY is often compensation for risk, not a gift.</p>"
        ),
    },
    # ── M6 — Вывод из Aave ───────────────────────────────────────────────────
    6: {
        ...
        "wallet_limit_usd": 150,
        "theory_html_ru": (
            "<p><strong>Withdraw</strong> возвращает первоначальный депозит вместе с процентами, "
            "накопившимися за время удержания. Соответствующий <strong>aToken</strong> при этом "
            "автоматически сжигается (burn) — квитанция погашается, вы получаете обратно базовый актив.</p>"
            + _SVG_UTIL_RU +
            "<h4>Как это устроено</h4>"
            "<p>Вывод возможен только из <em>свободной</em> (не занятой) части пула. Обычно её достаточно, "
            "но при экстремально высоком <em>utilization</em> (почти всё роздано в займы) мгновенный вывод "
            "крупной суммы может быть временно недоступен — придётся подождать, пока заёмщики вернут "
            "средства или подойдут новые вкладчики. Частичный вывод при этом обычно проходит.</p>"
            "<h4>Разбор на примере</h4>"
            "<p>У вас ~100.33 aUSDC. Withdraw сжигает aUSDC и возвращает ~100.33 USDC на кошелёк. Пул на 78% "
            "занят → свободных 22% с запасом хватает на ваш вывод. Если бы было занято 99% — вывод крупной "
            "суммы мог бы «подождать». Позицию важно <strong>мониторить</strong>: ставки, utilization и "
            "риски протокола меняются со временем.</p>"
            "<h4>Что может пойти не так</h4>"
            "<ul>"
            "<li><strong>Ликвидность иссякла.</strong> Utilization ~100% → крупный вывод недоступен сразу. "
            "Не паникуйте: выводите частями или ждите.</li>"
            "<li><strong>«Досижу до разворота».</strong> Самая дорогая ошибка — держать позицию против "
            "заранее заданного правила выхода. <strong>Kill-rule</strong> — условие, при котором вы выходите "
            "немедленно, механически, без надежды.</li>"
            "<li><strong>Забыл мониторить.</strong> Протокол, безопасный вчера, может измениться. Проверяйте "
            "позицию, а не «поставил и забыл» на месяцы.</li>"
            "</ul>"
            "<p class=\"glossary\"><strong>Словарь:</strong> <em>withdraw</em> — забрать депозит; "
            "<em>burn</em> — сжигание aToken при выводе; <em>utilization</em> — занятая доля пула; "
            "<em>kill-rule</em> — заранее заданное правило выхода.</p>"
        ),
        "theory_html_en": (
            "<p><strong>Withdraw</strong> returns your original deposit plus the interest accrued over the "
            "holding period. The corresponding <strong>aToken</strong> is automatically burned — the receipt "
            "is redeemed and you get the underlying asset back.</p>"
            + _SVG_UTIL_EN +
            "<h4>How it works</h4>"
            "<p>Withdrawing is only possible from the <em>free</em> (unborrowed) part of the pool. Usually "
            "there is plenty, but at extremely high <em>utilization</em> (almost everything lent out), an "
            "instant large withdrawal may be temporarily unavailable — you wait until borrowers repay or new "
            "suppliers arrive. A partial withdrawal usually still goes through.</p>"
            "<h4>Worked example</h4>"
            "<p>You hold ~100.33 aUSDC. Withdraw burns the aUSDC and returns ~100.33 USDC to your wallet. The "
            "pool is 78% utilized → the free 22% comfortably covers your withdrawal. If it were 99% utilized, "
            "a large withdrawal might have to \"wait.\" It matters to <strong>monitor</strong> the position: "
            "rates, utilization, and protocol risk change over time.</p>"
            "<h4>What can go wrong</h4>"
            "<ul>"
            "<li><strong>Liquidity dried up.</strong> ~100% utilization → a large withdrawal isn't instant. "
            "Don't panic: withdraw in parts or wait.</li>"
            "<li><strong>\"I'll hold for the rebound.\"</strong> The costliest mistake is holding against a "
            "pre-set exit rule. A <strong>kill-rule</strong> is a condition where you exit immediately, "
            "mechanically, without hope.</li>"
            "<li><strong>Forgot to monitor.</strong> A protocol safe yesterday can change. Check the "
            "position — don't \"set and forget\" for months.</li>"
            "</ul>"
            "<p class=\"glossary\"><strong>Glossary:</strong> <em>withdraw</em> — take the deposit back; "
            "<em>burn</em> — destroying the aToken on withdrawal; <em>utilization</em> — the borrowed share "
            "of a pool; <em>kill-rule</em> — a pre-defined exit rule.</p>"
        ),
        "practice_html_ru": (
            "<p><strong>Задание:</strong> выведите свой депозит из Aave (withdraw), убедитесь, что aToken "
            "сожжён, а стейбл с процентами вернулся на кошелёк. Сформулируйте свой личный kill-rule.</p>"
            "<p><strong>Шаги:</strong> (1) откройте позицию в Aave; (2) сделайте withdraw; (3) убедитесь, "
            "что aUSDC сожжён, а USDC (с процентами) вернулся; (4) запишите свой kill-rule (например: "
            "«выхожу при depeg > 1%»).</p>" + _SAFETY_NOTE
        ),
        "practice_html_en": (
            "<p><strong>Task:</strong> withdraw your deposit from Aave, confirm the aToken is burned and the "
            "stablecoin plus interest is back in your wallet. Write your own personal kill-rule.</p>"
            "<p><strong>Steps:</strong> (1) open your Aave position; (2) withdraw; (3) confirm the aUSDC is "
            "burned and USDC (with interest) is back; (4) write down your kill-rule (e.g. \"I exit on a depeg "
            "> 1%\").</p>" + _SAFETY_NOTE_EN
        ),
        "spa_connection_html_ru": (
            "<p><strong>Что бы здесь сделал SPA:</strong> у SPA — двухуровневый kill-switch (SOFT-derisk при "
            "−5%, HARD all-cash при −10%): выход заранее задан правилом, а не эмоцией, и исполняется "
            "детерминированно (LLM запрещён в этом контуре). Ваш аналог: определите kill-rule ДО входа и "
            "исполняйте его механически. Дисциплина выхода важнее точки входа.</p>"
        ),
        "spa_connection_html_en": (
            "<p><strong>What SPA would do here:</strong> SPA has a two-tier kill-switch (SOFT de-risk at "
            "−5%, HARD all-cash at −10%): the exit is defined by a rule, not emotion, and executes "
            "deterministically (LLM forbidden in this path). Your version: define a kill-rule BEFORE entering "
            "and execute it mechanically. Exit discipline matters more than the entry point.</p>"
        ),
    },
