...
Assembles a single read-only "investor report" that an allocator/LP would want:
  ...
  (a) PnL attribution by protocol + portfolio roll-up
      (``spa_core.reporting.pnl_attribution``).
...
CLI::
    ...
    python -m spa_core.reporting.investor_report
    python -m spa_core.reporting.investor_report --output data/investor_report.json \\
        --limit 50 --no-pdf
"""
...
from spa_core.reporting.pnl_attribution import compute_pnl_attribution
...
def _maybe_generate_pdf(report: dict, json_path: str | Path) -> str | None:
    ...
    try:
        from spa_core.reporting.pdf_generator import generate_report
    except Exception as exc:  # noqa: BLE001
