...
4. sharpe_trigger    — Sharpe < -1.0 за 30 дней (из data/analytics_summary.json)
...
class KillSwitchChecker:
    ...
    def check_sharpe_trigger(self) -> tuple[bool, str]:
        ...
        # Если данных мало (num_days < 5), Sharpe не является надёжным сигналом
        num_days = doc.get("num_days", 0)
        if isinstance(num_days, (int, float)) and num_days < 5:
            return False, f"sharpe {sharpe_val:.4f} — insufficient data ({num_days} days < 5)"
