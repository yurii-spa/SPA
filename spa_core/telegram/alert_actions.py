...
# маячок перед тем, как навесить кнопки. Fail-CLOSED: маячка нет / бота давно нет /
# не тот набор умений / файл битый → кнопок нет, алерт уходит как обычный текст.
# Никакого ручного шага: как только бот перезапустится с новым кодом, кнопки
# включатся сами. Окно ПЕРЕЗАПУСКА кнопок НЕ снимает (ADR-400, см. BEACON_MAX_AGE_S):
# нажатие в это окно бот получит после подъёма, а неизвестный глагол с 16.09 роутер
# больше не превращает в правку сообщения (router._apply_action).
...
#: Насколько СТАРЫМ может быть маячок, чтобы кнопки ещё вешались (ADR-400).
#:
#: До 16.09 здесь стояло 300 с — тот же порог, которым `telegram_health` судит,
#: КРУТИТСЯ ли цикл бота. Это два разных вопроса, и один порог отвечал на оба:
#:
#:   * «жив ли бот прямо сейчас?» — вопрос сторожа; ему и нужны минуты;
#:   * «есть ли на этой машине бот, чей код знает этот глагол?» — вопрос отправителя
#:     кнопок. На него отвечает СПИСОК умений в маячке, а не его возраст: нажатие,
#:     сделанное пока бот лежит, Telegram держит в очереди до 24 ч и отдаёт первому же
#:     `getUpdates` — кнопка обрабатывается, когда бот поднимется.
#:
#: Цена совмещения измерена по журналам, а не предположена: бот перезапускается
#: ~2 раза в сутки (замер 02.09 — 50 перезапусков за 25 суток: сентинел ADR-117 после
#: каждой доставки, кикстарты сторожей, зависания), и каждое окно перезапуска дольше
#: 300 с снимало кнопки с любого вопроса, отправленного в это окно. Владелец получал
#: «⚠️ Кнопки сейчас недоступны — бот не подтвердил, что готов их обработать» под
#: карточкой, чьи варианты разобрались безупречно, — и жаловался на это 10.08, 14.08,
#: 22.08 и 16.09, при том что «починено» ему отвечали каждый раз. Ремонт вдогонку
#: (`heal_buttonless`) зависит от ещё одной цепочки условий и бежит только на старте
#: бота, то есть закрывал этот класс лишь частично.
#:
#: Ноль тоже не годится: маячок, которому месяц, означает, что бота на этой машине
#: больше нет, а нажатие по кнопке через 24 ч Telegram уже не отдаст — владелец решил
#: бы, что ответил. Порог — часы: заведомо больше любого окна перезапуска и заведомо
#: меньше срока хранения нажатия. Бот, лежащий дольше, — CRITICAL у `telegram_health`
#: и `self_heal` независимо от кнопок, а вопросы, уехавшие за это время без них,
#: по-прежнему добирает `heal_buttonless` при подъёме.
BEACON_MAX_AGE_S = 6 * 3600
#: Отметка маячка вправе быть чуть ПОЗЖЕ момента, от которого считаем возраст:
#: бот пишет маячок каждый виток, а `now` берётся до чтения файла (класс ADR-216 —
#: у `telegram_health` ноль здесь стоил перезапусков живого бота). Маячок из будущего
#: означает, что бот ЖИВ, а не что его нет. Далёкое будущее (разошлись часы) —
#: по-прежнему отказ.
BEACON_FUTURE_TOLERANCE_S = 60
...
def handler_available(
    *,
    ...
    """Есть ли на этой машине бот, умеющий обработать ИМЕННО ЭТО нажатие. Сомнение → False.

    Вопрос — про КОД бота (объявил ли он умение), а не про то, крутится ли он в эту
    секунду: нажатие, сделанное пока бот перезапускается, Telegram доставит ему после
    подъёма (см. :data:`BEACON_MAX_AGE_S`). Свежесть маячка проверяется в ЧАСАХ — только
    чтобы не вешать кнопку там, где бота давно нет.
    ...
    try:
        ...
        return -BEACON_FUTURE_TOLERANCE_S <= age <= BEACON_MAX_AGE_S
