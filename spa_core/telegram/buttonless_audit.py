...
#: Наша дверь, а вариантов в журнале нет: кнопкам неоткуда взяться. ПОЧЕМУ их не было,
#: этот код не знает и знать не может — тела карточки модуль не читает по построению
#: (сопоставление строго по ``message_id``). Лекарство «чинить РАЗБОР» отсюда СНЯТО
#: (ADR-178): у той же карточки `buttonless_reason`, читающий тело, отвечает
#: ``multiselect_card`` — «разбор НЕ трогать, он отказывает верно», и два соседних
#: вердикта об одном предмете противоречили друг другу в одном и том же отчёте.
...
# Правило текстов (ADR-178). Лекарство здесь допустимо ТОЛЬКО там, где его
# устанавливает СОБСТВЕННОЕ измерение этого модуля: «журнал говорит «кнопки были», а
# канал — «не было»» — это спор двух НАШИХ записей, и путь между сборкой клавиатуры и
# отправкой назван по праву; «у нашей двери message_id пишется всегда» — тоже наше
# наблюдение. А вот ПОЧЕМУ карточка не дала вариантов, решается по её ТЕЛУ, которого
# этот модуль не читает: там лекарство назначает `buttonless_reason`, и он же знает
# случаи, где верным ответом является «разбор НЕ трогать» (`multiselect_card`,
# ADR-177). Назначать лекарство в чужом домене — значит звать следующую сессию сломать
# верный отказ; ровно это и печаталось в шаге 0-офис 30.08.
_JOIN_TEXT = {
    JOIN_OWN_NO_OPTIONS: "наша дверь, вариантов в журнале нет — ПОЧЕМУ, отвечает "
                         "`buttonless_reason` по телу карточки",
...
def _window_tail(report: Dict[str, Any]) -> str:
    """Хвост про ОКНО журнала. Ставится только рядом с утверждением о полноте.

    Замер #563 (заказ #562, ADR-322/ADR-323): ``alert_history.json`` есть кольцевой
    буфер (`max_entries`), и на день замера он ПОЛОН — 500 из 500. Значит журнал
    начинается не «с начала времён», а с самого раннего уцелевшего дня, и сам этот
    день срезан с головы: сколько отправок было на нём в действительности, журнал
    не помнит.

    Для находки это безразлично — найденное найдено. Но строка «все с кнопками»
    есть утверждение о ПОЛНОТЕ, и без границы окна она читается как «никогда не
    было», а означает «в пределах удержанного». Поэтому хвост печатается ровно у
    тех двух веток, которые такое утверждение делают.

    Блока `window` нет ⇒ окно НЕ ИЗМЕРЕНО, и так и сказано: отчёт старого образца
    не имеет права выглядеть как журнал с доказанно целым окном (fail-CLOSED).
    """
    window = report.get("window")
    if not isinstance(window, dict):
        return " · окно журнала НЕ ИЗМЕРЕНО (отчёт без блока `window`)"
    if not window.get("measured"):
        return f" · окно журнала НЕ ИЗМЕРЕНО ({window.get('reason')})"
    if window.get("saturated") is None:
        return " · окно журнала НЕ НАЗВАНО: буфер не объявляет вместимости"
    if not window.get("saturated"):
        return ""
    since = window.get("boundary_day") or "?"
    return (f" · окно журнала СРЕЗАНО: видно с {since}, и сам этот день частичен "
            f"(вытеснение с головы) — утверждение о полноте верно лишь внутри окна")
...
def summary_line(report: Dict[str, Any]) -> str:
    ...
    if healed_tail:
        ...
        return (f"сообщения с вариантами: {choice}, незакрытых без кнопок нет"
                f"{healed_tail}{_window_tail(report)}")
    ...
    return f"сообщения с вариантами: {choice}, все с кнопками{tail}{_window_tail(report)}"
