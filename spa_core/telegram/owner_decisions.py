...
def parse_options(body: str) -> List[ParsedOption]:
    ...
    explicit_dialect = any(_OPTION_RE.match(ln) or _OPTION_LETTERED_RE.match(ln)
                           or _OPTION_LETTERED_PLAIN_RE.match(ln) for ln in section)
    ...
    for ln in section:
        ...
        numbered = False
        if m is None:
            # Буквенный пункт без жирного — подпись берётся из `plain`, как у
            # нумерованного без жирного; флаг `numbered` здесь и означает «подпись в
            # `plain`», а не «метка — цифра».
