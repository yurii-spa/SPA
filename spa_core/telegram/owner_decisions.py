...
_UNREAD_CHOICE_RE = re.compile(
    ...
    #
    # ГОЛАЯ ОДНА БУКВА — метка ТОЛЬКО перед концом строки или знаком, а не перед другим
    # словом (#484). Без этого условия метка-буква съедала два разных класса прозы:
    #
    #   «это выбор варианта из какой-то карточки» — жадное `\w*` откатывалось на один
    #       символ, и ПОСЛЕДНЯЯ БУКВА САМОГО СЛОВА шла за метку («варианта» = «вариант»
    #       + метка «а»); живой предмет — `owner-decision-utochnenie-po-zametke-1`,
    #       карточка БЕЗ перечня, владельцу ехало «варианты есть, кнопок нет — заведи
    #       дефект» с лекарством «научить `parse_options` этой форме», то есть чинить
    #       НЕСУЩЕСТВУЮЩУЮ форму;
    #   «вариантов у нас пока нет» — односимвольные предлоги и союзы (у, и, а, в, к, о,
    #       с, я) стоя́т ровно там, где метка «А»/«Б».
    #
    # Признак обязан требовать ПЕРЕЧНЯ, а не слова — это его собственный докстринг.
    # Числовые формы («вариант 2», «вариант А1») однозначны и ограничения не требуют.
    r"(?:вариант\w*|option)\s*[—–:-]?\s*"
    r"(?:[A-Za-zА-Яа-я]\d{1,2}\b|\d{1,2}[A-Za-zА-Яа-я]\b|\d{1,2}\b"
    r"|[A-Za-zА-Яа-я](?=\s*(?:[)\].,;:!?—–-]|$)))"
    # Строка, ОТКРЫВАЮЩАЯ перечень: слово «вариант*» и двоеточие в конце строки.
    # Прежняя редакция требовала двоеточия ВПЛОТНУЮ к слову («Варианты:»), и живая
    # карточка `owner-decision-10-deneg-stoyat-bez-raboty-prichina-naid` открывает
    # перечень иначе — «…(варианты не исключают друг друга):». Она числилась находкой
    # ТОЛЬКО из-за отката жадного `\w*` выше, то есть по дефекту; убрав дефект и не
    # добавив эту форму, мы потеряли бы ВЕРНУЮ находку — обеднение сторожа, ровно то,
    # против чего написан #349. Двоеточие в КОНЦЕ строки обязательно: без него признак
    # снова свёлся бы к голому слову в прозе.
    r"|варианты\s*[:—–-]"                      # «Варианты:» — перечень открыт
    r"|вариант\w*[^\n]*:\s*$"                 # «…(варианты не исключают друг друга):» — тоже
