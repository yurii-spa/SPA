...
def build_message(title: str, body: str, options: List[ParsedOption],
    ...
    if options:
        ...
        if has_buttons:
            ...
            #
            # Жалоба владельца 17.08 вскрыла, что прежняя фраза «Ответь номером варианта в
            # чат, я разберу» — ОБЕЩАНИЕ НА ТОЙ ЖЕ НЕПОДТВЕРЖДЁННОЙ СПОСОБНОСТИ. Разбирает
            # текстовый ответ ТОТ ЖЕ бот, чей маячок мы только что не смогли прочитать: раз
            # он не подтвердил нажатия, за текст он тоже не отвечает. Владелец писал номер
            # и не получал ничего — то есть fail-CLOSED честно снял кнопки и тут же выдал
            # непроверенную гарантию вместо них. Обещать несуществующее хуже, чем не
            # обещать: ровно этот закон записан в докстринге выше, и нарушала его соседняя
            # строка.
            #
            # Теперь текстовый путь предлагается как ПОПЫТКА, а рядом называется способ,
            # который работает без бота вообще (статус карточки — файл в git).
            parts += ["", "⚠️ Кнопки сейчас недоступны — бот не подтвердил, что готов их "
                          "обработать. Ответь номером варианта в чат; ручаться за это "
                          "нельзя (разбирает тот же бот), поэтому если я не отвечу — "
                          "переведи статус карточки в трекере, это работает без бота."]
