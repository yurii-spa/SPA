...
#: Карточка задаёт ОДИН вопрос, но разрешает взять НЕСКОЛЬКО пунктов («варианты не
#: исключают друг друга»). Отказ от кнопок ВЕРЕН — одна закрывающая кнопка выразить
#: такой ответ не может. Отдельный код по цене живого случая (цикл #426, 30.08): обе
#: карточки, висевшие «без кнопок», написаны безупречно по §2.4 (`**Вариант 1 — …**`,
#: форму разбор знает с #368) и обе получали `unreadable_options_in_card` с лекарством
#: «научить `parse_options` этой форме». Исполнить его значило бы собрать три
#: ВЗАИМОИСКЛЮЧАЮЩИЕ кнопки под вопросом, который сам себя называет невзаимоисключающим,
#: — то есть выдумать владельцу выбор, которого карточка не предлагала (ADR-075), сломав
#: работающий отказ. Тот же класс, что `multi_question_card` (#359), на одно состояние
#: в сторону: `multi_question` для этого состояния честно отдаёт ``None`` (разбор вышел
#: ДО подсчёта пунктов), и провал шёл дальше — в «не прочитали».
CODE_MULTISELECT = "multiselect_card"
...
_REMEDY_MULTISELECT = (
    "разбор НЕ трогать: он отказывает верно — одна кнопка не выражает ответ «1 и 3». "
    "Владелец отвечает номерами в чат (бот так и пишет), а закрыть состояние насовсем "
    "можно только формой вопроса: снять «варианты не исключают друг друга» и разделить "
    "карточку на решения, каждое со своим взаимоисключающим перечнем")
...
def _explain_no_options(path: Path, ref: str) -> Reason:
    ...
    if allows_multiple(body):
        # Пунктов много, а вопрос ОДИН — и брать их можно вместе. Ветка стоит ВЫШЕ
        # `multi_question` в том же порядке, в каком их различает единственный писатель
        # правила (`owner_decisions.prepare`): две копии одного порядка, разошедшись,
        # объяснили бы состояние, которого нет. Функционально они не пересекаются —
        # `_parse_options_measured` выходит по `allows_multiple` ДО подсчёта пунктов,
        # поэтому `multi_question` здесь всегда `None`, — и ровно поэтому раньше
        # состояние проваливалось в «мы не прочитали».
        return Reason(
            CODE_MULTISELECT,
            f"карточка разрешает выбрать НЕСКОЛЬКО пунктов, поэтому кнопок нет: одна "
            f"кнопка закрывает карточку целиком и ответ «1 и 3» ею не выразить. Выбор "
            f"написан и РАЗБИРАЕТСЯ — отказ верен, дефекта разбора здесь нет "
            f"(сверено с `{ref}`, {short})",
            _REMEDY_MULTISELECT,
        )
