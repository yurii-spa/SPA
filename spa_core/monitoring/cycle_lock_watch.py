...
# Код отказа замка — из ЕДИНСТВЕННОГО словаря исходов. Импорт безопасен и
# предусмотрен самим словарём: `cycle_exit` не имеет ни одной зависимости
# (пакет `spa_core.paper_trading` пуст), money-path в read-only слой не тянется.
from spa_core.paper_trading.cycle_exit import EXIT_LOCK_REFUSED
...
def count_refusals(
    ...
    for m in _REFUSAL_RE.finditer(text):
        if m.group("code") != str(EXIT_LOCK_REFUSED):
            continue
        try:
...
# ── Что означает ОТКАЗ ЗАМКА для здоровья агента ────────────────────────────
# Вторая половина карточки `inbox-otkaz-zamka-tsikla-neotlichim-ot-avarii`.
#
# Первую половину закрыл цикл #219: коды разведены, авария теперь 1, отказ замка — 2.
# Но карточка требовала ОБЕ половины сразу — «новый код без правки читателя даст то
# же жёлтое, то есть смену цифры без смены смысла», — и читатель остался прежним:
# `agent_health` красит двойку тем же WARNING, что и аварию.
#
# ПОЧЕМУ ДВОЙКУ НЕЛЬЗЯ ПРОСТО ПОГАСИТЬ. Отказ замка порождается двумя
# противоположными причинами, и код выхода их не различает:
#
#   * замок держит ЖИВОЙ цикл — отказ второго вызова есть работа защиты
#     (08.08: 18 отказов из 20 вызовов, ни одной аварии — весь жёлтый на пульте
#     был порождён УСПЕШНОЙ работой);
#   * замок держит ТРУП — тот же самый отказ означает, что дневной цикл встал
#     (08.08 оба инцидента именно такие: pid 99899 и pid 98535, оба мертвы).
#
# Погасить код 2 «чтобы пульт был зеленее» значило бы заглушить вторую причину
# вместе с первой — починить тишину вместо сигнала. Различить их по коду нечем,
# а по замку — можно: номер держателя лежит в файле, живость меряется одним
# системным вызовом. Поэтому тишина здесь покупается ТОЛЬКО доказательством.
#
# Решение живёт ЗДЕСЬ, а не в читателе, намеренно: это вывод из состояния замка,
# то есть предмет именно этого сторожа. Читателю остаётся вызов в три строки
# (см. `docs`-заметку в отчёте цикла), и вся логика покрыта тестами тут же.
def judge_lock_refusal(
    verdict: Optional[CycleLockVerdict],
    last_exit: object = EXIT_LOCK_REFUSED,
) -> Optional[tuple]:
    """``(severity, слова)`` для агента, вернувшего код отказа замка.

    ``None`` — «это не про отказ замка, судить нечем»: код не тот, и вызывающий
    обязан разбираться с ним по-прежнему (fail-CLOSED, чужой исход не молчит).

    ``severity == OK`` возвращается ТОЛЬКО когда предъявлен живой держатель.
    Всё остальное — труп, протухший замок, нечитаемый замок, замок вообще не
    измерен (``verdict is None``) — это ``WARNING`` с названной причиной: «не
    измерено» никогда не хранится как «в порядке».
    """
    if last_exit != EXIT_LOCK_REFUSED:
        return None

    if verdict is not None and verdict.state == STATE_HELD_ALIVE:
        who = "" if verdict.pid is None else f" pid={verdict.pid}"
        return (OK, f"отказ ЗАКОНЕН: держатель{who} ЖИВ — цикл идёт, трек защищён")

    state = "НЕ ИЗМЕРЕНО" if verdict is None else verdict.state
    return (
        WARNING,
        f"законность отказа НЕ ДОКАЗАНА: живой держатель не предъявлен "
        f"(замок: {state})",
    )
