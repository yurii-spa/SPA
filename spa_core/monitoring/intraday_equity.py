...
def main(argv=None) -> int:
    ...
    # ── ЗАМЕР 2026-08-18: отказ обязан отличаться от «просадки нет» ──────────
    #
    # Внутри модуля разница честная (UNCHECKED, tier=None, лестница не
    # вызывается). СНАРУЖИ её не видел никто: процесс выходил 0 и в обоих
    # случаях — «измерено, спокойно» и «не измерено вообще» — выглядел для
    # launchd и `agent_health_monitor` ОДИНАКОВО успешным. Сенсор мог быть
    # слеп неделями, а все сторожа отвечали «агент жив, отработал».
    # Это тот же класс, что снятый таймлок, читавшийся как «RPC не ответил».
    #
    # Ненулевой выход — существующий и уже наблюдаемый канал: у
    # `agent_health_monitor` `last_exit not in (None, 0)` краснеет
    # (`architecture_conformance` использует ту же шкалу: UNCHECKED → 1).
    # Ни одного нового ПОРОГА и ни одного нового такта здесь не вводится —
    # пороги лестницы (SOFT −5% / HARD −10%) не тронуты, такт агента (300с)
    # не тронут; меняется только СЛЫШИМОСТЬ отказа.
