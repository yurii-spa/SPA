...
def _reader_reconciliation(data_dir: Path, history: List[dict],
    ...
    ШАГ РАНЬШЕ: `load_history` молча пропускает нечитаемую строку. Поэтому
    сверяются ДВА независимых чтения одного файла, и расхождение читателей —
    «не измерено», а не поправка к числу.

    ПОПРАВКА 2026-09-21, снятая замером. Прежняя редакция этого докстринга
    утверждала, что `load_history` «схлопывает повтор даты (побеждает последняя
    строка)», и сверка была построена на этой посылке: ``len(dates) ==
    len(history)``, то есть РАЗЛИЧНЫЕ ДАТЫ сравнивались со СТРОКАМИ. Посылка
    ложна, и ложна она в опасную сторону — схлопывание есть ДЕФЕКТ, а не
    контракт:

      · приёмочная проба заказа владельца #602/G16
        (`card_acceptance.run_identity_key_price`, звено 2) объявляет отказом
        ровно схлопывание: «читатель СХЛОПНУЛ день: `load_history` отдала N
        записей против M — вторая копия правила замены жива у читателя»;
      · замер того же заказа: **не менее 25 читателей из 108** схлопывали день
        сами, среди них сам критерий взвода;
      · писатель `append_rationale_history` идемпотентен по ПАРЕ
        ``(cycle_date, run_identity)`` с ADR-395, и «другой прогон того же дня
        ОСТАЁТСЯ»; ключ по одной дате стёр единственный ACT за сорок дней
        (ADR-383) и 206 прогонов на 17 днях (ADR-314).

    Следствие для этой сверки: у дня законно бывает несколько строк, поэтому
    «дат меньше, чем строк» НЕ есть расхождение читателей. Сравнивать надо
    сравнимое — МНОЖЕСТВА ДАТ обоих читателей. Живой замер до поправки:
    57 строк, 47 дат ⇒ флаг `readers_agree` был ложен ПО ПОСТРОЕНИЮ на любой
    истории, где хоть один день имеет два прогона, и статус вырождался в
    UNMEASURED без всякого дефекта данных.
    """
    ...
    # Сравниваем СРАВНИМОЕ: множество дат канонического читателя против
    # множества дат независимой переписи. Прежнее `len(dates) == len(history)`
    # сравнивало даты со строками и было ложно на любом дне с двумя прогонами.
    canon_dates = {str(h.get("cycle_date")) for h in history
                   if isinstance(h, dict) and h.get("cycle_date")}
    agrees = canon_dates == dates and bad == bad_lines
    return {
        "measured": True,
        ...
        # Три числа порознь (решение ARB §2B): сырые строки, канонические дни и
        # строки-повторы дня. Одно число на троих скрывало бы, какое из них
        # разошлось.
        "raw_row_count": len(raw),
        "canonical_day_count": len(canon_dates),
        "duplicate_same_day_count": len(history) - len(canon_dates),
        "readers_agree": agrees,
        "reason": None if agrees else (
            f"канонический читатель дал даты {sorted(canon_dates)[:3]}… ({len(canon_dates)} "
            f"шт.) и {bad_lines} нечитаемых строк, независимая перепись — "
            f"{len(dates)} дат и {bad}: два чтения одного файла спорят, и число "
            f"дней НЕ ИЗМЕРЕНО"),
    }
