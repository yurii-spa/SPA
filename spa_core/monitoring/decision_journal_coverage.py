...
_SANDBOX_FILES = (
    ...
    # Носитель ставок ПО ПРОГОНАМ. Добавлен циклом #555 и это НЕ косметика:
    # потребитель, чей вход в песочницу не попал, отвечает на обоих журналах
    # одинаково — и получает `insensitive`, ВЕРНЫЙ ПО ПОСТРОЕНИЮ. Ровно этот
    # дефект ADR-317 нашёл у соседа и назвал классом #540 наизнанку: не
    # «потребитель, чья цена не измеряется», а вердикт, который не мог выйти
    # другим. Файл копируется, если он есть; отсутствие остаётся отсутствием.
    "apy_composition_log.jsonl",
    HISTORY_FILENAME,
)
...
READERS: Tuple[dict, ...] = (
    ...
    # Прибор заказа #552 (`day_replacement_verdict_loss`, ADR-316) сюда ВНЕСЁН
    # НЕ БЫЛ, и это замер, а не недосмотр: население этого списка — потребители
    # КЛЮЧА `apy_evidenced_pct`, а не читатели журнала вообще (предикат храповика
    # `ReaderPopulationRatchet._production_modules_touching_the_key` спрашивает
    # ровно про ключ). Прибор #552 ключа не касается ни разу (грep = 0 против 3
    # у соседа выше), поэтому его ответ на базовом и на расширенном журнале
    # СОВПАДАЕТ ПО ПОСТРОЕНИЮ, и `probe_readers` выдал бы ему `insensitive` —
    # верное по букве и пустое по существу: «населению карты не подчинён» про
    # того, кто в население и не входил. Замер 10.09 (цикл #554) на живом
    # журнале это подтвердил прямо: строка `day_replacement_verdict_loss →
    # insensitive` появлялась в отчёте, деля знаменатель переписи с настоящими
    # потребителями и стоя́ два прогона прибора в песочнице дневного моста.
    # Это тот самый класс #540, против которого храповик и написан, только
    # с другой стороны: не «потребитель, чья цена не измеряется», а
    # НЕ-потребитель, чья неизмеримость выдана за измеренную нечувствительность.
    #
    # Прибор заказа #554 (`intraday_rate_input_movement`, ADR-318) сюда ВНЕСЁН —
    # и по тому же предикату, по которому сосед выше внесён НЕ был. Он читает
    # `apy_evidenced_pct` как ДАННЫЕ: слой рода величин сверяет ставку носителя
    # со ставкой журнала того же дня (замер 10.09 — 17 сравнимых пар на живом
    # журнале, не ноль). Его собственный счётчик покрытия — `population.scoreable`,
    # пары «день знаменателя × нога»: шире журнал ⇒ шире знаменатель ⇒ больше пар,
    # то есть рост здесь означает ровно то, что означает у соседей, а не то же
    # число из несуществующего населения.
    {"module": "spa_core.monitoring.intraday_rate_input_movement", "probe": "run"},
    # Прибор заказа #586 (`capital_observability_history`, ADR-366) внесён по тому
    # же предикату: он читает `apy_evidenced_pct` как ДАННЫЕ — доля наблюдённого
    # капитала за день считается ИЗ этого ключа (плюс `apy_unevidenced` вторым
    # сигналом), а не рядом с ним. Внесён ЦИКЛОМ #588, который его и доставил, —
    # но не по своей догадке: храповик `ReaderPopulationRatchet` покраснел на
    # полном предписанном прогоне субъекта (21 падение против 20 в одновременном
    # контроле на той же базе, разница ровно в этой строке). Это ровно та работа,
    # ради которой храповик написан, и единственная честная реакция — вписать
    # потребителя в население, а не понизить проверку.
    #
    # Счётчика покрытия он НЕ публикует, и это РЕШЕНИЕ, проверенное пробой, а не
    # недосмотр. Первая правка #588 объявила таким счётчиком `population.measured_days`
    # (дни журнала, которые прибор сумел оценить) — и проба на живом журнале показала,
    # что он отвечает НЕ НА ТОТ вопрос: расширение карты добавляет НАСЕЛЕНИЕ ВНУТРЬ
    # дня, а не новые дни, поэтому счётчик стоял 39 → 39, ответ при этом менялся
    # (`answer`, `denominator`, `findings`), и прибор печатал `moved_without_coverage`,
    # то есть ОБВИНЕНИЕ В РЕГРЕССИИ там, где потребитель работает как задуман.
    # Причина сдвига у него иная, чем у соседей: его ответ есть утверждение О
    # ЗНАМЕНАТЕЛЕ, который выбирает `shadow_trigger_eval.scored_days`; меняется
    # СОСТАВ дней знаменателя — обязан измениться и ответ. Такой оси у прибора нет,
    # и выдавать за неё счётчик, нечувствительный к ширине карты, нельзя: это ровно
    # класс #540 («нечувствительность из несуществующего населения»), только с
    # обратным знаком — ложная регрессия вместо ложного покрытия. Поэтому третий
    # исход `unmeasured` с названной причиной, как у четверых соседей выше.
    {"module": "spa_core.monitoring.capital_observability_history", "probe": "run"},
    # Прибор заказа #587/G6 (`unobserved_turnover_dependence`, ADR-368) внесён по тому
    # же предикату: он читает журнал решений как ДАННЫЕ — ход каждой ноги берётся из
    # разности `current_positions`/`target_positions` и из поля `legs` той же записи.
    #
    # Счётчика покрытия он НЕ публикует, и это РЕШЕНИЕ, а не недосмотр — причина та же,
    # по которой его нет у соседа выше, и проверена она тем же способом. Население его
    # замера выбирает СУДЬЯ (`per_verdict` → отказы по `unpriced_protocols`), а ход ног
    # берётся из разности КНИГ. Карта `apy_evidenced_pct` шире или уже — население не
    # меняется ни на пару: она лишь ПЕРЕМЕЧИВАЕТ ноги, которые уже в замере, и любой
    # счётчик «сколько пар удалось оценить» стоял бы на месте при движущемся ответе.
    # Это ровно класс #540 с обратным знаком (ложная РЕГРЕССИЯ вместо ложного
