# ПАРАЛЛЕЛЬНАЯ реализация ADR-066 C1/C2 (цикл #125, 2026-08-06) — СОХРАНЕНА, НЕ ПОДКЛЮЧЕНА.
#
# В ночь на 06.08 одну и ту же Фазу 3 сделали две независимые сессии. Победила не «лучшая», а
# РАЗВЁРНУТАЯ: `com.spa.decision_loop` уже работает в проде каждые 6ч и владеет каноническими
# именами `spa_core/monitoring/house_view_gap.py`, `findings_bridge.py` и файлом
# `data/house_view_gap.json`. Трогать живой контур ради своей версии я не стал — деплой и
# выгрузка агентов owner-gated.
#
# Эта версия переименована и разведена ПО ФАЙЛАМ ВЫВОДА (`*_c125.json`), чтобы два писателя
# никогда не спорили за один артефакт. Она НИКЕМ не вызывается: ни цикл, ни агент её не зовут.
# Живёт ради (а) сохранности работы и (б) 59 своих тестов, которые продолжают её проверять.
# Что оставить насовсем — решает владелец: карточка
# `owner-decision-nochyu-odnu-zadachu-sdelali-dvazhdy-moya`. Ничего чужого не удалено.
#!/usr/bin/env python3
...
STATE_REL = os.path.join("data", "findings_bridge_c125_state.json")
REPORT_REL = os.path.join("data", "findings_bridge_c125.json")

# Источники находок: id → относительный путь отчёта.
SOURCES = {
    "arch": os.path.join("data", "architecture_conformance.json"),
    "hvg": os.path.join("data", "house_view_gap_c125.json"),
}
SOURCE_TITLES = {
