...
#: значения НЕ снимаются, хотя корень тот же (сырые байты сравниваются как
#: разобранный YAML): замер того же дня дал НОЛЬ расхождений вида ``1`` против
#: ``"1"``, а снятие кавычек могло бы склеить два разных ответа — то есть
#: заглушить сторожа там, где он прав. Пустота может только УБРАТЬ призрачный
#: ответ, склеить два настоящих она не способна.
