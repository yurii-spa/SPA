...
от него **только строками следа**: добавленной строкой с ключом из
`OWNER_ANSWER_FIELDS` — либо (ADR-188) заменой строки, где тот же ключ написан на
origin ПУСТЫМ скаляром, то есть не несёт решения владельца. Непустое значение на
origin не заменяется никогда. Доказательство независимое
(:func:`verify_trace_only`), а не «доверьтесь конструктору»: кандидат разбирается
заново и сверяется с remote построчно. Не сошлось — ОТКАЗ по этой карточке с
названной причиной.
...
def empty_scalar_keys(blob: bytes, keys) -> set:
    """Ключи, чья СТРОКА во frontmatter есть, а значение — объявленное отсутствие.

    Ровно разность между :func:`present_keys` и :func:`_read_fields`, названная
    своим именем. Это и есть та форма, из-за которой перенос ломался: строка
    занята, ответа в ней нет, а дописать рядом вторую нельзя.
    """
    parts = card_parts(blob)
    if parts is None:
        return set()
...
def key_occurrences(blob: bytes, key: str) -> int:
    """Сколько РАЗ ключ написан верхнеуровневой строкой frontmatter.

    Отдельный вопрос от «есть ли ключ»: читатель следа берёт ПЕРВОЕ вхождение,
    поэтому два вхождения одного ключа — не избыточность, а невидимый ответ
    владельца (авария 30.08). Спрашивается и про origin (можно ли заменять), и
    про кандидата (не создали ли мы второй сами).
    """
    parts = card_parts(blob)
    if parts is None:
        return 0
    return len(_field_re(key).findall(parts[0]))


def empty_scalar_replacement(r_line: bytes, c_line: bytes, allowed: set) -> bool:
    """Законна ли замена строки origin ``r_line`` строкой кандидата ``c_line``.

    Замена законна РОВНО когда: ключ один и тот же, он из следа ответа владельца,
    и на origin у него объявленное ОТСУТСТВИЕ значения. Любое иное отличие — не
    замена, а затирание строки origin; непустое значение на origin не заменяется
    никогда, это единственное, чего переносу нельзя (ответ владельца выбирает
    владелец).

    Что строка КАНДИДАТА несёт значение, здесь НЕ проверяется, и это осознанно:
    утверждение «читатель следа видит поле» живёт одной проверкой в
    :func:`verify_trace_only`. Стояв оно и тут, две проверки закрыли бы одну ось,
    и ни одну из них нельзя было бы покрасить в одиночку — мутация снимала бы
    любую, а набор оставался зелёным (замер цикла #438: обе выжили).
    """
    # Отдельного запрета на строку С ОТСТУПОМ здесь нет, и это не упущение:
    # у вложенной строки ключ приходит вместе с отступом (`b"  owner_choice"`) и в
    # `allowed` не попадает никогда. Условие, которое нечем покрасить, — украшение,
    # и оно же прячет, что настоящую границу держит сверка ключа (замер #438:
    # мутация «снять отступ» оставляла набор зелёным).
    if b":" not in r_line or b":" not in c_line:
        return False
    r_key, r_val = r_line.split(b":", 1)
    c_key, c_val = c_line.split(b":", 1)
    if r_key != c_key or r_key not in allowed:
        return False
    r_v = r_val.strip().decode("utf-8", "replace")
    if r_v and r_v not in EMPTY_SCALARS:
        return False
    return True
...
def merge_trace(local: bytes, remote: bytes) -> tuple:
    ...
    # Замер 30.08 (цикл #429). `theirs` не видит поля, написанного ПУСТЫМ скаляром
    # (`owner_choice: ""` = «владелец ещё не ответил», и это верно), но СТРОКА в
    # файле есть. Дописав рядом вторую с тем же ключом, мы получили бы frontmatter
    # с двумя `owner_choice`, читатель берёт ПЕРВЫЙ — пустой, — и ответ владельца,
    # уехавший на origin, становится НЕВИДИМ. Хуже прежнего: раньше его просто не
    # было, теперь он есть и не читается.
    #
    # ADR-176 закрыл это ОТКАЗОМ, и отказ был верен как первая помощь, но ценой:
    # ответ владельца по такой карточке не уезжал в git НИКОГДА и сторож повторял
    # ту же причину каждый прогон. ADR-188 чинит класс: строка с пустым скаляром
    # ЗАМЕНЯЕТСЯ на месте. Пустой скаляр не несёт решения владельца — заменить его
    # нечем затереть; непустое значение не заменяется ни при каких условиях, и это
    # проверяет независимое доказательство (`verify_trace_only`), а не намерение
    # конструктора.
    #
    ...
    replace = sorted(k for k in added if k in present_keys(remote, added))
    # Ключ, УЖЕ написанный на origin дважды, — испорченный файл (пункт 2 карточки
    # `inbox-perenos-sleda-umeet-tolko-dopisyvat-pust`). Схлопывать вхождения можно
    # только когда непустое ровно одно и совпадает с нашим; замер по origin/main
    # 31.08 (841 карточка) дал таких файлов НОЛЬ, поэтому здесь fail-CLOSED с
    # названной причиной, а не догадка о форме, которой в природе нет.
    multi = sorted(k for k in replace if key_occurrences(remote, k) > 1)
    if multi:
        return None, (f"на origin ключ написан НЕСКОЛЬКО раз ({', '.join(multi)}) — "
                      "какое вхождение несёт ответ владельца, отсюда не видно; "
                      "схлопывать вхождения этот перенос не умеет. Чинить руками"), {}

    r_fm, r_body = rp
    # Строки с пустым скаляром заменяем НА МЕСТЕ (порядок и окружение frontmatter
    # остаются origin'ными), остальное дописываем в КОНЕЦ в стабильном порядке
    # CARRIED_FIELDS — словарный порядок сделал бы содержимое коммита зависящим от
    # порядка чтения.
    if replace:
        out_lines = []
        for line in r_fm.splitlines(True):
            ...
            if not line[:1].isspace() and key in replace:
                out_lines.append(f"{key}: {added[key]}\n".encode())
            else:
                out_lines.append(line)
        r_fm = b"".join(out_lines)
    lines = b"".join(f"{k}: {added[k]}\n".encode()
                     for k in CARRIED_FIELDS if k in added and k not in replace)
...
def verify_trace_only(remote: bytes, candidate: bytes) -> tuple:
    ...
    Допускаются ровно два вида отличий (ADR-188):

    ``добавленная строка``  ключ из :data:`CARRIED_FIELDS`, которого на origin нет;
    ``заменённая строка``   тот же ключ из следа, на origin — ПУСТОЙ скаляр
                            (:func:`empty_scalar_replacement`).

    Сверх этого — две проверки исхода, а не формы, и обе про ровно ту аварию,
    которой класс и был: КАЖДАЯ написанная нами строка обязана читаться читателем
    следа, и ни один ключ следа не смеет оказаться во frontmatter кандидата
    дважды — читатель берёт первое вхождение, и вторая строка была бы невидимым
    ответом владельца.
    """
    rp, cp = card_parts(remote), card_parts(candidate)
    if rp is None or cp is None:
    ...
    for line in c_lines:
        if i < len(r_lines) and line == r_lines[i]:
            i += 1
            continue
        if i < len(r_lines) and empty_scalar_replacement(r_lines[i], line, allowed):
            replaced.append(line)
            i += 1
            continue
        extra.append(line)
    if i != len(r_lines):
        return False, ("строки frontmatter с origin изменены или пропали — допускается "
                       "только добавление строки следа и замена ПУСТОГО скаляра следа")
    for line in extra:
    ...
    # Каждая строка, которую мы написали (и добавленная, и заменившая), обязана
    # ЧИТАТЬСЯ читателем следа. Проверка исхода, а не формы: авария 30.08 была
    # ровно в том, что строка есть, а читатель её не видит. Форма «дописали пустое
    # поле, которого на origin не было» проходит проверку ключей насквозь — ключ
    # законный, строка добавлена, — и без этой проверки перенос объявил бы
    # доставленным то, чего не видно.
    for line in extra + replaced:
        key = line.split(b":", 1)[0].decode("utf-8", "replace")
        if key not in _read_fields(candidate, (key,)):
            return False, (f"строка {key} не читается читателем следа — ровно та "
                           f"авария, против которой доказательство и стоит")
    for key in CARRIED_FIELDS:
        if key_occurrences(candidate, key) > 1:
            return False, (f"во frontmatter кандидата ключ {key} написан дважды — "
                           f"читатель следа взял бы ПЕРВОЕ вхождение, и ответ "
                           f"владельца стал бы невидим")
    return True, ""
