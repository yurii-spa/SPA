...
_TVL_ESTIMATES: dict[str, float] = {
    ...
    # "frax": 100_000_000.0 — УДАЛЕНО (ADR-070 п.17). Ключ снят с канонического
    # реестра; литерал был опасен сам по себе: $100M — ВЫШЕ порога $5M, то есть
    # константа проходила floor тавтологически (ADR-053 запрещает ровно это).
    # Без записи ключ получает tvl=0.0/"static" и честно НЕ проходит порог.
    ...
    # "notional_v3": 50_000_000.0 — УДАЛЕНО (ADR-070 п.18): ключ ВЫВЕДЕН, а у
    # выведенного не должно остаться константы, способной перекрыть порог $5M.
}
