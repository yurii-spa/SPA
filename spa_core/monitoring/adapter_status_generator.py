...
_POOL_ID_LOOKUP: dict[str, str] = {
    # ── ADR-398: две крупнейшие книги — Ethereum USDC (замер живого фида 2026-09-16) ──
    # Эти два ключа разрешались ХИНТОМ, и разрешались ВЕРНО: `_CANONICAL_UNDERLYING`
    # отсекает чужой актив, и победителем хинта сегодня выходит ровно тот пул, что
    # закреплён ниже. Дефект был не в тождестве, а в ПРОВЕНАНСЕ: хинту запрещено
    # штамповать TVL `live`, поэтому второй артефакт нёс литерал `_TVL_ESTIMATES` —
    # $12B у `aave_v3` против $58.3M наблюдённых у оркестратора (×205.9) и $3B у
    # `compound_v3` против $35.5M (×84.4). Гейт финансирования этим не обманут (он
    # судит по снимку оркестратора), но второй артефакт читают шесть потребителей,
    # включая отчёт владельцу: литерал доезжал до отчёта там, где наблюдение лежало
    # рядом. Замер `scripts/capital_observability_census.py` на живом каталоге 16.09:
    # «СПОР АРТЕФАКТОВ» на $45 000 — 47.4 % развёрнутого капитала.
    #
    # Пин здесь — единственный выход, не удобство: по контракту
    # `test_tvl_pinned_provenance` наблюдением может называться ТОЛЬКО закреплённый
    # UUID, потому что «побеждает больший TVL» есть тождество, способное молча уехать
    # на другой пул. Альтернатива «писать null вместо литерала» честнее нынешнего, но
    # спор со вторым артефактом не снимает: ненаблюдённое остаётся ненаблюдённым.
    #
    # Ethereum / aave-v3 / USDC — $190.4M @ 3.567 % (вся ставка `apyBase`), underlying
    # РОВНО 0xa0b8…eb48 (нативный Circle USDC). Ближайший промах, который допускает
    # подстрочный хинт, — `21e1ac8a…` USDE $548M @ 4.75 % (из них 4.09 пп — раздача
    # токена): ставка ВЫШЕ, актив ДРУГОЙ. Оценка в `_TVL_ESTIMATES` говорила $12B — ×63.
    "aave_v3":           "aa70268e-4b52-42bf-a116-608b370f9501",
    # Ethereum / compound-v3 / USDC — $38.7M @ 3.226 %, underlying 0xa0b8…eb48.
    # Оценка говорила $3B — ×77. Соседние рынки того же проекта крупнее (WBTC $364.8M,
    # WSTETH $149.3M), и «побеждает больший TVL» удерживается здесь только тем, что
    # символ отфильтрован хинтом; пин снимает и эту зависимость.
    "compound_v3":       "7da72d09-56ca-4ec5-a45f-59114353e487",
