...
============================================
Arbitrum One gas price monitor (multichain expansion).

Kill-switch: Arbitrum gas > ARBITRUM_GAS_THRESHOLD_GWEI for 3+ consecutive days
→ advisory signal to reduce Arbitrum allocation. Arbitrum L2 fees are typically
0.01–0.1 Gwei (very low); a sustained spike implies sequencer/L1 congestion.

Data sources (no API key required):
  1. BlockNative Gas Estimator (chainid=42161 = Arbitrum One)
  2. Infura free gas endpoint (networks/42161)
  3. Fallback: conservative constant (Arbitrum is typically 0.01–0.1 Gwei)

Data file: data/arbitrum_gas_history.json

Rules:
  - STDLIB ONLY — no external dependencies
  - READ-ONLY (monitoring/advisory) — only writes to data/arbitrum_gas_history.json
...
CLI:
  python3 -m spa_core.monitoring.arbitrum_gas_monitor [--check | --run]
  --check  (default) compute and print, no write
  --run    compute + atomic write to data/arbitrum_gas_history.json
"""
...
ARBITRUM_GAS_THRESHOLD_GWEI: float = 2.0   # advisory kill-switch threshold (L2)
ARBITRUM_GAS_KILL_DAYS: int = 3            # consecutive days above threshold
MAX_HISTORY_DAYS: int = 30                 # ring-buffer size
HTTP_TIMEOUT_SEC: float = 5.0

# Typical Arbitrum gas is 0.01–0.1 Gwei; 0.1 is a safe conservative fallback
FALLBACK_GWEI: float = 0.1

GAS_APIS: List[str] = [
    "https://api.blocknative.com/gasprices/blockprices?chainid=42161",
    "https://gas.api.infura.io/networks/42161/suggestedGasFees",
]


class ArbitrumGasMonitor:
    """Monitors Arbitrum One gas prices and emits an advisory kill-switch."""

    DATA_FILENAME = "arbitrum_gas_history.json"
    CHAIN = "arbitrum"
    ...
    def get_current_gas_gwei(self) -> float:
        """Fetch current Arbitrum gas in Gwei; FALLBACK_GWEI if all sources fail."""
        for url in GAS_APIS:
            try:
    ...
    def record_reading(
        ...
        *,
        ...
        above = gwei > ARBITRUM_GAS_THRESHOLD_GWEI
        readings.append(
        ...
        kill_switch_active = consecutive_above >= ARBITRUM_GAS_KILL_DAYS
        ...
        return {
            ...
            "action": action,
            "date": today_str,
            "threshold_gwei": ARBITRUM_GAS_THRESHOLD_GWEI,
        }
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "threshold_gwei": ARBITRUM_GAS_THRESHOLD_GWEI,
            "kill_days": ARBITRUM_GAS_KILL_DAYS,
        }
...
def main(argv: Optional[List[str]] = None) -> None:
    if argv is None:
        argv = sys.argv[1:]
    monitor = ArbitrumGasMonitor(data_dir=_repo_root() / "data")
    write = "--run" in argv
    ...
    print("\n[ArbitrumGasMonitor] Gas Check (chainid=42161)")
    print(f"  Date:               {result['date']}")
    print(f"  Gas (Gwei):         {result['gwei']}")
    print(f"  Threshold (Gwei):   {result['threshold_gwei']}")
    print(f"  Consecutive above:  {result['consecutive_above']} / {ARBITRUM_GAS_KILL_DAYS} days")
