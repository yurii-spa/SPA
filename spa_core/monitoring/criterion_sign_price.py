...
**Ноль стоимости через данные НЕДОСТИЖИМ, и это факт о судье, а не о приборе.**
``cost_usd = 0`` читается судьёй как «записи нет» и заменяется допущением
``ASSUMED_COST_BPS_OF_TURNOVER`` — пол fail-CLOSED. Поэтому вариант «ход бесплатен»
не докладывается вовсе, а вариант «запись стоимости убрана» докладывается как то,
чем он является: переход с записанной цены на допущение.
...
def measure(data_dir: Path, *, now: Optional[datetime] = None,
    ...
    return {
        ...
        "what_it_does_not_prove": [
            ...
            "«ход бесплатен» не измеряется вовсе: `cost_usd = 0` читается судьёй "
            "как «записи нет» и заменяется допущением — это пол, а не ноль",
        ],
    }
