...
#: Итоговая строка pytest и число прошедших в ней.
...
def passed_count(output: str) -> Optional[int]:
    ...
    «passed» в выводе хватает где угодно, а сводка одна. Сводка есть, а
    «passed» в ней нет ⇒ прошло НОЛЬ (так выглядит целиком пропущенный файл);
    сводки нет вовсе ⇒ ``None``, и это третий исход, а не ноль.
    """
    summary = None
