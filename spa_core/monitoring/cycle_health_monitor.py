...
class CycleHealthMonitor:
    ...
    def check_evidence_matches_curve(self, data_dir: str = "data") -> dict[str, Any]:
        ...
        Механизм (замерен прогоном 2026-08-17, не выведен чтением). Прежняя версия
        «кривую пишут ДВА пути, и в день остановки они берут «вчера» из разных
        источников» ОПРОВЕРГНУТА и убрана отсюда: путь остановки (`equity._write_equity`)
        зовёт тот же `_upsert_equity_point`, «вчера» у него одно. Настоящая причина —
        РАЗНАЯ семантика повторной записи за один и тот же день:

          * кривая (`equity.py:240`) сбрасывает сегодняшний бар и пересчитывает его
            от вчерашнего закрытия ⇒ выигрывает ПОСЛЕДНИЙ прогон дня;
          * доказательная база (`paper_evidence_tracker.py:119`) на дубликат даты
            возвращает уже записанную строку и НЕ обновляет её ⇒ выигрывает ПЕРВЫЙ
            прогон дня.

        Один прогон в сутки ⇒ числа совпадают; два и больше ⇒ расходятся всегда, на
        величину накопления между прогонами (замер: −$4.11 при двух обычных прогонах,
        +$8.77 когда после обычного прогона день переписал halt-прогон — отсюда оба
        знака дельты). Июньский блок до $215.99 — ОТДЕЛЬНЫЙ дефект: в самой кривой
        один разрыв цепочки, 2026-06-10 `open_equity=100000.00` против вчерашнего
        закрытия 100216.00 (бар записан с `daily_return_pct=0.0`).

        Починка обеих причин переписывает записанные значения трека ⇒ money-path,
        решение владельца (own-32), не правка на ходу.
