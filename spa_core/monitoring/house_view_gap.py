...
Типы расхождений:
  opportunity_unheld  офис называет возможность (evidence-level сохранён),
                      книга её не держит, и отказ НЕ назван нигде:
                        ...
                        - `cash.excess_pct` ИЗМЕРЕН и равен нулю   → INFO
                          (`gap:opportunity_no_idle_capital`): свободных денег нет,
                          вход требовал бы ПРОДАЖИ — простоя, о котором говорит
                          ADR-055, не существует, и слово «простой» было бы ложью
                          о факте. Не измерен ⇒ права снять находку это НЕ даёт
                        - иначе                                   → WARN (безымянный
                          простой возможности — нарушение духа ADR-055), и находка
                          НАЗЫВАЕТ все четыре опрошенных регистра И измеренный
                          простой: «не назван» — это ИЗМЕРЕНИЕ по поимённому списку,
                          а не общее место
...
#: ПЯТЫЙ регистр — и единственный, который отвечает на вопрос, ради которого WARN и писался.
#: Четыре регистра выше спрашивают «НАЗВАН ли отказ по этому протоколу». Слово «простой» в
#: тексте находки утверждает ДРУГОЕ: что деньги стоят без дела. ADR-055 обязывает объяснять
#: кэш СВЕРХ буфера — и ровно эту величину аллокатор публикует в том же файле, в том же
#: цикле: `cash.excess_pct`. Замер 2026-09-01 (цикл #449): `excess_pct = 0.0`,
#: `status = "explained"`, книга размещена на 95 % при мандатном буфере 5 % — свободных денег
#: НОЛЬ, а сторож четвёртый раз подряд кричал про `spark_susds` (4.01 % против 5.42 % у
#: книги) «безымянный простой». Простоя не было: вход туда требовал бы ПРОДАЖИ более
#: доходной позиции — это вопрос предпочтения, а не необъяснённого простоя, и ADR-055 сам
#: велит концентрации идти за доходностью.
#:
#: Тот же класс, что #394 (третий регистр) и #418 (четвёртый): честный ответ на СВОЙ вопрос,
#: прочитанный как ответ на нужный. Цена измерена — 10 карточек моста этого класса закрыто,
#: три из них про тот же `spark_susds`, и находка вернулась снова.
KEY_NO_IDLE_CAPITAL = "gap:opportunity_no_idle_capital"

#: Ниже этого модуля порога считаем `excess_pct` нулём: доли процента свободных денег —
#: не простой, а округление. Порог тот же, что у самого аллокатора
#: (`rebalance_economics.py:413`, `excess_pct <= 1e-6`), и держится тестом: два числа на
#: одной оси, разъехавшись, дали бы сторожу и подопечному разные ответы о ОДНОМ состоянии.
IDLE_EPS_PCT = 1e-6


def idle_excess_pct(rationale):
    """`cash.excess_pct` из rationale → (значение, причина-почему-нет). Три исхода, инв. #17.

    Возврат ``(None, "<причина>")`` означает НЕ ИЗМЕРЕНО — и тогда утверждать «простоя нет»
    запрещено ровно так же, как запрещено утверждать «отказ не назван», не прочитав ни
    одного регистра. Возврат ``(0.0, "")`` — ИЗМЕРЕННЫЙ ноль: свободных денег нет.
    """
    if not isinstance(rationale, dict) or not rationale:
        return None, "rationale недоступен"
    cash = rationale.get("cash")
    if not isinstance(cash, dict):
        return None, "в rationale нет раздела cash"
    if "excess_pct" not in cash:
        return None, "в cash нет поля excess_pct"
    raw = cash.get("excess_pct")
    if isinstance(raw, bool) or not isinstance(raw, (int, float)):
        return None, f"cash.excess_pct не число ({type(raw).__name__})"
    value = float(raw)
    if value != value or value in (float("inf"), float("-inf")):   # NaN / ±inf
        return None, "cash.excess_pct не конечное число"
    return value, ""
...
def compute_gaps(chief: dict | None,
    ...
    # Пятый регистр читается ОДИН раз на всю сверку и НЕ зависит от протокола: вопрос
    # «есть ли вообще свободные деньги» — про книгу целиком, а не про имя возможности.
    # Держим и значение, и ПРИЧИНУ его отсутствия: «не измерено» без причины неотличимо
    # от измеренного нуля, а это ровно те два исхода, которые здесь нельзя склеивать.
    idle_pct, idle_why = idle_excess_pct(rationale)
    ...
    # Запись о НЕизмеренном простое идёт ПОСЛЕ записей о регистрах — намеренно. Читатель
    # (`test_house_view_gap_refusal_unmeasured`) берёт ПЕРВУЮ запись про `allocation_rationale`
    # и ждёт там ответ про РЕГИСТРЫ; встав раньше, эта строка молча ответила бы на другой
    # вопрос — ровно тот класс подмены, ради которого весь этот модуль и правится.
    if idle_pct is None:
        unchecked.append({"input": "allocation_rationale",
                          "reason": f"простой капитала не измерен ({idle_why}) — "
                                    f"утверждать «простоя нет» нельзя, находки идут прежним путём"})
            ...
            if proto in explained_protocols or proto in blob:
                ...
                gaps.append({"key": f"{KEY_REFUSAL_UNMEASURED}:{proto}",
                             ...
                             "message": f"возможность {proto} {v.get('apy_pct')}% "
                                        ...
                                        f"({REGISTERS_RU}) — назван отказ или нет, НЕ ИЗМЕРЕНО "
                                        f"[{ticks}]", **base})
            elif idle_pct is not None and idle_pct <= IDLE_EPS_PCT:
                # ИЗМЕРЕННЫЙ ноль свободных денег (пятый регистр, `cash.excess_pct`).
                # Слово «простой» здесь было бы ложью о ФАКТЕ, а не строгостью: книга
                # размещена, кэш равен мандатному буферу, и вход в эту возможность
                # требовал бы ПРОДАЖИ уже стоящей позиции. Это вопрос предпочтения
                # (им ведает тюнер и триггер ADR-060), а не необъяснённого простоя.
                #
                # Степень понижена, но находка НЕ ИСЧЕЗАЕТ и ключ ДРУГОЙ: мост узнаёт
                # находку по ключу, и молчание здесь скрыло бы от читателя, что офис
                # называет возможность, которой у книги нет денег взять. Названы ОБА
                # числа — и опрошенные регистры, и измеренный простой: отрицательное
                # утверждение без списка опрошенных дважды подряд оказывалось ложным.
                read_ru = " · ".join(registers_read) or "ни одного"
                gaps.append({"key": f"{KEY_NO_IDLE_CAPITAL}:{proto}",
                             "type": "opportunity_unheld", "severity": "INFO",
                             "registers_read": list(registers_read),
                             "cash_excess_pct": idle_pct,
                             "message": f"возможность {proto} {v.get('apy_pct')}% "
                                        f"(evidence {opp.get('evidence_level')}) не в книге, и "
                                        f"отказ не назван в прочитанных регистрах ({read_ru}); "
                                        f"но ПРОСТОЯ НЕТ — аллокатор измерил свободных сверх "
                                        f"буфера {idle_pct:.2f} %, вход требовал бы ПРОДАЖИ, а "
                                        f"не размещения кэша (ADR-055 велит концентрации идти "
                                        f"за доходностью) [{ticks}]", **base})
            else:
                ...
                #
                # Про ПРОСТОЙ здесь утверждать можно ровно в двух состояниях: свободные
                # деньги измерены и их больше нуля, либо не измерены вовсе. Второе —
                # fail-CLOSED: «не измерено» не даёт права ни утверждать простой, ни
                # снимать находку, и потому идёт прежним путём (WARN), но с НАЗВАННОЙ
                # причиной, почему пятый регистр промолчал.
                ...
                idle_ru = (f"свободных сверх буфера {idle_pct:.2f} %"
                           if idle_pct is not None
                           else f"простой НЕ ИЗМЕРЕН ({idle_why})")
                gaps.append({"key": f"gap:opportunity_unnamed:{proto}",
                             ...
                             "message": f"возможность {proto} {v.get('apy_pct')}% "
                                        ...
                                        f"простой (дух ADR-055; {idle_ru}) [{ticks}]", **base})
    else:
