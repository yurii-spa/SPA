...
def registry_protocol_keys() -> set[str] | None:
    ...
    ВЫБОР НАБОРА ЯВНЫЙ: берём КАНОН `spa_core.adapters.ADAPTER_REGISTRY` (36 записей,
    список кортежей, ключ крупнейшей позиции книги `aave_v3`) — это вселенная выбора
    аллокатора. Соседний `spa_core.adapters.registry.ADAPTER_METADATA` (22, dict) сюда
    НЕ годится: `aave_v3` там отсутствует вовсе (она зовётся `aave_usdc`), и сверка
    «книга ↔ реестр» по нему честно доложила бы, что 40 % книги не зарегистрировано.

    С цикла #274 имя `ADAPTER_REGISTRY` определено в дереве РОВНО ОДИН раз, и это
    держит храповик `spa_core/tests/test_adapter_registry_single_name.py` — то есть
    форма здесь уже гарантирована. Разбор формы ниже оставлен намеренно как ремень
    поверх подтяжек — он читает форму, а не предполагает её; тот же приём — в
