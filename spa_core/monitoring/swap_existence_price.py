...
def _charge_swap_only(relief_by_date: Dict[str, float],
    ...
    Цена, упавшая до НУЛЯ, не записывается НИКОГДА: у судьи ноль и отсутствие —
    одна ветка (``cost_rec > 0.0``), и записанный ноль обернулся бы ЗАВЫШЕНИЕМ
    цены до ``ASSUMED_COST_BPS_OF_TURNOVER``. Такой день называется в ``refused``
...
def zero_is_absent(data_dir: Path, *, horizon_days: int,
    ...
    Инвариант #16 CLAUDE.md требует, чтобы отсутствие наблюдения имело СВОЁ
    представление. У судьи обе величины проходят одной веткой ``cost_rec > 0.0``,
    поэтому бесплатный ход оценивается ДОРОЖЕ дешёвого: ноль уводит день в
    ``ASSUMED_COST_BPS_OF_TURNOVER``. Замер печатает оба счёта рядом — утверждение
    о ветке, сделанное чтением кода, здесь подтверждается ИСХОДОМ.
    """
    csp = _neighbour()
