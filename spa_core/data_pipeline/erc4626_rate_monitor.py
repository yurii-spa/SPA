...
def derive_apy_pct(history: list) -> Optional[float]:
    ...
    # НУЛЕВОЙ РОСТ — это «не измерено», а НЕ «доходность 0 %».
    #
    # Измерено 2026-08-07: производитель набрал вторую точку, но обе имели
    # ОДИНАКОВУЮ цену доли (stusd 1.1617319305415383, wusdm 1.0816726646961985) —
    # за сутки хранилище не выросло в пределах точности числа. Расчёт честно
    # вывел 0.0, это число ушло в live_apy КАК НАБЛЮДЕНИЕ, монитор увидел два
    # адаптера с доходностью вне вменяемого диапазона, и оценка здоровья
    # портфеля просела 74.7 → 69.43, ниже критического порога. Ложное критичное.
    #
    # Защиты были от близких замеров (12ч) и от абсурдных значений (−5…60 %), но
    # НЕ от нулевого роста. А нулевой рост у стейблкоин-хранилища за сутки
    # означает не «ставка равна нулю», а «разрешения не хватило, чтобы измерить».
    # Разница принципиальная: «не измерено» ≠ «измерено и равно нулю», и путать
    # их — тот самый класс, который в этом проекте вычищается везде.
    #
    # Ставка появится, когда накопится ИЗМЕРИМОЕ изменение — ряд для этого и
    # копится, точки не теряются.
    if p1 == p0:
        log.info(
            "цена доли не изменилась (%.18f) — рост неизмерим, ставка НЕ выводится",
            p1,
        )
        return None
...
def _apy_note(history: list, apy: Optional[float]) -> Optional[str]:
    """Почему ставки нет — своими словами, а не ближайшей похожей причиной."""
    if apy is not None:
        return None
    pts = [p for p in (history or []) if isinstance(p, dict)]
    if len(pts) < 2:
        return "нужно ≥2 наблюдения с интервалом ≥%.0fч; сейчас %d" % (
            _MIN_HOURS_BETWEEN, len(pts))
    first, last = pts[0].get("share_price"), pts[-1].get("share_price")
    if first == last:
        return ("цена доли не изменилась (%s) — рост неизмерим; ставка появится, "
                "когда накопится измеримое изменение" % last)
    return "рост есть, но выведенная ставка вне диапазона вменяемости — отказ"
...
def observe(data_dir: Optional[Path] = None, now: Optional[datetime] = None) -> dict:
    ...
    for key, meta in VAULTS.items():
        ...
        vaults[key] = {
            ...
            # Причина отказа должна называть СЕБЯ, а не ближайшее правдоподобное
            # объяснение: при двух точках с одинаковой ценой доли текст «нужно
            # ≥2 наблюдения; сейчас 2» противоречил сам себе и увёл бы читателя
            # искать не там.
            "apy_note": _apy_note(history, apy),
            "history": history,
        }
