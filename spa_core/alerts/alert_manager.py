...
def already_sent_today(key: str) -> bool:
    """Публичный вид на подавление по «уже отправлено сегодня».

    Нужен потому, что `send_*` возвращает `False` И при подавлении, И при настоящем
    провале — эти два исхода неразличимы для вызывающего. Замер 10.08: цикл печатал
    четыре `FAILED` подряд, хотя рассылки были просто повторными, и читать это как
    поломку было нельзя. Настоящий отказ отправки при этом выглядел бы точно так же —
    то есть тонул бы среди штатных.

    Менять тип возврата `send_*` не стали: у него много вызывающих, и такая правка
    сломала бы их молча. Отдельный вопрос отвечает на отдельный вопрос.
    """
    return _already_sent_today(key)
