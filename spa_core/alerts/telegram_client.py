...
def _record_history(text: str, ok: bool, message_id=None, error: str | None = None,
    ...
    try:
        ...
        try:
            ...
            pass
        # СВОЙСТВО СООБЩЕНИЯ СТАРШЕ СЛОВ ОТПРАВИТЕЛЯ (жалоба владельца 17.08:
        # «пишут раз по 200 в день одно и то же»). `solicited` приходит параметром, а
        # параметр можно не передать — и тогда `send_message` считает вопрос ответом на
        # команду владельца. Дальше петля замыкается сама: `_duplicate_recently` берёт в
        # кандидаты только НЕ солиситированные записи, поэтому поток помечает сам себя
        # невидимым для дедупа, который обязан был его остановить. Воспроизведено:
        # шесть побуквенно одинаковых вопросов подряд — шесть отправок, ноль кандидатов.
        #
        # Сообщение, ПРЕДЛАГАЮЩЕЕ ВЫБОР, ответом на команду быть не может по построению:
        # это вопрос владельцу, которого он не задавал. Поэтому здесь мы не верим
        # вызывающему, а СМОТРИМ на текст — тем же измерением, что уже стоит в журнале
        # (`offers_choice`, цикл #229).
        #
        # Почему это НЕ глушит ответы владельцу: `solicited` влияет ТОЛЬКО на состав
        # кандидатов дедупа, а сам дедуп спрашивается лишь при `dedup=True`. Ответ на
        # `/status` уходит с `dedup=False` и не подавляется ничем, как бы он ни был
        # записан.
        if solicited and not entry.get("offers_choice"):
