...
#
#   ОСОЗНАННЫЙ ЗАПАС ребалансера — СНЯТ 2026-08-26 (ADR-144). Владелец кнопкой
#   «Зеркалить политику» отменил своё же решение 25.08 «запас оставить»: два источника
#   лимитов — не двойная страховка, а гарантированный разъезд (ADR-060 §1.4). Бывшие
#   запасы (per_protocol_max 25%, t2_max 45%, cash_min 7%, max_protocols 7) больше не
#   существуют — поля берутся из RiskConfig. Формула min() жива: вернуть запас можно
#   одним полем, но ужесточение с этого момента — через ПОЛИТИКУ и отдельный ADR,
#   а не тихим числом здесь.
