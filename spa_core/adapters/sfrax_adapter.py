"""Frax Staked FRAX (sFRAX) ERC-4626 vault adapter (T2) — MP-430.

Конкретный адаптер для Frax sFRAX Vault на Ethereum mainnet.
Vault address: 0xA663B02CF0a4b149d2aD41910CB81e23e1c41c32

Ключевые характеристики:
- Tier T2 (TVL $600M, Risk Score 0.40) — лимит 20% портфеля
- APY читается из data/adapter_status.json (поле sfrax.apy),
  fallback = 6.0% при отсутствии / ошибке чтения
- Benchmark rate привязан к IORB (Interest On Reserve Balances)
- Peg-gate: is_peg_healthy() True пока |frax_price - 1.0| <= PEG_TOLERANCE.
  RISK_SCORE = 0.40 выше Morpho (0.35) — у FRAX исторический depeg-риск
- ERC-4626 redeem мгновенный (EXIT_LATENCY_HOURS = 0.0); однако на практике
  есть swap-friction FRAX↔USDC (slippage / ликвидность пула) — выход в USDC
  не атомарен, в отличие от PSM-маршрутов Sky. Это учитывается отдельно
  оркестратором, здесь декларируем только латентность ERC-4626 redeem.
...
class SfraxAdapter(BaseAdapter):
    """Read-only advisory адаптер для Frax sFRAX ERC-4626 vault (T2).

    APY берётся из ``data/adapter_status.json`` → ``sfrax.apy``
    (значение в процентах, например 6.0). При отсутствии поля или ошибке
    чтения используется ``DEFAULT_APY_PCT = 6.0``.

    Peg-gate (вместо GSM-gate у Spark): ``is_peg_healthy()`` возвращает True
    пока цена FRAX держит привязку — отклонение ``|frax_price - 1.0|`` не
    превышает ``PEG_TOLERANCE`` (0.5%). Логика default-safe в сторону healthy:
    отсутствие поля ``frax_price`` трактуется как 1.0 (нет данных о депеге !=
    ...
    # ── идентичность ─────────────────────────────────────────────────────
    PROTOCOL = "sfrax"
    PROTOCOL_NAME = "Frax Staked FRAX (sFRAX)"
    VAULT_ADDRESS = "0xA663B02CF0a4b149d2aD41910CB81e23e1c41c32"
    ...
    CHAIN = "ethereum"
    CHAIN_ID: int = 1
    RISK_SCORE: float = 0.40      # выше Morpho (0.35): исторический depeg-риск FRAX

    # ERC-4626 redeem мгновенный; swap-friction FRAX↔USDC учитывается оркестратором
    EXIT_LATENCY_HOURS: float = 0.0

    # ── APY параметры ────────────────────────────────────────────────────
    MIN_APY_PCT: float = 3.0
    MAX_APY_PCT: float = 12.0
    DEFAULT_APY_PCT: float = 6.0   # fallback (benchmark rate привязан к IORB)

    TVL_USD: float = 600_000_000

    # ── peg compliance ───────────────────────────────────────────────────
    PEG_TOLERANCE: float = 0.005   # 0.5% — макс отклонение frax_price от 1.0
    ...
    def _read_status(self) -> dict:
        """Читает sfrax-секцию из data/adapter_status.json.
        ...
        try:
            ...
            with open(path, encoding="utf-8") as fh:
                data = json.load(fh)
            result = data.get("sfrax", {})
            return result if isinstance(result, dict) else {}
        except Exception as exc:  # noqa: BLE001 — graceful fallback
            logger.debug("sfrax: не удалось прочитать status JSON: %s", exc)
        return {}

    def _read_apy_from_status(self) -> Optional[float]:
        """Читает APY (%) из sfrax.apy. Возвращает float или None."""
    ...
    def get_apy(self) -> float:
        ...
        Источник: data/adapter_status.json → sfrax.apy.
    ...
    def vs_morpho_gap(self, morpho_apy: float = 6.5) -> float:
        """Возвращает morpho_apy - sfrax_apy (отрицательный = sFRAX лучше).
