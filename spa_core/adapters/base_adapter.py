...
class YieldInfo:
    ...
    # ADR-239: ПОЧЕМУ личность не измерена — записывается там, где решается.
    # Строго аддитивно (default None). Пустой `pool_id` без причины неотличим
    # от «адаптер про пулы не умеет вовсе»; с причиной — видно, что вопрос был
    # задан и на него честно не ответили (нет ноги PT в фиде · фид недоступен ·
    # несколько пулов на один инструмент). Поле-близнец `pool_match_refused` с
    # той же ролью уже живёт на стороне `adapter_status` — словарь общий
    # намеренно, чтобы два артефакта оставались сравнимыми.
    pool_id_refused: Optional[str] = None
