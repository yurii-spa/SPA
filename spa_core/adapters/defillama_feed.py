...
class DeFiLlamaFeed:
    ...
    def get_pool_id(
        ...
        """Вернуть UUID пула, который ЭТОТ ЖЕ отбор выбрал бы, — или ``None``.

        ADR-233. ``get_apy``/``get_tvl`` отдают ЧИСЛО и молча выбрасывают
        сведения о том, из какого пула оно взято. До сих пор личность
        разрешённого пула не покидала этот метод ни по одному пути: поле
        ``pool_id`` у адаптеров — рукописный слаг-константа
        (``"aave-v3-usdc-ethereum"``), одинаковый при любом исходе отбора, а
        снимок оркестратора не нёс о пуле вообще ничего.

        Цена молчания измерена 2026-09-05 (цикл #492) на КРУПНЕЙШЕЙ позиции
        книги. У ключа ``aave_v3`` в живом фиде ЧЕТЫРЕ кандидата
        ``aave-v3``/Ethereum/USDC, и два производителя выбрали РАЗНЫЕ::

            aa70268e…  $153.55M  3.587 пп  (ядро рынка, underlying USDC)
            6f00d46b…  $ 58.62M  5.247 пп  "Umbrella"  underlying 0xD4fa… — НЕ USDC,
                                            из них 1.66 пп — эмиссия (apyReward)
            effcb4a4…  $  1.53M  2.580 пп  "Prime Instance"
            27296bf9…  $  0.65M  4.372 пп  "Aave Horizon Market"

        В снимке 06:00Z ``adapter_status`` предъявил 2.5804, снимок оркестратора —
        5.2651, отметки наблюдения врозь на 2.16 с (столько же, сколько у восьми
        СОШЕДШИХСЯ протоколов того же снимка). Сторож назвал это «противоречием
        двух наблюдений» и потребовал fail-CLOSED — а наблюдения были верны оба,
        просто о РАЗНЫХ инструментах. Починка такого — закрепить пул, а не
        выбрать число, и различить эти два случая нельзя, пока личность пула не
        записана.

        Отбор здесь НЕ дублируется: метод — тонкая обёртка над ``get_pool``, тем
        же снимком (``_fetch_pools`` кэширован TTL 300 с) и тем же правилом
        «побеждает больший TVL». Иначе ответ относился бы к другому моменту, а не
        к тому наблюдению, которое отдано рядом.
        """
        pool = self.get_pool(project, symbol, chain, symbol_mode=symbol_mode)
        if pool is None:
            return None
