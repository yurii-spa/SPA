...
class GoLiveChecker:
    ...
    def _check_gap_monitor_ok(self, blockers: list[str]) -> bool:
        ...
        if not isinstance(doc, dict):
            ...
            return False
        # ADR-087 (выписан как ADR-067, номер разошёлся): блокируют только
        # АКТИВНЫЕ (восстановимые) дыры.
        #
    ...
    def _compute_consecutive_ready_days(self, ready: bool) -> int:
        ...
        # ИНВАРИАНТ: дней ПОДРЯД не может быть больше, чем прошло дней.
        #
        # Он в КОДЕ, а не только в тесте, по измеренной причине: исправления
        # формулы оказалось мало. Прошлое состояние уже содержало ложные 58, и
        # новая логика честно взяла «прошлое + 1» — то есть продолжила врать,
        # унаследовав отравленное начало. Симптом починен, данные остались.
        #
        # Здесь счётчик ОТКАЗЫВАЕТСЯ от прочитанного значения, если оно больше
        # возможного. Тогда любое отравленное состояние — от старой версии, от
        # чужой записи, от ручной правки — самоисправляется при первом прогоне,
        # и чинить данные руками больше никогда не понадобится.
        #
        # Граница — EVIDENCED-дни трека, а не расстояние от PAPER_REAL_START.
        #
        # Первая версия этой проверки брала PAPER_REAL_START и не сработала: он
        # приходится на 2026-06-10, ровно 58 дней до замера — то есть граница
        # совпала с самим дефектным числом и пропустила его. 58 никогда не было
        # «накопленным счётчиком»: это просто возраст трека, и мерить его тем же
        # якорем бессмысленно.
        #
        # Честный потолок: готовым нельзя быть дольше, чем трек существует как
        # ДОКАЗАННЫЙ. Evidenced-дней 45 при заявленных 58 — расхождение видно.
        # Неизвестная длина трека — НЕ повод сбрасывать счёт: «не знаю» и
        # «невозможно» — разные ответы, и путать их здесь значит уничтожать
        # верные данные из осторожности. Инвариант применяется только когда есть
        # чем мерить (тот же принцип, что «неразрешённое = не проверено, а не
        # нарушение»).
        _evidenced = int(self._real_track_days or 0)
        max_possible = _evidenced if _evidenced > 0 else None
        if max_possible is not None and prior_days > max_possible:
            # В модуле нет логгера, а молчать здесь нельзя: самоисправление
            # должно быть ВИДНО, иначе число «починится» незаметно и никто не
            # узнает, что состояние было отравлено.
            import sys as _sys
            print(
                f"[golive] consecutive_ready_days={prior_days} превышает возможное "
                f"({max_possible} evidenced-дней) — состояние отравлено, счёт начат заново",
                file=_sys.stderr,
            )
            return 1
        ...
        if prior_date < today:
            nxt = prior_days + 1
            return min(nxt, max_possible) if max_possible is not None else nxt
