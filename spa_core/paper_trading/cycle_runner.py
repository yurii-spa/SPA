...
def run_cycle(
    *,
    ...
    # ADR-331: второе имя пула — под каноническим ключом ДО любого сравнения с целью
    # (иначе фантомная перекладка «fluid_usdc → fluid_fusdc» с издержками).
    ...
    try:
        ...
        _cio_doc = write_shadow_rationale(
            ...
            # ADR-312 (заказ CIO #549), АДДИТИВНО: момент наблюдения каждой
            # ставки. Тот же принцип, что у провенанса TVL строкой выше — брать
            # ГОТОВОЕ у аллокатора, а не заводить второе определение «когда это
            # наблюдалось». Снимок оркестратора перезаписывается каждым
            # прогоном, поэтому без этого поля момент наблюдения не переживал
            # следующий прогон.
            apy_as_of=(getattr(alloc, "feed_coverage", {}) or {}).get("as_of"),
            capital_usd=capital_usd,
            cycle_date=today,
            run_ts=run_ts,
