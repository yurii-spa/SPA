...
test_skipped_unsupported_protocol` (см. AUD-12), deprecated-pendle collection errors,
`test_data_integrity::test_only_stdlib_imports` (di.py импортит spa_core), и
**висячий импорт** `spa_core.monitoring.telegram_watcher`/`parse_alert_type` в
`dev_agents/auto_fixer.py` (модуль не существует → 2 теста `run_auto_fix` падают).
