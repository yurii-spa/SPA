...
### AUD-13 — ruff: 4063 замечания 🟢 DONE (F401) / F841 follow-up
...
**F401 — ✅ DONE (поэтапно, по пакетам, с прогоном тестов):**
- `spa_core/analytics/` — 562 F401 удалено. Верификация: analytics/adapter
  набор (~639 файлов) даёт идентичный набор падений (157 failed, 2 errors)
  до и после → ноль регрессий.
- `paper_trading`/`strategies`/`backtesting`/`alerts`/`reporting` — 153 F401.
  Целевой набор: идентичные 76 падений до/после.
- Остальные нечувствительные подпакеты spa_core (agents/bee/reports/data_trust/
  orchestrator/persistence/tuner/monitor/milestone/tournament/scheduler/
  data_pipeline/coordinator/optimization/audit/allocator/adapters/utils) — 88 F401.
  Полный прогон (84919 passed): идентичные 590 failed / 3 errors до/после.
- `spa_core/tests/` — 1163 F401 (506 файлов). Полный прогон: идентичные 590 failed.
- Топ-уровень `tests/`/`scripts/`/`verifier/`/`adapters/` + корневые модули —
  527 F401 + 44 F541 (repo-wide) + 3 F811. Затронутые tests/-файлы: идентичные
  10 failed / 1 error (все предсуществующие).
- **Намеренно НЕ тронуты** (политически чувствительные домены, чистый hygiene-pass):
  `spa_core/{risk,execution,monitoring,golive,governance}` — остаётся ~53 F401.
  Изменение этих файлов требует осознанного review, не косметического линта.
- Остался 1 неоднозначный F811 (`_std` в `test_tournament_evaluator` — локальный
  def шадует импорт).

**Follow-up (🔜, осторожно):** F841 (332 unused-var) — `--fix` рискован (RHS
с side-effect). Делать вручную, по случаю.
