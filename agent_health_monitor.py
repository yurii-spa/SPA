...
def check_agent(label: str, plist: Optional[dict], parse_ok: bool,
    ...
    if health.last_exit not in (None, 0):
        ...
        # always-on server: CRITICAL only if pid==0 (server is down).
        # If pid>0 the server restarted successfully; treat as WARNING only.
        if cat == CAT_ALWAYS_ON:
            sev = CRITICAL if health.pid == 0 else WARNING
        else:
            sev = WARNING
