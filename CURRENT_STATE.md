...
> 🔴 **ЖИВОЙ ПРОД на 2026-08-24** (`data/golive_status.json` в боевом дереве): GoLive **29/29**,
> трек **62/30** evidenced с того же честного anchor **2026-06-22**. Строки ниже — КОММИТНУТЫЙ
> снимок: в git тот же файл заморожен на **13/30**, и расхождение с живым — 49 дней. Оба числа
> стоят здесь НАМЕРЕННО: сторож `test_narrative_docs_match_golive_state` читает тот
> `data/golive_status.json`, который лежит в ЕГО дереве, а канон в git и живой прод отвечают
> на этот вопрос по-разному — расхождение названо, а не спрятано. Живой ряд — `docs/SYSTEM_BRIEFING.md`.
>
> Текущая правда (2026-07-03, синхронизировано с `data/golive_status.json`): GoLive **27/29**,
> трек **12/30** evidenced (anchor **2026-06-22**; всё до — backfill/демо, недействительно;
> 2026-06-27/28/29 восстановлены из реальных daily_cycle-логов после git-reset stale-restore инцидента — WS1.1 self-heal;
> target go-live **~2026-07-21**), агентов **47** (launchctl после retirement'ов, agent_health
