...
- **Бренд-нейминг и числа — OWNER-GATED.** Не менять расшифровку «SPA», отображаемые числа
  доходности, наименования тиров и legal-формулировки без карточки владельца. Это **предмет №2**
  из трёх, оставшихся за владельцем по ADR-285 (граница проходит по предмету, а не по важности —
  полная таблица в `CLAUDE.md`, раздел «Граница „решай сам“ / „спроси меня“»). Всё, что не
  попадает в три предмета, агент решает сам и записывает в журнал, а не спрашивает.
  ⚠️ Замечание по механизму (замер 09.09): класс `honesty.token.removed` срабатывает на ЛЮБУЮ
  правку строки с evidence-токеном — в том числе когда напечатанное число заменяется рендером
  того же числа. Это верный отказ, а не ложное срабатывание: сторож по построению не может
  отличить «число убрали» от «число теперь считается». Такая правка идёт карточкой.
