...
class TestOrchestratorRun(unittest.TestCase):
    ...
    def test_default_registry_has_four_readonly_adapters(self):
        # реестр по умолчанию — 4 read-only адаптера из spa_core/adapters
        names = [cls.__name__ for (_, _, cls) in orch.ADAPTER_REGISTRY]
        self.assertEqual(len(orch.ADAPTER_REGISTRY), 4)
