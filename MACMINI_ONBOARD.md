...
2. **Проверить набор plist'ов:** канонический набор — `scripts/com.spa.*.plist` (~42).
   В корне остались только always-on демоны (`apiserver`, `autopush`, `httpserver`).
   Дубль `com.spa.cyclerunner` (морозил трек) уже удалён — канонический раннер цикла
   `com.spa.daily_cycle`. На новом хосте: `bash scripts/install_all_agents.sh`.
3. **Доустановить недостающее при необходимости** (напр. `com.spa.bot_commands` — интерактивный Telegram-бот).
4. **Проверить autopush / туннель / сайт 8765** вживую.
5. **Сверить CURRENT_STATE.md / SYSTEM_BRIEFING.md** с фактическим статусом.
