...
# ── Шаг 5.5: якорение головы цепи доказательств в Bitcoin (ADR-YL-010, ADR-090 п.2) ─────
# ADR-YL-010 (03.07) обещал дословно: «Daily agent runs `python3 scripts/ots_anchor.py both`».
# Замер 15.08 показал, что обещание не исполнялось 43 дня: в `proofs/ots/ots_anchors.jsonl`
# ОДНА строка от 2026-07-02T22:46Z со статусом `pending`, ни одного якорения и ни одного
# upgrade. Причина — «daily agent» не существовал: у скрипта не было ни плиста, ни строки
# в цикле. То есть у трека не было НИ ОДНОЙ независимой отметки времени — ровно того
# доказательства, против которого у скептика нет довода «вы могли нарисовать задним числом».
#
# ЗДЕСЬ, а не отдельным LaunchAgent'ом — по той же причине, что шаги 4 и 5: флот не растёт
# на одного ради взгляда на себя, а шаг цикла нельзя забыть при установке (именно так этот
# и потерялся). ПОРЯДОК ОБЯЗАТЕЛЕН: строго ПОСЛЕ cycle_runner, иначе якорится вчерашняя
# голова цепи, а не свежая.
#
# НЕ ФАТАЛЬНО и это часть решения: аудит-слой не двигает капитал и не гейтит трек.
# Календари OpenTimestamps — внешняя сеть; её недоступность не имеет права стоить дня трека.
# Молчание при этом видно снаружи: статус каждой записи лежит в `proofs/ots/ots_anchors.jsonl`,
# и `pending`, не превратившийся в подтверждённый, — сам по себе находка.
"$PYTHON" scripts/ots_anchor.py both >> "$LOG_FILE" 2>&1 \
  || echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] ots_anchor: якорение или upgrade не удались (non-fatal, см. proofs/ots/ots_anchors.jsonl)" >> "$LOG_FILE"

# ── Шаг 6: публичный research-changelog (карточка agent-changelog-generator-never-called).
...
# ВОССТАНОВЛЕН 17.08: шаг пропал при слиянии, когда обёртка была взята из дерева агента с
# устаревшей базой — CI это поймал (test_research_changelog_wiring). Четвёртый случай того же
# класса за сутки; вывод записан в журнал, а не только здесь.
