...
**Коды возврата — и почему находка НЕ единица (12.09, цикл #571).** Прежний контракт был
«0 — чисто, 1 — находка», и он был неразличим по построению: **единицу выдаёт сам CPython**
при любом необработанном исключении, в том числе при падении на собственном импорте. В
то утро сторож умер на `import spa_core` (бутстрап `sys.path` стоял НИЖЕ импорта), вышел с
кодом 1 — и обёртка честно, по контракту, вклеила его трассировку в промпт цикла словами
«ШАГ 0a-ГОЛОД — НАХОДКА, возьми ЭТУ карточку первой». Крах сторожа предъявили как его
вердикт. Инвариант #17 требует различать три исхода — измерено · измерено и пусто · НЕ
измерено, — а здесь «не измерено» было склеено с самым громким из измеренных.

Поэтому единица **отдана интерпретатору** и не занята нами ни одной веткой:

* **0** — измерено, голодающих приказов нет;
* **1** — сторож НЕ ДОШЁЛ до вердикта (краш, сорванный импорт, синтаксис). Это «не
  измерено», а не «чисто» и не «находка»;
* **2** — измерено НЕ ПОЛНОСТЬЮ: очередь на `--ref` не прочитана (и argparse-отказ);
* **3** — **НАХОДКА** (fail-CLOSED: голодом считается и карточка, чей возраст не удалось
  измерить как заведомо свежий).

Тройку не может подделать никто, кроме явного `return _RC_FINDING` ниже: у интерпретатора
такого кода нет. Перекос версий между обёрткой и скриптом (прод-дерево отстаёт от origin
по построению) падает в безопасную сторону в ОБЕ стороны — старая обёртка прочитает 3 как
«не измерено», новая прочитает 1 как «не измерено»; ни один перекос не выдаёт находку за
чистоту и наоборот.
...
# Бутстрап пути ОБЯЗАН стоять ВЫШЕ любого импорта пакета репозитория. Обёртка
# `agent_orchestrator.sh` зовёт этот файл ПО ПУТИ (`"$PYTHON" "$STARVE_PY"`), а не модулем
# через `-m`: тогда `sys.path[0]` — каталог `scripts/`, и `spa_core` оттуда не виден ни при
# каком рабочем каталоге. До 12.09 строка `from spa_core.utils.observation import …` стояла
# на четыре строки ВЫШЕ этого блока — сторож умирал на собственном импорте каждый запуск.
...
#: Коды возврата. Единица НЕ ЗАНЯТА намеренно — она принадлежит интерпретатору
#: (необработанное исключение), и занять её значило бы снова склеить «сторож упал»
#: с «сторож что-то нашёл». См. раздел про коды в докстринге модуля.
_RC_CLEAN = 0
_RC_CRASHED = 1
_RC_PARTIAL = 2
_RC_FINDING = 3
...
def main(argv=None) -> int:
    ...
    return _RC_PARTIAL if ref_error is not None else _RC_CLEAN


if __name__ == "__main__":
    # Перехват здесь — не «на всякий случай», а часть контракта кодов. Без него
    # необработанное исключение всё равно дало бы 1, но БЕЗ названной причины в том
    # виде, в каком её читает обёртка: трассировка ушла бы в stderr как есть, а
    # читателю нужен явный ответ «вердикта НЕТ». `BaseException` не ловим — Ctrl-C и
    # SystemExit из argparse обязаны проходить насквозь со своими кодами.
    try:
        raise SystemExit(main())
    except Exception as exc:  # noqa: BLE001 — намеренно широко: см. выше
        print(f"❌ СТОРОЖ ГОЛОДАНИЯ НЕ ДОШЁЛ ДО ВЕРДИКТА: {type(exc).__name__}: {exc}. "
              f"Это НЕ «голода нет» и НЕ находка — голодание НЕ ИЗМЕРЕНО.", file=sys.stderr)
        traceback.print_exc()
        raise SystemExit(_RC_CRASHED) from exc
