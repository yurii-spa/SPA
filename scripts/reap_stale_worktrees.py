...
4. **Каждый непустой путь дерева объяснён.** По каждому пути, где содержимое дерева расходится
   ...
   - ``absent`` — файла на базе нет вовсе. Всегда ``unique``-класс: отсутствие на базе
     самодостаточно;
   - ``not_in_tree`` — **пути нет на диске ЭТОГО дерева, а на базе он есть** (цикл #377).
     Шестое имя, и появилось оно потому, что до него отсутствие файла дочитывалось до
     ``unique`` МОЛЧА: `_blob_sha` у несуществующего пути возвращает ``None``, ветка
     ``delivered`` закрыта условием ``sha is not None``, база по пути не двигалась — и
     вердикт выходил вывернутым наизнанку, «здесь может лежать НЕДОСТАВЛЕННАЯ работа» о
     пути, которого в дереве НЕТ, а на базе он ЕСТЬ. Терять нечему.

     Замер #376 (воспроизведён #377 целиком, от `git init` до вердикта): состояние приходит
     из СТАНДАРТНОГО хода протокола — страж перезаписи пушера отбивает пуш, лечение по
     `CLAUDE.md` — перенести правку на свежий origin и переставить HEAD
     (`git reset --mixed origin/main`), после чего HEAD перечисляет чужие пути, которых
     дерево никогда не выкладывало. Чем аккуратнее сессия соблюдает протокол, тем вернее
     упрётся в неснимаемое дерево — а неснимаемые `/tmp`-дерева и есть тот механизм, которым
     шаг 0a обзаводится вечными жителями раздела «НЕ ДОСТАВЛЕНО» (класс #243/#354).

     **Имя выбирает ЗАМЕР, а не догадка о происхождении состояния** — ``_never_materialised``
     спрашивает stat-запись индекса (`git ls-files --debug`): нули в `ino`/`ctime` значат, что
     файл в индекс попал из дерева коммита и на диске его тут не было НИ РАЗУ. Настоящее
     удаление выложенного файла (тоже возможная работа сессии) несёт живые `ino`/`ctime` и
     по-прежнему судится как ``unique`` — послабления нет. Не измерили ⇒ судим строже,
     как раньше;
...
DROPPED = "dropped"
# Шестой вердикт — «этого пути дерево НИКОГДА не выкладывало» (цикл #377, карточка
# `inbox-uborschik-zovet-nedostavlennoi-rabotoi-otsutstvuyuschii-fail`). См. п. 4б шапки.
...
_LS_FILES_INO = re.compile(r"^\s*dev:\s*(\d+)\s+ino:\s*(\d+)\s*$", re.M)
_LS_FILES_CTIME = re.compile(r"^\s*ctime:\s*(\d+):(\d+)\s*$", re.M)


def _never_materialised(wt_path, rel, git=_git):
    """Выкладывало ли ЭТО дерево путь на диск хоть раз. True — никогда, None — не измерено.

    Спрашивается ИНДЕКС, а не догадка о том, как дерево пришло в текущее состояние. Git
    хранит в индексе stat-запись каждого файла (`dev`/`ino`/`ctime`/`size`) и заполняет её
    ровно тогда, когда САМ трогает файл на диске — при выкладке (`checkout`, `reset --hard`)
    или при `add`. Запись, попавшая в индекс из дерева коммита без выкладки (`reset --mixed`
    на свежий `origin/main` — предписанное протоколом лечение отбитого пуша), несёт нули:
    `dev: 0  ino: 0`, `ctime: 0:0`. Отличие прямое и однозначное:

    | как путь попал в индекс | ino | ctime | что это значит |
    |---|---|---|---|
    | `git reset --mixed` на базу с чужим путём | 0 | 0:0 | файла тут НИКОГДА не было |
    | выложен и потом удалён сессией | ≠0 | ≠0 | удаление — возможная работа дерева |

    Пустой файл сюда не проваливается: у него ноль в `size`, но `ino`/`ctime` настоящие.
    Не разобрали вывод / путь не в индексе / git не ответил ⇒ ``None`` (fail-CLOSED: зовущий
    судит как раньше, то есть строже)."""
    rc, out, _ = git(wt_path, "ls-files", "--debug", "--", rel)
    if rc != 0 or not out.strip():
        return None
    m_ino, m_ctime = _LS_FILES_INO.search(out), _LS_FILES_CTIME.search(out)
    if not m_ino or not m_ctime:
        return None
    return (int(m_ino.group(2)) == 0
            and int(m_ctime.group(1)) == 0 and int(m_ctime.group(2)) == 0)
...
def classify_path(root, base_ref, head, wt_path, rel, git=_git):
    ...
    # ТРЕТИЙ вопрос — «а лежит ли путь на диске?» (цикл #377). Без него отсутствие файла
    # дочитывалось до `unique` МОЛЧА: `_blob_sha` у несуществующего пути даёт None, ветка
    # `delivered` закрыта условием `sha is not None`, база по пути не двигалась (HEAD только
    # что переставлен НА неё) — и вердикт выходил вывернутым наизнанку: «здесь может лежать
    # НЕДОСТАВЛЕННАЯ работа» о пути, которого в дереве нет, а на базе он ЕСТЬ. Терять нечему.
    if not os.path.lexists(os.path.join(wt_path, rel)):
        never = _never_materialised(wt_path, rel, git=git)
        if never:
            return NOT_IN_TREE, (f"пути нет на диске дерева, на {base_ref} он есть, "
                                 f"а в индексе у него нулевая stat-запись — дерево его "
                                 f"никогда не выкладывало, терять нечего")
        # never is False ⇒ файл ТУТ БЫЛ и его удалили: удаление — возможная работа, судим
        # как раньше. never is None ⇒ не измерено, и тем более судим как раньше (строже).
