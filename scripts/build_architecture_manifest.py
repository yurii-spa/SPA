...
Разделение полей на агенте:
  ...
  КУРИРОВАННЫЕ (генератор сохраняет как есть; сеет дефолт только для НОВОГО агента):
    ...
    consumer_required — продукт ОБЯЗАН иметь читателя (ресит потребления, Фаза 2)
                     Срок годности ПОТРЕБЛЕНИЯ у артефакта — свой: `consumption_slo_hours`
                     (по умолчанию 26ч). Он НЕ равен `slo_hours`: тот — контракт
                     производителя («файл свежий?»), этот — вопрос B3 («его кто-то
                     читает?»), и масштаб у них разный. Ставить его строже такта самого
                     частого читателя нельзя — B3 назовёт литерал невыполнимым (цикл #348).
...
CURATED_DEFAULTS = {
    ...
    "curation": "none",
    "notes": "",
    # Паспорт агента (AI1 гл.3/24): деловая цель, метрика качества, эскалация.
    # ОБЯЗАН быть в CURATED_DEFAULTS, а не только в схеме: пересборка манифеста
    # копирует ТОЛЬКО перечисленные здесь поля, поэтому паспорт, заполненный
    # где-то ещё, молча пропадал бы при первой же пересборке — и мы бы решили,
    # что «паспорта опять никто не заполнил». Закреплено тестом.
    "passport": {},
}
