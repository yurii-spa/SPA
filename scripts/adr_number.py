...
остались следы обоих исходов: `ADR-073` разошёлся честно (живая строка + строка-указатель
`Superseded`), `ADR-067` — НЕ разошёлся (два разных решения, оба `Accepted`, оба действуют).
