#!/usr/bin/env python3
"""
scripts/edge_asymmetric_kelly.py — Idea #27: Asymmetric-Lookback Kelly (ALK)
                                   Decoupling the μ-signal from the σ²-signal

NOVEL EDGE IDEA #27 (docs/DYNAMIC_LEVERAGE_GUARDIAN.md registry)
...
def main() -> None:
    print("=" * 76)
    print("IDEA #27: Asymmetric-Lookback Kelly (ALK)")
    ...
    print("\n── PER-CRISIS DRAWDOWN: static #3 → KODS#15(sym-10) → best ALK #27 ─────────")
    kods_15_sym10 = kods15_10
    print(f"  {'event':32s} {'static':>8} {'KODS15':>8} {'ALK#27':>8} {'saved vs#3':>11}")
    print(f"  {'-'*32} {'-'*8} {'-'*8} {'-'*8} {'-'*11}")
    for w in STRESS_WINDOWS:
        k = w["key"]
    ...
    if len(test_dates) > 10 and best is not None:
        ...
        print(f"    ALK #27 best   : APY {_f(a_a)}%  maxDD {_f(d_a)}%  Calmar {_f(c_a)}")
    ...
    print(f"  ALK #27 best Calmar:      {_f(best['calmar'])} "
