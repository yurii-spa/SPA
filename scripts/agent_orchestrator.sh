...
#
# Fail-CLOSED в обе стороны: код 3 — находка; код 0 — голода нет; ЛЮБОЙ другой код (и
...
#
# ⚠️ Находка — код 3, а НЕ 1 (правка 12.09, цикл #571, ADR-347). Прежде находкой считалась
# единица — и ровно единицу выдаёт CPython при ЛЮБОМ необработанном исключении. В то утро
# сторож умер на собственном импорте `spa_core` (бутстрап `sys.path` стоял ниже импорта),
# вышел с кодом 1 — и эта ветка вклеила его ТРАССИРОВКУ в промпт цикла словами «ШАГ 0a-ГОЛОД
# — НАХОДКА, возьми ЭТУ карточку первой». Крах сторожа был предъявлен сессии как вердикт
# сторожа, и третий исход («НЕ ИЗМЕРЕНО») оказался недостижим для самой частой поломки.
# Классификация вынесена в sourceable-файл, потому что у неё обязан быть положительный
# контроль на паре «код 1, трассировка»: прежний изображал поломку кодом 7, которого у
# падающего python-скрипта не бывает — контроль был, настоящей аварии он не видел ни разу.
...
if [ ! -f "$STARVE_PY" ]; then
    ...
    # 127 («команды нет»), а не 2: двойка теперь означает ИЗМЕРЕННУЮ неполноту (не прочитан
    # ref), и склеивать с ней «скрипта нет вовсе» значило бы снова смешать два исхода.
    STARVE_RC=127
else
...
if [ -f "$STARVE_LIB" ]; then
    # shellcheck disable=SC1090
    . "$STARVE_LIB"
    STARVE_PREFIX="$(starvation_prefix "$STARVE_RC" "$STARVE_OUT")"
else
    # Библиотеки нет (прод-дерево отстало) — молчать нельзя: молчание читается как «голода
    # нет». Классифицировать без неё мы не умеем, поэтому честный третий исход.
    STARVE_PREFIX="ШАГ 0a-ГОЛОД НЕ ИЗМЕРЕН: нет $STARVE_LIB — классифицировать вердикт сторожа \
(код $STARVE_RC) нечем. Это НЕ «приказ владельца не голодает» — проверь голодание руками перед \
выбором задачи. Вывод сторожа: $STARVE_OUT. "
fi
...
(1г) если цикл ДОБАВИЛ или ПРАВИЛ тест-файл — ОБЯЗАТЕЛЬНО python3 scripts/import_time_anchor_bombs.py (ADR-348: тест, чей якорь времени вычислен на импорте, краснеет от ДЛИТЕЛЬНОСТИ прогона, а не от кода; храповик замороженных дат этот фитиль не видит — литеральной даты в нём нет вовсе; код 1 => чинить тем же пушем, код 2 => НЕ ИЗМЕРЕНО, карточка); (1д) ОБЯЗАТЕЛЬНО python3 scripts/book_second_record.py — вторая запись о деньгах: журнал ходов обязан сходиться с книгой (ADR-350). Код 1 => расхождение НАЗВАНО в цикле (дыра в записи или переименование ключа), код 2 => НЕ ИЗМЕРЕНО, карточка. Прибор только ЧИТАЕТ и ничего не чинит; новая дыра обязана быть замечена в тот же день, а не через два месяца. (1е) ОБЯЗАТЕЛЬНО python3 scripts/capital_observability_census.py --data-dir <прод>/data — приёмка G1 приказа владельца «Portfolio CIO» (ADR-364): доля КАПИТАЛА на НАБЛЮДЁННЫХ числах, по ДВУМ осям (APY ранжирует, TVL допускает) и у ПОВЕРХНОСТИ РЕШЕНИЯ, а не у самого свежего файла. Код 1 => находка НАЗВАНА в цикле (доля упала ниже 100 % либо второй артефакт спорит), код 2 => НЕ ИЗМЕРЕНО, карточка. Прибор только ЧИТАЕТ. Из worktree живого data/ нет по построению — передавать --data-dir прод-дерева. (1е) ОБЯЗАТЕЛЬНО python3 scripts/build_site_numbers.py --if-due — витрина чисел сайта (ADR-365, установка владельца ADR-357 п. 5: один адрес чтения, такт НЕДЕЛЬНЫЙ, ставки годовые). Внутри недели команда ничего не переписывает и выходит 0; код 2 => НЕ ИЗМЕРЕНО (источник не прочитан), карточка. Файл под landing/** => пуш ТОЛЬКО через scripts/safe_site_push.py. (1ж) ОБЯЗАТЕЛЬНО python3 -m spa_core.monitoring.python_reader_clock_doors --data-dir <прод>/data --if-due — двери часов, связанные НА ИМПОРТЕ, у питоньих читателей переписи (ADR-406/408/409, заказ G33 п. 2: артефакта на прод-пути не было ВОВСЕ, а прогон стоит десятки минут — поэтому такт НЕДЕЛЬНЫЙ и решает его ФАЙЛ). Внутри недели команда ничего не мерит и выходит 0; код 1 => двери НАЗВАНЫ в цикле; код 2 => НЕ ИЗМЕРЕНО (пин не наблюдён / стенд не построен), карточка. Прибор только ЧИТАЕТ и зовёт читателей против КОПИИ стенда. Из worktree живого data/ нет по построению — передавать --data-dir прод-дерева. (2) разбери Inbox (задача/идея/непонятно) и инжест owner-done (ADR + set-status ingested через \
