...
(1г) если цикл ДОБАВИЛ или ПРАВИЛ тест-файл — ОБЯЗАТЕЛЬНО python3 scripts/import_time_anchor_bombs.py (ADR-347: тест, чей якорь времени вычислен на импорте, краснеет от ДЛИТЕЛЬНОСТИ прогона, а не от кода; храповик замороженных дат этот фитиль не видит — литеральной даты в нём нет вовсе; код 1 => чинить тем же пушем, код 2 => НЕ ИЗМЕРЕНО, карточка); (2) разбери Inbox (задача/идея/непонятно) и инжест owner-done (ADR + set-status ingested через \
