...
(1г) если цикл ДОБАВИЛ или ПРАВИЛ тест-файл — ОБЯЗАТЕЛЬНО python3 scripts/import_time_anchor_bombs.py (ADR-347: тест, чей якорь времени вычислен на импорте, краснеет от ДЛИТЕЛЬНОСТИ прогона, а не от кода; храповик замороженных дат этот фитиль не видит — литеральной даты в нём нет вовсе; код 1 => чинить тем же пушем, код 2 => НЕ ИЗМЕРЕНО, карточка); (1д) ОБЯЗАТЕЛЬНО python3 scripts/book_second_record.py — вторая запись о деньгах: журнал ходов обязан сходиться с книгой (ADR-350). Код 1 => расхождение НАЗВАНО в цикле (дыра в записи или переименование ключа), код 2 => НЕ ИЗМЕРЕНО, карточка. Прибор только ЧИТАЕТ и ничего не чинит; новая дыра обязана быть замечена в тот же день, а не через два месяца. (2) разбери Inbox (задача/идея/непонятно) и инжест owner-done (ADR + set-status ingested через \
