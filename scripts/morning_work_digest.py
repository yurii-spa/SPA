...
ФОРМАТ (строго):
1. Первая строка — заголовок «☀️ Что сделано вчера (<дата>)».
2. Вторая строка — ИТОГ ОДНИМ ПРЕДЛОЖЕНИЕМ: главное, что вчера изменилось для владельца.
3. Затем не больше ЧЕТЫРЁХ коротких буллетов (каждый с эмодзи, одна строка, ≤120 знаков).
Весь текст — не длиннее 1000 знаков.
ПРАВИЛА: простой человеческий русский, ШИРОКИМИ мазками. Без тех-жаргона, без имён файлов,
...
def main() -> int:
    ...
    try:
        ...
        # dedup=True: это ПУШ, а не ответ на команду владельца — побуквенно тот же
        # текст в окне 30 минут второй раз не уходит (двойной запуск агента).
        resp = TelegramBot().send_message(text, dedup=True)
