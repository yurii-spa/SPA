...
# site_freshness (Site Custodian на Маке, решение владельца 2026-08-14 вариант 1) — раз в 6ч
# гоняет `scripts/site_freshness_monitor.py` ОТСЮДА. Облачный прогон умеет только ПОВЕСИТЬ
# табличку честности: снятие доставляется пушером, а тот по контракту работает лишь из живого
# дерева Мака ⇒ из CI половина правила ADR-YL-011 не исполнима по построению. Workflow не
# снимаем — он остаётся вторым глазом на случай спящего Мака.
install_agent \
    "$REPO/launchd/com.spa.site_freshness.plist" \
    "com.spa.site_freshness" \
    "1"

# smoke_flagship (цикл #248) — раз в неделю гоняет `scripts/smoke_test_flagship.py`, тяжёлый
# end-to-end смоук двух флагманских поверхностей (liquidation-NAV-by-size + публичный
# refusal-log). Скрипт называл себя «единственной командой, которую запускает владелец или CI»
# и не запускался НИКЕМ: до #228 он числился подключённым по докстрингу соседнего smoke.py.
# Почему агент, а не workflow: замер 15.08 — в чистом чекауте 5 отказов из 5, потому что
# data/rates_desk не в git (`git ls-files` = 0) и нет landing/node_modules. Живые данные и
# сборка лендинга есть только здесь. Установка — решение владельца; перед первым прогоном
# убедиться, что в landing/ выполнен `npm install` (иначе шаг 4 будет падать еженедельно).
install_agent \
    "$REPO/launchd/com.spa.smoke_flagship.plist" \
    "com.spa.smoke_flagship" \
    "1"
