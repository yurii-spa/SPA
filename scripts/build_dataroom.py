...
def _readme(entries: list, generated_at: str) -> str:
    lines = [
        ...
        "python3 verifier/verify_spa.py . --replay   # ALSO re-derive every verdict from its own numbers",
        "```",
        "",
        "`--replay` goes beyond the hash chain: it re-derives each decision from its OWN published",
        "numbers (fair_yield == baseline − total_haircut; approved ⇒ net_edge > 0, the refusal-first",
        "invariant; every refusal reasoned). A verdict that does not FOLLOW from its inputs fails CLOSED —",
        "so this proves not merely \"we did not edit the row\" but \"the decision follows from the numbers\".",
        "It re-derives the refusals on positive-edge-but-toxic books too (the refusal moat, checkable).",
        "For the fully-offline frozen-snapshot path (`--offline`) see `docs/DD_PACK.md` §6–7.",
        "",
