...
def last_voice_by_session(entries, parse_ts=None) -> dict:
    """Ярлык сессии → время её ПОСЛЕДНЕГО объявления (ЛЮБОГО, не про одну карточку).

    **Зачем.** Живой якорь отвечает на «существует ли процесс», а не на «работает ли
    сессия». Пока якорем был процесс самой сессии, разницы не было — он умирал вместе с
    ней. Замер 28.08 (цикл #412): цикл #410 объявил якорем pid10980 — процесс `claude`
    ДЕСКТОПНОГО приложения (ppid=1533 = Claude.app), который хостит идущие одна за другой
    сессии и переживает каждую. Сессия умерла, работа осталась в `/tmp/spa_c410`, а
    `ps -p 10980` отвечал «жив» — и ответил бы так же через неделю.

    Голос — самый мягкий honest-признак «она ещё здесь»: сессия, копающая одну задачу
    четвёртый час, по дороге объявляет владение файлами и остаётся свежей.

    **Живёт ЗДЕСЬ, а пользуется пока только шаг 0b** (`check_card_claim`, где живой якорь
    больше не держит карточку бессрочно). Дом общий намеренно: тот же дефект есть и у
    вердикта ЭТОГО модуля (живой якорь безусловно велит МОЛЧАТЬ о дереве — так 28.08 был
    скрыт `/tmp/spa_c410`), и чинить его надо здесь же, а не второй копией правила.
    Почему не починено этим же циклом — карточка
    `inbox-shag-0a-molchit-o-dereve-po-zhivomu-yako`: правка красит 5 ЦЕНТРАЛЬНЫХ тестов
    доставки, и такой разбор обязан быть отдельной задачей, а не довеском к решению
    владельца."""
    parse_ts = parse_ts or _parse_ts
    out: dict = {}
...
def worktree_of(entry):
    """Рабочее дерево, названное записью, либо None, если оно из неё НЕ читается.
    ...
    **Второй источник — поле ``cwd`` самой записи (цикл #475).** Оно и заведено ровно на этот
    случай: `log_session_change` пишет его ТОЛЬКО когда среди объявленных путей есть
    относительный, то есть ровно тогда, когда из файлов дерево не выводится. До #475 поле
    читал РОВНО ОДИН потребитель — `declaring_tree` (шаг 0a, «чьё это расхождение»), — а
    второй, `classify_orphan` (шаг «сирота ли этот прогон»), спрашивал то же самое у файлов
    и получал «дерево НЕ названо». Источник без всех своих потребителей — тот же класс, что
    измеритель без читателя.

    **Цена измерена, а не предположена.** 2026-09-04, 03:00 местного: два `pytest` с
    ``ppid=1``, 96 % CPU каждый, 1 ч 03 мин стены, ``cwd=/private/tmp/spa_c474``; заказчик
    `cycle-20144` (pid 20144) измеренно мёртв — `ps` пуст. Запись #474 объявила пути
    ОТНОСИТЕЛЬНЫМИ и своё дерево назвала полем ``cwd``. Сторож `check_concurrent_pytest`,
    написанный циклом #417 ровно про таких сирот, ответил «дерево /tmp/spa_c474 не объявлено
    в журнале — заказчик НЕ НАЗВАН», то есть UNMEASURED вместо ORPHAN, и восьмой случай
    класса снова нашли ГЛАЗАМИ. Ответ лежал в самой записи.
    ...
    **Три сужения, каждое fail-CLOSED** (несработавшее оставляет прежнее «не измерено»):
    ...
    2. отказ по неоднозначности не снимается: два дерева в путях ⇒ по-прежнему None, иначе
       правило «не угадывать» уходило бы с чёрного хода;
    3. ``cwd`` проходит через `tree_of_path` — ту же дверь, что и пути. Иначе `/private/tmp`
       (как его пишет macOS) и `/tmp` (как его регистрирует git) остались бы разными
       деревьями, и сверка с `worktree_of` не сошлась бы ни разу.
    ...
    if roots:
        return None                          # сужение 2: неоднозначность — это отказ, а не пробел
    return _tree_of_announce_cwd(entry)       # сужение 1: только там, где путей не хватило


def _tree_of_announce_cwd(entry):
    """Дерево из поля ``cwd`` записи, либо None. Отдельной функцией — чтобы `worktree_of`
    оставалась одним выражением на каждый источник, а тест мог целиться в этот шаг отдельно."""
    cwd = (entry or {}).get("cwd")
    if not isinstance(cwd, str) or not cwd.strip():
        return None
    cwd = cwd.strip()
    if not os.path.isabs(cwd):
        # Относительный `cwd` не отвечает на вопрос «какое дерево» — он и сам требует базы.
        return None
    root = tree_of_path(os.path.join(cwd, "spa_core"))
    if root:
        return root
    # `git` о таком дереве не знает (снято, ещё не заведено) — но каталог назван ФАКТОМ из
    # записи, а не догадкой читателя. Нормализуем той же меркой, что и пути.
    try:
