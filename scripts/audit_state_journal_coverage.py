...
#: в ней не было речи. Обе ошибки молчаливы. Поэтому берутся ЧЕТЫРЕ формы, в которых
#: номер действительно называет цикл:
#:   * `цикл #131` / `Циклом #124` / `циклы #49–#50` — слово «цикл» вплотную перед решёткой;
#:   * `цикл оркестратора #39` — то же слово, но с ОДНИМ уточнением между ним и решёткой;
#:   * `(2026-07-30, #48)` — форма заголовков раздела «Последние решения»;
#:   * `Hardening #19 (автономный цикл env-setup-v3)` — номер, а «цикл» в скобке следом.
#:
#: **Почему форм стало четыре (цикл #150).** Первые две пропускали САМУЮ РАСПРОСТРАНЁННУЮ
#: форму заголовка журнала — `## … Автономный цикл оркестратора #39 — …`: между словом
#: «цикл» и решёткой стоит `оркестратора`, а `цикл\w*\s+#` требует их вплотную. Отчёт
#: получался обратный правде: инструмент назвал 16 циклов «записи НЕТ ВОВСЕ», по нему
#: завели карточку на восстановление 16 записей из git-истории — а записи всё это время
#: лежали в `docs/journal/` целиком. Восстановление «по отчёту» удвоило бы их.
#:
#: **Это не ослабление правила, а недостающая форма.** Требование слова «цикл» рядом с
#: номером сохранено — именно оно отсекает `инв. #16`, `класс #29/#31`, `PR #8`, `идеи
#: #42 CSDG`, `отчёт #109`; уточнение между словом и решёткой РОВНО ОДНО и берётся из
#: белого списка (`_CYCLE_QUALIFIERS`), а не «любые 1-2 слова»: свободный зазор поймал бы
#: `работа цикла #87 … числа идей #16` и записал бы идею #16 в покрытие цикла. Цена ошибок
#: несимметрична (ложное «покрыто» ⇒ запись STATE удаляют, а в журнале её нет ⇒ потеря
#: безвозвратна), поэтому расширение точечное и закреплено тестами в ОБЕ стороны.
_CYCLE_QUALIFIERS = "оркестратора|сессии"

CYCLE_RE = re.compile(
    r"(?:"
    r"цикл\w*\s+#(?P<near>\d{1,3})"                                  # «цикл #131»
    rf"|цикл\w*\s+(?:{_CYCLE_QUALIFIERS})\s+#(?P<qualified>\d{{1,3}})"  # «цикл оркестратора #39»
    r"|,\s*#(?P<listed>\d{1,3})\s*[),]"                              # «(2026-07-30, #48)»
    r"|#(?P<paren>\d{1,3})\s*\((?=[^)]{0,60}цикл)"                   # «#19 (автономный цикл …)»
    r")",
    re.I,
)
...
def cycle_ids(text: str) -> list:
    ...
    for m in CYCLE_RE.finditer(text):
        # Сработавшая альтернатива — ровно одна; остальные группы None.
        out.add(next(v for v in m.groupdict().values() if v is not None))
