...
echo "=== SPA-V501 Push (MP-579 — YieldOptimizer yield optimization engine) ==="
echo "Files: 5 (yield_optimizer.py, test_yield_optimizer.py, KANBAN.json, SPA_sprint_log.md, push_v501.sh)"
echo ""

echo "--- Preflight: проверка EXISTS/MISSING ---"
[ -f "$SPA_DIR/spa_core/paper_trading/yield_optimizer.py" ] \
  && echo "EXISTS : spa_core/paper_trading/yield_optimizer.py" \
  || echo "MISSING: spa_core/paper_trading/yield_optimizer.py"
[ -f "$SPA_DIR/tests/test_yield_optimizer.py" ] \
  && echo "EXISTS : tests/test_yield_optimizer.py" \
  || echo "MISSING: tests/test_yield_optimizer.py"
...
[ -f "$SPA_DIR/scripts/push_v501.sh" ] \
  && echo "EXISTS : scripts/push_v501.sh" \
  || echo "MISSING: scripts/push_v501.sh"
echo ""

[ -f "$SPA_DIR/spa_core/paper_trading/yield_optimizer.py" ] \
  && push_file "spa_core/paper_trading/yield_optimizer.py" \
  || echo "⚠️  SKIP (not found): spa_core/paper_trading/yield_optimizer.py"

[ -f "$SPA_DIR/tests/test_yield_optimizer.py" ] \
  && push_file "tests/test_yield_optimizer.py" \
  || echo "⚠️  SKIP (not found): tests/test_yield_optimizer.py"
...
[ -f "$SPA_DIR/scripts/push_v501.sh" ] \
  && push_file "scripts/push_v501.sh" \
  || echo "⚠️  SKIP (not found): scripts/push_v501.sh"

echo ""
echo "=== SPA-V501 Push Complete ==="
