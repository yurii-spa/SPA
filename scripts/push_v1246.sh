#!/bin/bash
# push_v1246.sh — push the security action items doc (2026-06-21 audit).
#
...
FILES=(
  "$PROJECT_DIR/docs/SECURITY_ACTION_ITEMS.md"
  "$PROJECT_DIR/scripts/push_v1246.sh"
)
...
python3 push_to_github.py \
  --files "${FILES[@]}" \
  --message "docs(security): add SECURITY_ACTION_ITEMS.md — CF token + Family Fund cred rotation (2026-06-21 audit)"
