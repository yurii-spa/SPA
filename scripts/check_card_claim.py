...
def entry_hit(entry, cid, *, card_claimed=None) -> tuple:
    ...
    **329 часов подряд** из записи цикла #91, у которой `card:` указывает на
    ...
    ── ЗАПИСЬ БЕЗ `card:` ВОВСЕ: спрашиваем САМУ КАРТОЧКУ (цикл #457) ──
    Строка «запись БЕЗ поля `card:` — не тронута вовсе» была намеренно оставленной дырой #262,
    и цена её измерена дважды за одни сутки. Цикл, который честно доводит работу до конца и
    оставляет СЛЕДУЮЩЕМУ циклу карточки на названные остатки, объявляет их файлы (он их везёт
    на origin) — и этим же действием запирает их; у ярлыка без pid — БЕССРОЧНО, потому что
    ветка `UNKNOWN`+`STRONG` возраста не спрашивает вовсе. Живой
    замер: запись `cycle-84717` от 2026-09-02T04:22:01Z (`verified` = sha уже сделанного пуша,
    поля `card:` нет ни какого) сделала неберущимися обе оставленные ею карточки; #456
    перебил вердикт руками, #457 получил на второй из них `НЕ ИЗМЕРЕНО` и пришёл сюда.

    Косвенный вывод из `verified` («там sha ⇒ доставлено ⇒ не захват») ПРОВЕРЕН и отвергнут:
    в той же популяции есть `"verified": "в работе"` — поле свободнотекстовое, машинного
    признака доставки в записи нет. Поэтому спрашиваем не запись, а **саму карточку**:
    настоящий захват пишет `claimed_by` во frontmatter (`check_card_claim.py claim`), и его
    отсутствие — прямой ответ, а не догадка.

    - файл карточки во владении + в записи НЕТ `card:` + у карточки НЕТ `claimed_by`
      ⇒ признак СЛАБЫЙ (у подтверждённо живой сессии по-прежнему `claimed`);
    - `card_claimed=None` — «не измерено» (карточка не прочиталась): прежний СИЛЬНЫЙ признак,
      fail-CLOSED сохранён;
    - `card_claimed=True` ⇒ прежний СИЛЬНЫЙ признак.

    Сужение защиты «сессия взяла карточку в обход инструмента» закрыто с ДРУГОЙ стороны и в
    ту же доставку: `log_session_change.py` теперь ОТКАЗЫВАЕТ объявить файл карточки трекера
    без `--card` (единственная дверь, где объявления пишутся), так что будущая запись этой
    формы просто не появится. Пересечение по `--files` — независимое измерение — не тронуто.
    """
        ...
        if Path(str(f)).name == f"{cid}.md":
            if named_other:
                return WEAK, ("файл карточки объявлен во владении, но запись машинно называет "
                              ...
                              "не считается")
            if card_claimed is False:
                return WEAK, ("файл карточки объявлен во владении, но поля `card:` в записи "
                              "нет ВОВСЕ, а у самой карточки нет `claimed_by` — настоящий "
                              "захват пишет его; это объявление файла (создана/доставлена/"
                              "дописана), а не взятие работы")
