...
- **слабый признак (упоминание в тексте) блокирует только пока свеж.** Старое упоминание уходит
  ...
  **Это верно и когда активность сессии измерить НЕ удалось** (31.07, карточка
  `agent-weak-mention-locks-card-forever`): у старого признака решает СИЛА признака, а не
  измеримость сессии. Иначе правило выше отменялось бы ровно там, где нужнее всего —
  `session_state` отдаёт `UNKNOWN` для идентификатора без pid (`cycle49`, `cycle55` …)
  детерминированно и **необратимо**, поэтому старение не наступало бы никогда, и одно
  упоминание в свободном тексте запирало бы карточку до конца существования журнала
  (так на 19 часов выпали из очереди `agent-durable-session-id` и
  `agent-idea21-verdict-data-drift` — обе были упомянуты циклом #49 В ОТРИЦАНИИ, «обе НЕ беру»).
  **fail-CLOSED при этом не ослаблен:** старый СИЛЬНЫЙ признак (`claimed_by` / поле `card:` /
  файл карточки во владении) с неизмеримой активностью по-прежнему даёт «не измерено» —
  настоящий захват всегда несёт сильный признак (`announce_claim`, цикл #54);
...
def build_report(cid, path, entries, self_session, sibling, *, now=None,
    ...
    def _classify(session, ts, source, strength, detail):
        ...
        if strength == STRONG:
            ...
            # Старый СЛАБЫЙ признак (упоминание карточки в свободном тексте) — в историю,
            # даже если активность сессии измерить не удалось. Иначе одна обмолвка запирает
            # карточку НАВСЕГДА: `session_state` отдаёт UNKNOWN для id без pid
            # детерминированно и необратимо, поэтому старение не наступило бы никогда
            # (карточка `agent-weak-mention-locks-card-forever`: так были заперты
            # `agent-durable-session-id` и `agent-idea21-verdict-data-drift`).
