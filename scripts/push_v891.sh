#!/usr/bin/env bash
# scripts/push_v891.sh
# Sprint v8.91 — MP-1245 DeFiProtocolVaultPerformanceFeeGrossOfAvsOperatorFeeBaseGapAnalyzer
# Run on Mac: bash scripts/push_v891.sh
...
MSG="v8.91 MP-1245 DeFiProtocolVaultPerformanceFeeGrossOfAvsOperatorFeeBaseGapAnalyzer (98 tests, yield_quality Tier-B weight=0.5) — perf-fee gross-of-AVS-operator-fee base gap (the restaking node-operator's commission withheld from AVS rewards in an EigenLayer-style restaking vault, struck on GROSS reward before the depositor sees net → fee-on-avs-operator-fee base inflation; distinct from gross_of_validator_commission base PoS consensus-layer commission, gross_of_slashing_loss misbehaviour penalty, boost_fee, and management_fee); B=498 total=690 [skip ci]"
