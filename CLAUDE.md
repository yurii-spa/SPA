...
| Кнопки владельцу/под тревогой гейтит СПИСОК умений бота в маячке, а не его возраст в минутах: порог кнопок **6 ч** (`alert_actions.BEACON_MAX_AGE_S`), порог живости сторожа **300 с** (`telegram_health`) — два разных вопроса, равнять их снова = вернуть «⚠️ Кнопки сейчас недоступны» на каждом перезапуске бота (ADR-400). Неизвестный `act:`-глагол вопрос не переписывает | `grep -n "^BEACON_MAX_AGE_S" spa_core/telegram/alert_actions.py spa_core/monitoring/telegram_health.py` | 2026-09-16 |
...
| **«CI красный» ≠ «тесты падают».** Шаг `spa_core/tests/` обрывался: `--timeout-method=thread` зовёт `os._exit(1)` и убивает СЕССИЮ, поэтому сводки pytest в логе может не быть ВОВСЕ и ≈96 000 тестов остаются без вердикта под словом `failure` (ADR-474). Судить по СВОДКЕ/`ci_verdict.py`, а не по коду возврата шага; порог `--timeout` не поднимать (инв. #16) | найти сводку в логе джобы: `grep " passed" <лог>` — нет строки ⇒ НЕ ИЗМЕРЕНО | 2026-09-25 |
