...
| Кнопки владельцу/под тревогой гейтит СПИСОК умений бота в маячке, а не его возраст в минутах: порог кнопок **6 ч** (`alert_actions.BEACON_MAX_AGE_S`), порог живости сторожа **300 с** (`telegram_health`) — два разных вопроса, равнять их снова = вернуть «⚠️ Кнопки сейчас недоступны» на каждом перезапуске бота (ADR-399). Неизвестный `act:`-глагол вопрос не переписывает | `grep -n "^BEACON_MAX_AGE_S" spa_core/telegram/alert_actions.py spa_core/monitoring/telegram_health.py` | 2026-09-16 |
