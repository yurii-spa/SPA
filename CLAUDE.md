...
1. **⚠️ На Маке источники §1 читать из зеркала `~/Documents/SPA_mirror`** (ADR-152).
   ...
   через API, минуя индекс; синхронизация возит только `spa_core/scripts/tests/architecture`,
   а `docs/` и `nimbalyst-local/` — никогда). Рабочее дерево **не синхронизировать**: они
