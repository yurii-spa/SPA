...
14. **Агентам ЗАПРЕЩЕНО переводить карточку решения в `owner-done` И в `owner-accepted`.**
    Оба статуса ставит только владелец. Агент двигает лишь `needs-owner → ingested` (после
    инжеста ответа) и `owner-accepted → ingested` (после ИСПОЛНЕНИЯ — то есть отчёта о
    выполненном критерии приёмки карточки; ADR-124: «✅ Принято — беру в работу» — обещание
    совершить действие, а не действие, и потому НЕтерминально).
