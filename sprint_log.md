...
### MP-1176: DeFiProtocolVaultDepositActivationLagAnalyzer (`spa_core/analytics/defi_protocol_vault_deposit_activation_lag_analyzer.py`)
- Advisory/read-only: новый капитал, депонированный в волт, может **простаивать** (не задеплоен в стратегию) N часов до начала начисления (warmup / deposit queue / epoch boundary). За время холда это снижает **эффективный реализованный APR**, т.к. ранние дни не зарабатывают. Угол: «волт 10% APR, но депозит простаивает 3 дня до деплоя; за 30-дневный холд эффективный APR ~9%, за 5-дневный ~4%». Gap подтверждён грепом: `activation_delay`/`deposit_warmup`/`capital_deployment_lag`/`warmup_period`=0. В docstring отличие от `idle_cash_drag` (структурный постоянный резерв волта), `vault_redemption_cooldown` (лок на ВЫХОДЕ) и `harvest_cycle_entry_timing` (MP-1173, позиция в harvest-цикле) — этот изолирует **разовый лаг деплоя нового капитала относительно горизонта холдера**.
- Вход: `vault`/`token`, `headline_apr_pct` (max0, ≤0→INSUFFICIENT), `activation_lag_hours` (max0), `intended_hold_days` (max0, default 30; ≤0→INSUFFICIENT).
- Метрики: `lag_days` (=lag_hours/24), `hold_days`, `earning_days` (=max(0,hold−lag)), `effective_apr_pct` (=headline*earning/hold, safe_div), `yield_drag_pct` (=headline−effective), `drag_ratio` (=clamp(lag/hold,0,1)), `lag_exceeds_hold` (bool), `is_instant` (lag_hours≤1.0). score 0–100 (ВЫШЕ=быстрее деплой): efficiency ~70 (=70*(1−drag_ratio)) + absolute_lag ~30 (=30*(1−clamp(lag_days/7,0,1))); lag=0→100.
- classification INSTANT_DEPLOYMENT (drag≤0.02) / MINOR_LAG (≤0.10) / MATERIAL_LAG (≤0.30) / SEVERE_LAG (>0.30) (+ INSUFFICIENT_DATA); recommendation DEPLOY_NOW / ACCEPTABLE_FOR_HORIZON / LENGTHEN_HORIZON_OR_VERIFY / AVOID_FOR_SHORT_HOLD (lag_exceeds_hold) / VERIFY_DATA; grade A–F; флаги INSTANT_DEPLOYMENT/MINOR_LAG/MATERIAL_LAG/SEVERE_LAG, LAG_EXCEEDS_HOLD, INSTANT_ACTIVATION, LONG_HORIZON_OK (hold≥90), SHORT_HORIZON (hold≤7), INSUFFICIENT_DATA. Пороги в константах, протестированы на границах.
- `analyze` + `analyze_portfolio` (fastest_vault/slowest_vault по score, avg_score, severe_lag_count, position_count) + класс-обёртка. Чистый stdlib, read-only/advisory, защищённые деления, sentinels без inf/NaN, atomic tempfile+os.replace, ring-buffer 100 (`data/vault_deposit_activation_lag_log.json`). Новая запись registry Tier-B (category=yield_quality).
- **175 tests green**

### MP-1177: DeFiProtocolVaultRewardAutosellSlippageAnalyzer (`spa_core/analytics/defi_protocol_vault_reward_autosell_slippage_analyzer.py`)
- Advisory/read-only: авто-компаундящий волт периодически **продаёт reward-токен** для реинвеста в андерлаинг. Если размер рекуррентной продажи велик относительно рыночной глубины reward-токена → сама операция компаундинга несёт **slippage**, тянущий реализованную доходность ниже headline. Угол: «волт сбрасывает $200k CRV еженедельно в пул глубиной $1.5M → ~6.7% slippage за харвест → реализованный APY ниже headline». Gap подтверждён грепом: `harvest_sell`/`reward_dump`/`reward_sell_slippage`/`compounding_slippage`=0. В docstring отличие от `reward_token_price_exposure` (MP-1170, РЫНОЧНЫЙ ценовой риск удержания токена), `bribe_dependency` (MP-1175, внешнее bribe-финансирование APR) и `gas_cost_breakeven` (газовые $, не slippage) — этот изолирует **execution-slippage собственной рекуррентной reward-продажи волта**.
- Вход: `vault`/`token`, `headline_apr_pct` (max0, ≤0→INSUFFICIENT), `reward_apr_pct` (max0, clamp≤headline; ≤0→NO_AUTOSELL), `harvest_sell_usd` (max0), `reward_market_depth_usd` (max0; при reward>0 и depth≤0→INSUFFICIENT).
- Метрики: `base_apr_pct` (=max(0,headline−reward)), `reward_share_pct` (safe_div), `sell_to_depth_ratio` (=sell/depth, safe_div None), `est_slippage_pct` (=clamp(ratio*0.5*100,0,100); 100% глубины≈50% slippage), `realized_reward_apr_pct` (=reward*(1−slip/100)), `reward_apr_lost_pct`, `realized_headline_apr_pct` (=base+realized_reward), `high_slippage` (≥5%), `thin_market` (ratio≥0.10). score 0–100 (ВЫШЕ=чище компаундинг): slippage ~60 (=60*(1−clamp(slip/20,0,1))) + depth ~40 (=40*(1−clamp(ratio/0.5,0,1))); NO_AUTOSELL→100.
- classification NO_AUTOSELL (reward≤0) / NEGLIGIBLE_SLIPPAGE (≤0.5) / LOW_SLIPPAGE (≤2) / MODERATE_SLIPPAGE (≤5) / HIGH_SLIPPAGE (>5) (+ INSUFFICIENT_DATA); recommendation TRUST_HEADLINE / MINOR_COMPOUNDING_DRAG / DISCOUNT_FOR_SLIPPAGE / AVOID_OR_VERIFY / VERIFY_DATA; grade A–F; флаги по классификации, THIN_MARKET, HIGH_REWARD_SHARE (≥50), NO_AUTOSELL, HIGH_SLIPPAGE, INSUFFICIENT_DATA. Пороги в константах, протестированы на границах.
- `analyze` + `analyze_portfolio` (cleanest_vault/most_slippage_vault по score, avg_score, high_slippage_count, position_count) + класс-обёртка. Чистый stdlib, read-only/advisory, защищённые деления, sentinels без inf/NaN, atomic tempfile+os.replace, ring-buffer 100 (`data/vault_reward_autosell_slippage_log.json`). Новая запись registry Tier-B (category=yield_quality).
- **167 tests green**

**Total sprint tests:** 342 (all green) | **Push:** `bash scripts/push_v829.sh`
**Note:** Self-authored yield_quality код-спринт — готовых задач type=code/status=ready в backlog не было (проверено: backlog=26, features=7, ideas=3 — только AGENT-P0/P1-* type=agent_infra, требующие git/launchd/keychain на Mac (недоступно из sandbox), P0/P1-FIX-* и USER ACTION; + P3 features-эпики и LOW ideas). Оба модуля закрывают реальные пробелы (gap-check грепом: activation_delay/deposit_warmup/capital_deployment_lag=0; harvest_sell/reward_dump/reward_sell_slippage/compounding_slippage=0): один — разовый лаг деплоя нового капитала и его влияние на эффективный APR за горизонт холда, другой — execution-slippage собственной рекуррентной reward-продажи авто-компаундера. **Architect review:** последний завершённый спринт v8.28 не кратен 5 по minor → отдельный architect review не требуется; `spa_core.dev_agents.architect` в любом случае недоступен в sandbox → выполнен ручной gap/backlog grep-review (отклонены как уже покрытые: entry/exit round-trip fee → vault_round_trip_cost + withdrawal_fee_decay; high-water-mark perf fee; harvest/keeper reliability → autocompound_keeper_reliability; funding-rate basis; tvl dominance; reward vesting/lockup → vault_reward_lock_discount MP-1162; loss socialization; capacity dilution MP-1166; deposit_cap_headroom). Оба модуля зарегистрированы в `_module_registry.py` Tier-B (yield_quality / yield_quality, weight=0.5; B=426→428); счётчик в шапке файла обновлён (Tier-A=12, Tier-B=428, Tier-C=180, всего 618→620). KANBAN: sprint_completed/sprint_current v8.28→v8.29, done MP-1176/MP-1177 добавлены, done_count 872→874. push_to_github.py НЕ запускался (sandbox); создан только `scripts/push_v829.sh` для ручного запуска на Mac (PAT fallback: Keychain→GITHUB_PAT_SPA→SPA_GITHUB_PAT→~/.github_pat, без hardcoded секретов).
**Верификация (независимо оркестратором):** py_compile OK (вкл. _module_registry.py); `python3 -m unittest` обоих → Ran 342 — OK (175+167); forbidden-import grep (numpy/pandas/requests/web3/scipy/openai/anthropic/subprocess, spa_core.risk/execution/monitoring/allocator, os.system/eval/exec) → CLEAN; registry import OK, tier_counts={'A':12,'B':428,'C':180}, обе записи находятся через get_module_info; CLI обоих --run exit 0 + валидный JSON (6 позиций каждый, полный спектр классификаций вкл. INSUFFICIENT_DATA, NO_AUTOSELL, SEVERE_LAG/LAG_EXCEEDS_HOLD); без Infinity/NaN; data-стабы сброшены в `[]`; нет `.tmp` в data/.
**STRICTLY READ-ONLY (SPA-BL-011):** risk/, execution/, monitoring/, allocator/, cycle_runner.py, golive_checker.py — НЕ тронуты. PAT не встраивался, push_*.html не создавался.
