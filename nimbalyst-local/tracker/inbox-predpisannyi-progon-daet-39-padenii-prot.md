...
дал **39 падений на 119 225 пройденных** за 3 ч 04 мин.

**Поправка к первой редакции этой карточки, снятая перемером.** Я написал здесь, что
прежняя полоса «14» мерена по ДРУГОМУ знаменателю (только `tests/`), и это неверно: 14
снято 25.08 ТОЙ ЖЕ пятикаталожной командой. Разницы знаменателей нет — есть ДРЕЙФ
полосы: **14 (25.08) → 34–35 (03–04.09) → 37 (06.09) → 39 (21.09)**, и состав наборов
между замерами расходится пофамильно. Значит сравнение по ЧИСЛУ даёт ложный вердикт в
любую сторону, а «полоса 14» перестала годиться даже грубым ориентиром ещё в начале
сентября. Ошибка была ровно того класса, против которого карточка написана: я взял число
из памяти вместо прибора.
...
Набор падений предписанной команды записан ПОИМЁННО на пришпиленном sha и снят ОДИНОКИМ
прогоном — без единого соседнего набора в том же дереве. Число при этом второстепенно:
сравнивать циклы обязан `diff` имён, потому что счёт дрейфует (14 → 35 → 37 → 39) без
единой регрессии.
