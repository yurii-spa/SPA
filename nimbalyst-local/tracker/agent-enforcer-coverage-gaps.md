...
Карточка ветки `owner-decision-proverka-knigi-slabee-proverki-pered-sde` (18.08) — тот же объект
(`policy_enforcer` против `policy.py`), но с полным замером; переношу замер, карточку не дублирую.

Из **14** порогов на обеих проверках одинаково строго работают **три**: потолок на протокол
(40 % T1 / 20 % T2) и лестница просадки. Остальное на проверке КНИГИ либо молчит, либо понижено
до предупреждения:

```
[A] 3x20pct T2 + aave 35pct -> approved=True   violations: []  warnings: []  t2_total=60.0%
[B] та же книга через проверку перед сделкой -> approved=False
    X Total T2 allocation 60.0% would exceed limit 50.0%
```

SILENT на проверке книги: суммарный T2 ≤ 50 % · суммарный L2 ≤ 50 % · коридор доходности 1…30 %.
SILENT на ОБЕИХ: суммарный высокорисковый ≤ 15 %. То есть 60 % в рисковом уровне проходит как
«здоровый портфель».
