...
## 🔎 СВЕРКА 2026-08-18 (независимая) — вердикт «подключать нечего» подтверждён замером

* Прогон: `python3 -m pytest spa_core/tests/test_unsourced_markup_positive_control.py -q`
  → **11 passed in 0.76s** (файл существует, офлайн, `data/` не трогает).
* Ключевое утверждение вердикта перепроверено СВОИМ замером, а не чтением:
  `spa_core/adapters/defillama_feed.py:206` — фид по контракту отдаёт ровно
  `{"apy","tvl","pool_id"}`; `grep` по `spa_core/adapters/*.py` на трёх характерных
  недостающих ключах (`quorum_pct`, `audit_count`, `optimal_utilization`) даёт **0 совпадений**.
  То есть «источника нет» — свойство входов, а не лень разметки.

**Карточка остаётся `backlog`:** группы «б» (19 модулей) и «в» (16 модулей) без источников,
группа «а» (1 модуль) ждёт починки ИНСТРУМЕНТА замера, а не факта.
