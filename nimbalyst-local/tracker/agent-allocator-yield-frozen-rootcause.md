...
> **ФАЗА 0 (SHADOW) РЕАЛИЗОВАНА 2026-08-02, ждёт go на пуш.** Триггер считает вердикт каждый
> цикл и пишет `data/allocation_rationale.json`, капитал не трогает. Первый замер: HOLD,
> доказанный APY 5.7986 пп, кэш 5 % объяснён; подсвечено, что morpho (3.47 %, ниже медианы)
> держит 40 % при разрешённых 20 %, и что его TVL-floor пройден по литералу. Тесты нашли
> две мои ошибки (оборот вдвое занижался на вводе кэша; правило медианы было инертным без
> потолков) — правился код. 35 тестов, 3684 passed / 0 failed. ARM — отдельный owner-gate.
...
## Перепроверка 2026-08-02 (W4, дизайн-шаг) — диагноз уточнён

Замер кода + живых данных показал: аллокатор **не заморожен** — `optimized_yield` считает оптимум
каждый цикл, ALLOC-001 не основной актуатор (основной — `diff_usd > 0.2% капитала`). Книга не
двигается потому, что **входы ранжирования не меняются: они в основном не наблюдаемы**.
Из $80k развёрнутых только $20k (pendle) ранжированы по наблюдённому числу; morpho (40%) ранжирован
по литералу 6.5% при живых 3.4657%; «пустые» sdai/sfrax/scrvusd/frax из диагноза выше — зашитые
константы (`live_apy: null`), не наблюдаемая доходность. Полный разбор + дизайн триггера —
**черновик ADR-060** (`docs/decisions/ADR-060-yield-improvement-rebalance-trigger.md`, на ревью
владельца). Найденные дефекты D1–D6 вынесены в §5 ADR-060 и требуют отдельных карточек.
