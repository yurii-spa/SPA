...
**Поправка 2026-08-18 (сверка, тем же днём позже):** число вайтлиста **35, не 36** — в тот же
день коммит `9031fb1c0` (ADR-070 п.17) снял дубль `frax`. Проверено прогоном:
`len(ADAPTER_REGISTRY) = 35`, `'frax' in names → False`. Остальные числа замера подтверждены
независимо: watchlist = 2 (`aave-v3`, `compound-v3`), покрыто каналом 2.
...
## Независимая сверка 2026-08-18 — что исполнено, что осталось

Прогон мой: `pytest spa_core/tests/test_architecture_conformance_governance.py
spa_core/tests/test_bts_monitor_honesty.py -q` → **51 passed** (12 governance + 39 BTS).
Проверка **B7** реально существует в `spa_core/monitoring/architecture_conformance.py`
(шапка стр. 36–51, ветки `B7_governance` стр. 650–683), ссылается на реальные ADR-070 п.14 /
ADR-066, а не на несуществующий ADR-071.

**Исполнено:** части 1–4 задания (чужие пространства убраны данными, дыры названы, held измерен,
семантическая проверка watchlist ∩ held встала в conformance как B7).

**Осталось (решение владельца, не работа агента):** B7 на живом дереве честно WARN —
**2 held-протокола (`euler_v2`, `yearn_v3`) держат капитал без канала наблюдения и без
названной причины**. Заполнить причину без живой сети = выдумать её. Карточка остаётся
открытой до ответа.
