...
## ЧИТАТЕЛИ ПОДКЛЮЧЕНЫ 03.09 (работа среды; плановая сессия adr183-continue-gas-cio
НЕ выстрелила — висела enabled 4.5 ч после fireAt при живом планировщике (дневной аудит
в 10:08 отработал); сделано вручную в интерактивной сессии, задача отключена, чтобы не
дала позднего дубля. Причина невыстрела не установлена — наблюдать на следующей
одноразовой задаче):

1. **CIO** — `chief_investment._load_gas()`: девятый вход через fail-CLOSED `read_feed`
   (max_age 1.5 ч по mtime; протухло/нет файла ⇒ UNKNOWN, не вчерашний газ). Режим газа —
   в `house_view.gas`; строго ВНЕ постуры и вне `n_analysts` (advisory не гейтит, ADR-168).
   Квитанция потребления пишется. 5 тестов + мутация.
2. **Дневной отчёт** — четвёртая строка надзор-секции «⛽ Цена газа» (`_build_oversight_section`),
   режимы словами + $/ногу; unchecked-сети названы поимённо; маркер в `_REQUIRED_BLOCKS` —
   пропадёт строка, отчёт сам назовёт дыру. 4 теста + мутация.
3. Манифест: файл в `consumes` CIO, `consumer_required: true` у агента.

Экономика хода (измеренный газ вместо константы $12 в money-path) — по-прежнему ОТДЕЛЬНЫЙ
ADR, не начат сознательно.

## Первоначальный план читателей (для истории; ответ владельца 30.08 «кто общается с агентом? CIO?»)
