---
trackerStatus:
  type: inbox
title: "Находка петли: com.spa.decision_loop: код и манифест называют РАЗНЫЙ продукт (только "
status: done
source: nimbalyst
created: 2026-09-09
finding_key: "B7:manifest_parity:com.spa.decision_loop"
status_trail:
  - "2026-09-10T01:29:15.384413+00:00 new -> done · queue.set_status"
---

Находка петли ADR-066 (architecture_conformance, WARN, подтверждена 2 прогонами подряд):

com.spa.decision_loop: код и манифест называют РАЗНЫЙ продукт (только в объявлении: —; только в манифесте: data/ranking_tie_census.json, data/shadow_blockade_attribution.json, data/target_stability.json) — манифест знает продукт, которого нет в объявлении — объявление отстало или пишет не точка входа

Сделано = находка исчезает из отчёта источника при следующем прогоне (мост закроет карточку сам).

_finding_key: `B7:manifest_parity:com.spa.decision_loop` · ADR-066_
