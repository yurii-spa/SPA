...
## Прогресс 2026-08-22 (ночь, автономный цикл)

**П.1 СДЕЛАН** — `spa_core/tests/test_strategy_claimed_adapters_importable.py`:
AST-скан всех `spa_core/strategies/*.py` (try/except прозрачен — суть корня S23),
каждая заявленная связка стратегия→адаптер обязана импортироваться и нести символ;
мёртвая ссылка = красный CI с именем модуля и файла стратегии. Сегодняшний корпус:
21 связка, все живые (S23→pendle_pt проходит как живая — критерий карточки).
Якорь против ослепшего скана + позитивный контроль обеих форм смерти ссылки
(нет модуля / нет символа). 3 мутации краснят (скан ослеп · try невидим ·
hasattr снят), логика проверки одна на боевой тест и контроль.

**П.4 ЗАМЕРЕН как уже закрытый** — honesty-gate доказанно срабатывает:
`spa_core/tests/test_tournament_trust_honesty.py` (9 passed 2026-08-22) держит
ровно требуемое: вырожденный/mock лидерборд ⇒ trustworthy=False, ни один путь
не штампует True на вырожденном, API fail-closed на отсутствующем флаге.
Тихо заглушить gate нельзя — тест покраснеет.

**Остаётся (п.2, п.3):** AST-линт на `except: pass` вокруг импортов в стратегиях —
аккуратно: рантайм-глотание в S23 осознанное (ADR-059, честный `*_live=False`),
запрет в лоб покрасит принятый дизайн; и пометка/исключение mock-записей в самом
турнире (стыкуется с agent-tournament-trustworthy-real-apy). Ночному циклу не по
мандату big-bang — оставлено следующей итерации.
