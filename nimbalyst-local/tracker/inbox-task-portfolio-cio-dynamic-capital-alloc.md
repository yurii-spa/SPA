...
## Приоритет понижен `critical` → `high` (решение владельца, 2026-08-29)

Владелец после аудита (`docs/research/RS-portfolio-cio-audit-2026-08-29.md`): «понизь приоритет
якорной карточки с critical». Возврат к тому уровню, который он же поставил 19.08 (цикл #310);
`critical` карточка носила с 26.08.

**Почему это честно, а не занижение.** Аудит 29.08 показал: из 76 требований ТЗ 22 выполнено,
24 построено и обесточено, 30 не начато — и решающий слой CIO («куда/сколько/почему») не строится
сейчас ни одной сессией. Карточка со статусом `critical`, не двигавшаяся с 27.08, не срочность
обозначала, а маскировала простой: рядом с ней в очереди стоят задачи, которые реально исполняются.
Понижение до `high` возвращает ей честный вес — важная и живая, но не «бросить всё».

**Что НЕ изменилось:** статус остаётся `in-progress`, задача не закрыта, следующий шаг назван и
заведён отдельной карточкой `agent-zamer-g1-dolya-kapitala-na-nablyudennyh-chislah` (замер G1 с
хоста). Требования ТЗ ниже — в силе полностью.
