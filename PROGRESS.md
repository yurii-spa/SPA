...
## DOCS UNIFICATION — single canonical doc system (ADR-YL-009, owner-directed, 2026-07-02)
- **INVENTORY:** site (landing/src, node_modules noise excluded) references **14** docs/*.md — ALL RESOLVE (0 dangling): decision_index/underwriting_rubric/non_ethena_ladder + STRUCTURAL_DESK/RATES_DESK_VALIDATION/DD_PACK/FUNDABILITY/LIQUIDATOR_DERISK/PROOF_CHAIN_SPEC/COMPETITIVE_POSITION/VERIFIER_RELEASE/DFB_METHODOLOGY/SITE_DESIGN_SYSTEM(_V2). Total docs/*.md = **204** (45 NN-framework + ~155 legacy operational).
- **MAPPING (8 concepts A↔B):** mandate, RWA-floor, evidence-levels, underwriting-method, verdicts↔lifecycle, product-lines, capital-tiers, honest-edge-finding — mapped via parallel Explore agent.
- **CONFLICTS found: 3.** **C1 mandate** (charter "find 10-15%" vs honest ~4.75%) — **RESOLVED by ADR-YL-008** (search-range judged-as-spread), applied via canonical banners. **C2 verdict-taxonomy** (A's ADVANCE/WATCH/REFUSE/NO-EDGE/BASELINE vs B's lifecycle statuses) — ADR-YL-008 does NOT resolve → **OQ-11** (recommended bridge-mapping, owner-confirm). **C3 product-line bands** (B's absolute-APY Core=7-10% vs A's real Maple~4.7%=Core) — → **OQ-12** (recommended: spread-based docs/17 canonical) + FIXED the broken docs/07→docs/34 pointer (bands were never in 34).
- **CANONIZATION:** created **ADR-YL-009** (one-concept-one-file rule + concept→canonical-file map + 'site references only existing files' rule + operational-docs-keep-names + framework-docs-reference-not-duplicate). Rewrote **docs/00_index.md** as master index (Layer A operational / Layer B framework / Layer C ADR + canon-per-concept table). Inserted **5 canonical banners** (07/33/17/34/37) deduplicating mandate + product-lines + evidence + capital-tiers to their canon.
...
- **Host-fix DEPLOYED + verified live (owner 'деплой host-fix', 2026-07-03):** pushed 4 files to main (dbc3aac6) → CF Pages rebuilt ~60s → **apex X-Robots-Tag noindex REMOVED (verified live, empty header)** — DD/AI tools can now read canonical earn-defi.com; robots Allow:/+Disallow:/api/+Sitemap live; /api stays noindex; sitemap has yield-lab. **www→apex 301 NOT live** — needs owner Cloudflare-panel step (add www as custom domain on Pages project OR Redirect Rule). 4/5 verified; the critical apex-noindex fix is DONE.
- **Auto-sprint batch #5 (2 parallel agents, 2026-07-03):** **Maple-HY DD** — borrower-concentration number NOT public but ON-CHAIN retrievable (PoR) → stays CONDITIONAL-PASS (soft, not hard opacity like Centrifuge), ONE PoR-query from ADVANCE-with-cap; recourse RESOLVED (dual: overcollat 150-500% + legal recourse to borrower); zero losses post-rebuild thru Oct-2025 stress. `concentration_unverified_but_onchain_retrievable`. **Term Finance** — real organic non-Ethena fixed-rate (+400-1000bps) but DEPTH thin/declining ($12.5M, −74%; ~$1M not $10M) + TERM points + PT-sUSDe-collat leak → ADVANCE-WITH-CAP `thin_declining_depth`. **SYNTHESIS: ALL non-Ethena fixed-rate venues (Frax FXB/Notional/Term) are uniformly THIN — deep non-Ethena fixed-rate does NOT exist in 2026; only Ethena PT is deep.** decision_index 36; rubric +2; ladder +fixed-rate-synthesis.


- **Auto-sprint batch #6 (tokenized-credit screen, 2026-07-03):** scoped integrity 62 passed, 0 dangling links. Screened Clearpool/TrueFi/Goldfinch/Huma/Superstate-USTB for the Maple-HY on-chain-buffer trait: **Huma Finance → WATCH** (on-chain first-loss tranche like Maple, ~10.5% Classic, BUT points-subsidized Maxi + off-chain originator); Clearpool→NO-EDGE (no on-chain buffer, $1M dead); TrueFi/Goldfinch→REFUSE (realized defaults + winddown, GIP-87 confirmed dead); USTB→NO-EDGE (IS the floor 3.44%, deep, now Invesco). **Maple-HY remains the SOLE standout non-Ethena publicly-underwritable credit rung.** decision_index 37; rubric +1.


## STATIC PUBLIC NUMBERS — 3 public-layer fixes (branch fix/static-numbers, 2026-07-03)
- **FIX-1 (static snapshot):** `scripts/generate_track_snapshot.py` now also reads `paper_trading_status.json` (the API's authority) → equity / paper-APY / PoR-NAV from ONE source; adds `as_of` (data freshness = last evidenced bar date), `generated_at`, `paper_apy_pct`, `nav_usd`, `max_drawdown_pct`. Missing value => None → site renders honest "data unavailable", NEVER a stale number. Hero + track-record now render these STATICALLY with an "as of <date>" label (crawlers/AI-DD see real numbers, not JS-only "—"); JS still overrides to live. `landing/package.json` +prebuild (regenerate snapshot from committed data at build, non-fatal `|| true`).
- **FIX-2 (one number, one source):** removed **14 hardcoded metric fallbacks** across the site — index.astro (3: `?? 10`, `?? 27`, `?? 29` + apy-re-derived-from-bars→single-source `paper_apy_pct`), track-record.astro (6: `?? 10/30/27/29` + anchor/target literals), due-diligence.astro (1: `?? 10`), status.astro (4: `?? 27/29/10/30`). Every public metric is now single-source from `track_snapshot.json`; a missing field shows an honest em-dash. grep confirms 0 remaining `snap.X ?? <literal>`.
- **FIX-3 (ledger-gap footnote):** track-record bar-ledger gains a 1-2 sentence honest footnote — the warmup→backfill equity jump is expected (independent synthetic series, not part of the evidenced track; only cycle bars are continuous + counted).
- **VERIFY:** `npm run build` (prebuild regenerates snapshot real_track_days=11 gates=27/29) → 75 pages, 0 error. **curl no-JS on dist:** hero = `11 · ~3.7% · 27/29 · as of 2026-07-02`; track-record = `11 honest days · anchor 2026-06-22 · static snapshot as of 2026-07-02` + gap-footnote present. **hero ↔ track-record MATCH (11 / 27/29, one source).** Test `test_track_snapshot.py` 3 passed (fields present, one-source, honest-None-not-hardcoded, committed-snapshot-shape).
- **NOT DEPLOYED** — branch `fix/static-numbers`, 3 staged commits; owner deploys. Runtime/RiskPolicy/proof-pipeline NOT touched.


- **Auto-sprint whole-map cycle (2026-07-03):** (C measurement-moat) wrote **`docs/yield_landscape_2026.md`** — session-synthesis of all 37 decisions into ONE honest-landscape read (the one principle / floor-is-a-floor / ADVANCE rungs deep-vs-thin / 8-12% standout=Maple-HY / Ethena-concentration / structural scarcities / REFUSE archetypes / pick-two rule / edge=measurement-role), linked from decision_index + 00_index. (A) **2026 RWA-majors screen** — DEFINITIVELY closes 'do the big issuers offer a bounded spread?': **NO** — BUIDL/BENJI/OUSG/JPM MONY/USYC all `gov_tbill_at_floor` (ARE the floor, expansion=distribution not yield); only spreads = USDY bank-deposit-tail REFUSE + USCC funding-basis REFUSE (now 2.60%<floor). (D) research 7 + track_snapshot 3 passed, 0 dangling links (OTS tests live on fix/external-anchoring branch, not this one). decision_index 38; rubric +3 (gov_tbill_at_floor / tail_comp_deposit_spread / funding_dependent_basis_carry).


- **Auto-sprint cycle (B deepen-WATCH, 2026-07-03):** Huma deep DD (the WATCH closest to ADVANCE) → **STAYS WATCH.** Points-contamination RESOLVED for measurement (Classic = real ~8-10.5% organic, +4.6-7.1pp), but ADVANCE blocked on non-public binding numbers: (a) first-loss depth % NOT published for the $183M permissionless Solana pool (docs=per-pool-config; the 2-10%/junior≥20% figures are Institutional-scoped only), (b) only self-reported 0%-default (L1, not on-chain-auditable), (c) **98% single-originator (Arf) + off-chain receivable underwriting** = structural hold; + V1-Polygon $101k exploit (fees-only, hygiene flag). `offchain_originator_underwriting_unverified + single_originator_concentration_98pct`. Contrast Maple-HY (buffer PoR-retrievable=soft-conditional) vs Huma (per-pool-unpublished=harder). DEFERRED the Fluid advisory-sleeve build to a clean-branch cycle (avoid a half-synced strategy_lab push from fix/static-numbers). decision_index 38; rubric +1.


- **Auto-sprint (E build, 2026-07-03):** **Fluid plain USDC/USDT supply advisory sleeve BUILT** (`fluid`, CAND-FLUID-001) — the 1st DEEP non-Ethena ADVANCE-grade rung sleeved. SAFE path: verified all 4 strategy_lab files local==main (raw.githubusercontent, no-clobber) before editing; added strategies.fluid + capacity.fluid ($1.01B TVL, 2% pool) to committed config (+local override for Mac runtime), config.py _REQUIRED_STRATEGY, paper.py + backtest.py loops, test VALID fixture. ~4.7% (mid 4.3-5.5% organic, DeFiLlama 2026-07-03), is_advisory, T2, $100k, drawdown_stop 12%. **strategy_lab suite 21 passed**; sleeve instantiates (id=fluid apy=4.70%); hourly com.spa.strategy_lab_paper picks it up → forward track data/strategy_lab_paper/fluid_series.json. Pushed 64864d84 (5 committed files). NOT go-live-track/RiskPolicy/cycle_runner. 5 advisory sleeves now (pt_susde/pt_usde/maple_syrup/centrifuge_drop/fluid).
