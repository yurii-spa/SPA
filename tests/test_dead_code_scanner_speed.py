...
* **semantics** — ``_collect_file_facts`` is checked against reference
  implementations of the five original walks, written out in full below. These
  are equivalence controls: they pass on both the old and the new scanner, and
  they are what would catch a refactor that silently drops (or invents) an
  exclusion. The reference code is copied from the pre-fix implementation on
  purpose — comparing the new code against itself would prove nothing.
* **the fix itself** — ``test_scan_makes_a_single_traversal`` counts node
  visits and is RED on the old five-walk scanner. Without it, the walks could
  come back one at a time and only a CI timeout would notice, months later.
