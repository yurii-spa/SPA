...
==========================
25 unit tests for spa_core/devtools/auto_fixer.py
...
from spa_core.devtools.auto_fixer import (
    BASE_DIR,
    BACKUP_PREFIX,
    CLAUDE_MODEL,
    FORBIDDEN_FILES,
...
class TestFindAffectedFile:
    ...
    def test_ignores_forbidden_dirs_in_traceback(self, tmp_path):
        ...
        with patch("spa_core.devtools.auto_fixer.BASE_DIR", tmp_path):
            result = find_affected_file(text)
        assert result is None
...
class TestRateLimit:
    def test_not_rate_limited_initially(self, tmp_path, monkeypatch):
        monkeypatch.setattr("spa_core.devtools.auto_fixer.RATE_LIMIT_PREFIX",
                            str(tmp_path) + "/rl_")
        assert is_rate_limited() is False

    def test_rate_limited_after_max_fixes(self, tmp_path, monkeypatch):
        prefix = str(tmp_path) + "/rl_"
        monkeypatch.setattr("spa_core.devtools.auto_fixer.RATE_LIMIT_PREFIX", prefix)
...
class TestRunAutoFix:
    def test_no_api_key_graceful_degradation(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("spa_core.devtools.auto_fixer.RATE_LIMIT_PREFIX",
                            str(tmp_path) + "/rl_")
        with patch("spa_core.devtools.auto_fixer.get_anthropic_key", return_value=None):
            with patch("spa_core.devtools.auto_fixer._tg_request") as mock_tg:
                result = run_auto_fix(
                    "AttributeError: test",
                    token="TOKEN",
                    chat_id="CHAT",
    ...
    def test_fix_applied_and_pushed_on_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("spa_core.devtools.auto_fixer.RATE_LIMIT_PREFIX",
                            str(tmp_path) + "/rl_")
        monkeypatch.setattr("spa_core.devtools.auto_fixer.BACKUP_PREFIX",
                            str(tmp_path) + "/bak_")
        ...
        with patch("spa_core.devtools.auto_fixer.BASE_DIR", tmp_path):
            with patch("spa_core.devtools.auto_fixer.get_anthropic_key", return_value="key"):
                with patch("spa_core.devtools.auto_fixer.call_claude_api",
                           return_value="def broken():\n    x = None\n    return x or ''\n"):
                    with patch("spa_core.devtools.auto_fixer.run_tests",
                               return_value=(True, "3 passed")):
                        with patch("spa_core.devtools.auto_fixer.push_file",
                                   return_value="abc1234"):
                            with patch("spa_core.devtools.auto_fixer._tg_request"):
