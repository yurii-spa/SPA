...
- **CAPACITY DD RESULT (sourced 2026-07-02 — the last open gate, now QUANTIFIED):**
  - PT-sUSDe on Pendle = **Total Liquidity ~$200.89M / TVL $260.28M across 2 markets** — the **deepest PT market that exists** ("no other venue matches sUSDe depth"). BUT **24h volume only ~$1.08M** (thin daily flow) + near-expiry AMM flattening (exit liquidity thins in the final weeks).
  - **Capacity ceiling (honest):** at **$100k–~$2M per maturity**, held-to-maturity → entry slippage modest (<pool-moving), exit not needed → **fully fundable**. At **~$5–10M** you're 2.5–5% of the pool → material entry slippage + a forced early exit is costly (you'd be multiples of daily volume). At **$50–100M** PT-sUSDe **cannot** absorb it alone (25–50%+ of all liquidity).
  - **Resolution:** the **RISK logic clears (ADVANCE-to-paper); SCALE is capacity-bounded to ~single-digit $M per maturity.** To scale, **ladder across maturities + markets** (PT-USDe, PT-syrupUSDC, other stables) and pair with the deep core (RWA floor + overcollat lending, which scale to ~$100M). This confirms the desk's known "edge cliff is thin-PT-depth, not the deep core."
- **remaining conditions (paper → live):** (1) ✅ **PT capacity** — ANSWERED: fundable ≤~$2M/maturity, ceiling ~single-digit $M, ladder for scale; (2) **held-to-maturity discipline** + maturity-laddering (no MTM panic-exit), (3) **strict single-underlying + per-maturity cap**, (4) live monitors: negative-funding-regime duration + reserve-coverage % + PoR freshness (kill if reserve coverage breaks a floor), (5) full Red-Team sign-off.
